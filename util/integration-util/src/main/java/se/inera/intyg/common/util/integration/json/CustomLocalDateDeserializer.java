 ...
 *
 *     <ul>
 *       The ISODateTimeFormat.dateTimeParser() is used instead of the
 *       ISODateTimeFormat.localDateParser()
 * </ul>
 *
 * <ul>
 *   In method deserializer the case START_ARRAY handles year, month and day as the original method
 *   does but here we do not check if, after day, the next token is END_ARRAY since it's possible
 *   next token can be an int value (hours).
 * </ul>
 *
...
public class CustomLocalDateDeserializer extends StdDeserializer<LocalDate> {
  ...
  /**
   * <strong>Description copied from class: JsonDeserializer</strong> Method that can be called to
   * ask implementation to deserialize json content into the value type this serializer handles.
    ...
    switch (jp.getCurrentToken()) {
      ...
      case VALUE_STRING:
        ...
        return LocalDate.parse(
            str, str.contains("T") ? DateTimeFormatter.ISO_DATE_TIME : DateTimeFormatter.ISO_DATE);
      default:
    }

    throw ctxt.wrongTokenException(
        jp, LocalDate.class, JsonToken.START_ARRAY, "expected JSON Array, Number or String");
  }
}
