...
public class LisjpModuleApiV1 extends FkParentModuleApi<LisjpUtlatandeV1> {
  ...
  public PdfResponse pdf(
    ...
    return generatePdf(
        new DefaultLisjpPdfDefinitionBuilder(pdfWatermarkDescription),
        statuses,
        luseIntyg,
        applicationOrigin,
  ...
  @Override
  protected LisjpUtlatandeV1 getInternal(String internalModel, LocalDateTime created)
      throws ModuleException {
    try {
      final var lisjpUtlatandeV1 = objectMapper.readValue(internalModel, LisjpUtlatandeV1.class);
  ...
  @Override
  protected RegisterCertificateType internalToTransport(LisjpUtlatandeV1 utlatande)
  ...
  @Override
  protected LisjpUtlatandeV1 decorateWithSignature(
      LisjpUtlatandeV1 utlatande, String base64EncodedSignatureXml) {
  ...
  public String createRenewalFromTemplate(CreateDraftCopyHolder draftCopyHolder, Utlatande template)
    ...
    try {
      ...
      Optional<LocalDate> lastDateOfLastIntyg =
          internal.getSjukskrivningar().stream()
              .sorted(
                  (s1, s2) ->
                      s2.getPeriod()
                          .getTom()
      ...
      Optional<Sjukskrivning.SjukskrivningsGrad> lastSjukskrivningsgradOfLastIntyg =
          internal.getSjukskrivningar().stream()
              .sorted(
                  (s1, s2) ->
                      s2.getPeriod()
      ...
      lastSjukskrivningsgradOfLastIntyg.ifPresent(
          grad -> relation.setSistaSjukskrivningsgrad(grad.getLabel()));
  ...
  private PdfResponse generatePdf(
      AbstractLisjpPdfDefinitionBuilder builder,
      List<Status> statuses,
      LisjpUtlatandeV1 luseIntyg,
      ApplicationOrigin applicationOrigin,
      String filePrefix,
      UtkastStatus utkastStatus)
      throws ModuleException {
    try {
      ...
      final FkPdfDefinition fkPdfDefinition =
          builder.buildPdfDefinition(
              luseIntyg, statuses, applicationOrigin, texts, utkastStatus, pdfMinaIntygMarginText);

      return new PdfResponse(
          PdfGenerator.generatePdf(fkPdfDefinition, filePrefix),
  ...
  public String getJsonFromUtlatande(Utlatande utlatande) throws ModuleException {
    ...
    throw new IllegalArgumentException(
        "Utlatande was not instance of class LisjpUtlatandeV1, utlatande was instance of class: "
            + message);
  }
  ...
  public String getUpdatedJsonWithTestData(
    ...
    TestabilityToolkit.fillCertificateWithTestData(
        certificate, fillType, new LispTestabilityCertificateTestdataProvider());
    final var updateInternalCertificate =
        CertificateToInternal.convert(certificate, internalCertificate, moduleService);
    return toInternalModelResponse(updateInternalCertificate);
  }
  ...
  public void decorate(Certificate certificate, Map<String, CertificateDataElement> data) {
    final var previousLastDateRange =
        data.entrySet().stream()
            .filter(entry -> entry.getKey().equals(BEHOV_AV_SJUKSKRIVNING_SVAR_ID_32))
            .filter(
                entry -> entry.getValue().getValue() instanceof CertificateDataValueDateRangeList)
    ...
    certificate
        ...
        .forEach(
            entry -> {
              ...
              final var config =
                  (CertificateDataConfigCheckboxDateRangeList) entry.getValue().getConfig();
              final var configCheckboxDateRangeList =
                  config.withPreviousDateRangeText(
