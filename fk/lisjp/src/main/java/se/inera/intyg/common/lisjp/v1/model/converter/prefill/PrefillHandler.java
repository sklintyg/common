...
/**
 * Decorates a {@link LisjpUtlatandeV1} with corresponding values from a {@link Forifyllnad}
 * element. This process is similar to that of what {@link
 * se.inera.intyg.common.lisjp.v1.model.converter.TransportToInternal} executes when converting a
 * stored signed {@link se.riv.clinicalprocess.healthcond.certificate.v3.Intyg} to a {@link
 * LisjpUtlatandeV1}.
 *
 * <p>Since the CreateDraftCertificate schema is rather relaxed about what a Delsvar element may
 * contain and the source is not a fully validated intyg, there are a lot more errorhandling and
 * defensive measures in place to make sure the creation of a draft is not stopped by any problems
 * during the prefill process.
 */
public class PrefillHandler {
  ...
  static final String WARNING_INVALID_BOOLEAN_FIELD =
      "Ignoring - expected a String with 'true'/'false'";
  static final String WARNING_INVALID_STRING_FIELD = "Ignoring - expected a String delsvar element";
  static final String WARNING_INVALID_STRING_MAXLENGTH =
      "Ignoring - expected a String delsvar with maxlength of %s but was %s";
  static final String WARNING_INVALID_DATEPERIOD_CONTENT = "Ignoring - failed to parse DatePeriod";
  static final String WARNING_INVALID_DATE_CONTENT =
      "Ignoring - expected a date string in format yyyy-MM-dd";
  static final String WARNING_MISSING_DATE_DEFAULTING_TO =
      "No valid yyyy-MM-dd date provided for delsvar %s - defaulting to %s";
  static final String INFO_DUPLICATE_VALUES = "Value already exists and will be ignored";
  private static final String WARNING_MISSING_MANDATORY_CVTYPE_FOR_DELSVAR =
      "Ignoring - Missing mandatory CVType for delsvar %s";
  ...
  private static final String WARNING_MISSING_SJUKSKRIVNINGSNIVA =
      "Ignoring - missing sjukskrivningsniva";
  private static final List<String> VALID_DIAGNOSE_CODESYSTEM_VALUES =
      Arrays.asList(Diagnoskodverk.ICD_10_SE.getCodeSystem());
  ...
  public PrefillHandler(
      WebcertModuleService moduleService, String intygsId, String intygsTyp, String intygsVersion) {
  ...
  public PrefillResult prefill(Builder utlatande, Forifyllnad forifyllnad) {
    ...
    if (forifyllnad != null) {
      // These properties are lists and are collected throughout the loop and added after all Svar
      // has been processed.
      ...
      for (Svar svar : forifyllnad.getSvar()) {
        try {
          ...
          pr.addMessage(
              PrefillEventType.WARNING, pw.getSvarId(), pw.getMessage(), pw.getSourceContent());
        } catch (Exception e) {
          LOG.error(
              "Unexpected error while processing svar (NOTE: this indicates a bug or unhandled scenario in the prefill process!)",
              e);
          pr.addMessage(
              PrefillEventType.WARNING,
              svar,
              "Ignoring - Unexpected error while processing svar " + e.getMessage());
        }
      }
  ...
  private void handleAvstangningSmittskydd(Builder utlatande, Svar svar)
  ...
  private void handleGrundForMedicinsktUnderlag(Builder utlatande, Svar svar, PrefillResult pr)
      ...
      try {
        switch (delsvar.getId()) {
          ...
          case GRUNDFORMEDICINSKTUNDERLAG_TYP_DELSVAR_ID_1:
            String validatedCVType =
                getValidatedCVTypeCodeContent(delsvar, GRUNDFORMEDICINSKTUNDERLAG_CODE_SYSTEM);
            try {
              grundForMedicinsktUnderlagTyp =
                  RespConstants.ReferensTyp.byTransportId(validatedCVType);
            } catch (Exception e) {
          ...
          case GRUNDFORMEDICINSKTUNDERLAG_ANNANBESKRIVNING_DELSVAR_ID_1:
            utlatande.setAnnatGrundForMUBeskrivning(
                getValidatedString(delsvar, DEFAULT_FREETEXT_MAXLENGTH));
            break;
          default:
            throw new PrefillWarningException(delsvar, WARNING_INVALID_DELSVAR_ID);
        }
        ...
        pr.addMessage(
            PrefillEventType.WARNING, e.getSvarId(), e.getMessage(), e.getSourceContent());
      }
    }
    ...
    if (grundForMedicinsktUnderlagTyp == null) {
      throw new PrefillWarningException(
          svar,
          String.format(
              WARNING_MISSING_MANDATORY_CVTYPE_FOR_DELSVAR,
    ...
    if (grundForMedicinsktUnderlagDatum == null
      ...
      pr.addMessage(
          ...
          svar,
          String.format(
              WARNING_MISSING_DATE_DEFAULTING_TO,
  ...
  private void handleArbetslivsinriktadeAtgarder(
      List<ArbetslivsinriktadeAtgarder> arbetslivsinriktadeAtgarder, Svar svar, PrefillResult pr)
      throws PrefillWarningException {
    for (Delsvar delsvar : svar.getDelsvar()) {
      switch (delsvar.getId()) {
        case ARBETSLIVSINRIKTADE_ATGARDER_VAL_DELSVAR_ID_40:
          String arbetslivsinriktadeAtgarderValKod =
              getValidatedCVTypeCodeContent(delsvar, ARBETSLIVSINRIKTADE_ATGARDER_CODE_SYSTEM);
          try {
            final var arbetslivsinriktadAtgard =
                ArbetslivsinriktadeAtgarder.create(
                    ArbetslivsinriktadeAtgarderVal.fromId(arbetslivsinriktadeAtgarderValKod));
            addIfNotPresentOrElseLog(
                arbetslivsinriktadeAtgarder, arbetslivsinriktadAtgard, pr, svar);
          } catch (Exception e) {
            throw new PrefillWarningException(delsvar, WARNING_INVALID_CVTYPE_CODE_VALUE);
          }
          break;
  ...
  private void handleArbetslivsinriktadeAtgarderBeskrivning(
      LisjpUtlatandeV1.Builder utlatande, Svar svar) throws PrefillWarningException {
    for (Delsvar delsvar : svar.getDelsvar()) {
      switch (delsvar.getId()) {
        case ARBETSLIVSINRIKTADE_ATGARDER_BESKRIVNING_DELSVAR_ID_44:
          utlatande.setArbetslivsinriktadeAtgarderBeskrivning(
              getValidatedString(delsvar, DEFAULT_FREETEXT_MAXLENGTH));
          break;
        default:
          throw new PrefillWarningException(delsvar, WARNING_INVALID_DELSVAR_ID);
  ...
  private void handlePrognos(LisjpUtlatandeV1.Builder utlatande, Svar svar)
      throws PrefillWarningException {
    ...
    for (Delsvar delsvar : svar.getDelsvar()) {
      switch (delsvar.getId()) {
        ...
        case PROGNOS_DAGAR_TILL_ARBETE_DELSVAR_ID_39:
          dagarTillArbete =
              getValidatedCVTypeCodeContent(delsvar, PROGNOS_DAGAR_TILL_ARBETE_CODE_SYSTEM);
          break;
        default:
          throw new PrefillWarningException(delsvar, WARNING_INVALID_DELSVAR_ID);
      }
      if (prognosKod != null) {
        utlatande.setPrognos(
            Prognos.create(
                PrognosTyp.fromId(prognosKod),
                dagarTillArbete != null
                    ? PrognosDagarTillArbeteTyp.fromId(dagarTillArbete)
                    : null));
      }
    }
  }

  private void handleArbetsresor(LisjpUtlatandeV1.Builder utlatande, Svar svar)
      throws PrefillWarningException {
    for (Delsvar delsvar : svar.getDelsvar()) {
      switch (delsvar.getId()) {
  ...
  private void handleArbetstidsforlaggning(LisjpUtlatandeV1.Builder utlatande, Svar svar)
      throws PrefillWarningException {
    for (Delsvar delsvar : svar.getDelsvar()) {
      switch (delsvar.getId()) {
        ...
        case ARBETSTIDSFORLAGGNING_MOTIVERING_SVAR_ID_33:
          utlatande.setArbetstidsforlaggningMotivering(
              getValidatedString(delsvar, DEFAULT_FREETEXT_MAXLENGTH));
          break;
        default:
          throw new PrefillWarningException(delsvar, WARNING_INVALID_DELSVAR_ID);
    ...
    for (Delsvar delsvar : svar.getDelsvar()) {
      switch (delsvar.getId()) {
        case FORSAKRINGSMEDICINSKT_BESLUTSSTOD_DELSVAR_ID_37:
          utlatande.setForsakringsmedicinsktBeslutsstod(
              getValidatedString(delsvar, DEFAULT_FREETEXT_MAXLENGTH));
          break;
        default:
          throw new PrefillWarningException(delsvar, WARNING_INVALID_DELSVAR_ID);
  ...
  private void handleBehovAvSjukskrivning(
      List<Sjukskrivning> sjukskrivningar, Svar svar, PrefillResult pr)
      throws PrefillWarningException {
      ...
      try {
        switch (delsvar.getId()) {
          case BEHOV_AV_SJUKSKRIVNING_NIVA_DELSVARSVAR_ID_32:
            sjukskrivningsnivaString =
                getValidatedCVTypeCodeContent(delsvar, SJUKSKRIVNING_CODE_SYSTEM);
            try {
          ...
          case BEHOV_AV_SJUKSKRIVNING_PERIOD_DELSVARSVAR_ID_32:
            DatePeriodType datePeriod =
                getValidatedDatePeriodTypeContent(delsvar, defaultStartDate, pr);
            period =
                new InternalLocalDateInterval(
                    nullToEmpty(datePeriod.getStart()), nullToEmpty(datePeriod.getEnd()));
            break;
          default:
            throw new PrefillWarningException(delsvar, WARNING_INVALID_DELSVAR_ID);
        }
    ...
    // If we could not find a period, we default to a period with a default startDate and an empty
    // enddate
    if (period == null) {
      pr.addMessage(
          PrefillEventType.INFO,
          ...
          String.format(
              WARNING_MISSING_DATE_DEFAULTING_TO,
              BEHOV_AV_SJUKSKRIVNING_PERIOD_DELSVARSVAR_ID_32,
              defaultStartDate));
  ...
  private void handleFunktionsnedsattning(LisjpUtlatandeV1.Builder utlatande, Svar svar)
      throws PrefillWarningException {
    for (Delsvar delsvar : svar.getDelsvar()) {
      switch (delsvar.getId()) {
        case FUNKTIONSNEDSATTNING_DELSVAR_ID_35:
          utlatande.setFunktionsnedsattning(
              getValidatedString(delsvar, DEFAULT_FREETEXT_MAXLENGTH));
          break;
        default:
          throw new PrefillWarningException(delsvar, WARNING_INVALID_DELSVAR_ID);
  ...
  private void handleSysselsattning(
      List<Sysselsattning> sysselsattningar, Svar svar, PrefillResult pr)
      throws PrefillWarningException {
    for (Delsvar delsvar : svar.getDelsvar()) {
      switch (delsvar.getId()) {
        case TYP_AV_SYSSELSATTNING_DELSVAR_ID_28:
          String sysselsattningsTypString =
              getValidatedCVTypeCodeContent(delsvar, TYP_AV_SYSSELSATTNING_CODE_SYSTEM);
          try {
            final var sysselsattning =
                Sysselsattning.create(SysselsattningsTyp.fromId(sysselsattningsTypString));
  ...
  public static <T> void addIfNotPresentOrElseLog(
      List<T> list, T entry, PrefillResult pr, Svar svar) {
  ...
  private void handleAktivitetsbegransning(LisjpUtlatandeV1.Builder utlatande, Svar svar)
      throws PrefillWarningException {
    for (Delsvar delsvar : svar.getDelsvar()) {
      switch (delsvar.getId()) {
        case AKTIVITETSBEGRANSNING_DELSVAR_ID_17:
          utlatande.setAktivitetsbegransning(
              getValidatedString(delsvar, DEFAULT_FREETEXT_MAXLENGTH));
          break;
        default:
          throw new PrefillWarningException(delsvar, WARNING_INVALID_DELSVAR_ID);
  ...
  private void handlePagaendeBehandling(LisjpUtlatandeV1.Builder utlatande, Svar svar)
      throws PrefillWarningException {
    for (Delsvar delsvar : svar.getDelsvar()) {
      switch (delsvar.getId()) {
  ...
  private void handlePlaneradBehandling(LisjpUtlatandeV1.Builder utlatande, Svar svar)
      throws PrefillWarningException {
    for (Delsvar delsvar : svar.getDelsvar()) {
      switch (delsvar.getId()) {
  ...
  private void handleOvrigt(LisjpUtlatandeV1.Builder utlatande, Svar svar)
      throws PrefillWarningException {
    for (Delsvar delsvar : svar.getDelsvar()) {
      switch (delsvar.getId()) {
  ...
  private void handleOnskarKontakt(LisjpUtlatandeV1.Builder utlatande, Svar svar)
      throws PrefillWarningException {
    for (Delsvar delsvar : svar.getDelsvar()) {
      switch (delsvar.getId()) {
        ...
        case ANLEDNING_TILL_KONTAKT_DELSVAR_ID_26:
          utlatande.setAnledningTillKontakt(
              getValidatedString(delsvar, DEFAULT_FREETEXT_MAXLENGTH));
          break;
        default:
          throw new PrefillWarningException(delsvar, WARNING_INVALID_DELSVAR_ID);
  ...
  private void handleDiagnos(List<Diagnos> diagnoser, Svar svar, PrefillResult pr) {
      ...
      try {
        switch (delsvar.getId()) {
          ...
          case BIDIAGNOS_1_DELSVAR_ID_6:
            CVType bidiagnos1 =
                getValidatedCVTypeContent(delsvar, VALID_DIAGNOSE_CODESYSTEM_VALUES);
          ...
          case BIDIAGNOS_1_BESKRIVNING_DELSVAR_ID_6:
            bidiagnosBeskrivning1 =
                getValidatedString(delsvar, DEFAULT_DIAGNOSDESCRIPTION_MAXLENGTH);
            break;
    ...
    if (bidiagnosKod1 != null) {
      ...
      final var diagnos =
          Diagnos.create(
              bidiagnosKod1,
              bidiagnoskodverk1.toString(),
              bidiagnosBeskrivning1,
              bidiagnosDisplayName1);
      addIfNotPresentOrElseLog(diagnoser, diagnos, pr, svar);
    }
    if (bidiagnosKod2 != null) {
      ...
      final var diagnos =
          Diagnos.create(
              bidiagnosKod2,
              bidiagnoskodverk2.toString(),
              bidiagnosBeskrivning2,
              bidiagnosDisplayName2);
      addIfNotPresentOrElseLog(diagnoser, diagnos, pr, svar);
    }
  }

  private void ensureKnownAndValidDiagnosisCode(Delsvar delsvar, CVType diagnos)
      throws PrefillWarningException {
    if (!moduleService.validateDiagnosisCodeFormat(diagnos.getCode())
        || !moduleService.validateDiagnosisCode(
            diagnos.getCode(), Diagnoskodverk.getEnumByCodeSystem(diagnos.getCodeSystem()))) {
      throw new PrefillWarningException(
          delsvar, WARNING_INVALID_CVTYPE_CODE_VALUE + " for diagnosis code " + diagnos.getCode());
    }
  }

  private String handleDiagnoseLookup(
      PrefillResult pr, String diagnosKod, String diagnosKodSystem, String svarId) {
  ...
  private void addDiagnoseDescriptionLookupMessage(
      PrefillResult pr, String delsvarsId, String diagnosKod, String diagnosBeskrivning) {
    pr.addMessage(
        PrefillEventType.INFO,
        delsvarsId,
        "Missing diagnoseDescription for code "
            + diagnosKod
            + " - defaulting with lookup value '"
            + diagnosBeskrivning
            + "'",
        "");
  }
