...
public abstract class AbstractQuestionBehovAvSjukskrivning {
  ...
  public static CertificateDataElement toCertificate(
      QuestionBehovAvSjukskrivningConfigProvider configProvider,
      String questionId,
      String parent,
      int index,
        ...
        .config(
            CertificateDataConfigCheckboxDateRangeList.builder()
                ...
                .previousDateRangeText(
                    hasRenewalRelation(configProvider.getRenewalRelation())
                        ? getPreviousDateRangeText(
                            configProvider.getSickLeaveText(), configProvider.getExpirationalDate())
                        : null)
                .build())
        .value(
  ...
  private static String getPreviousDateRangeText(String sickLeaveText, LocalDate expirationalDate) {
    ...
    return String.format(
        ...
        DateTimeFormatter.ofPattern("yyyy-MM-dd").format(expirationalDate), sickLeaveText);
  }
  ...
  private static List<CertificateDataValueDateRange> createSjukskrivningValue(
      List<SjukskrivningValue> sickLeaves) {
    if (sickLeaves == null) {
  ...
  public static List<Sjukskrivning> toInternal(Certificate certificate, String questionId) {
    ...
    return list.stream()
        .map(AbstractQuestionBehovAvSjukskrivning::getSjukskrivning)
        .collect(Collectors.toList());
  }

  private static Sjukskrivning getSjukskrivning(CertificateDataValueDateRange item) {
    return Sjukskrivning.create(
        SjukskrivningsGrad.fromId(item.getId()), getInternalLocalDateInterval(item));
  }
  ...
  public static class QuestionBehovAvSjukskrivningConfigProvider {
    ...
    public QuestionBehovAvSjukskrivningConfigProvider(
        RelationKod renewalRelation,
        LocalDate expirationalDate,
        String sickLeaveText,
        List<SjukskrivningValue> values) {
