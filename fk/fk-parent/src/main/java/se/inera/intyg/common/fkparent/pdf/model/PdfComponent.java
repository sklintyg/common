 ...
 *
 * <p>The PdfComponent framework was developed to make it easier to programatically build an
 * (almost) identical copy of a sample FK (xfa) pdf.
 *
 * <p>The resulting structure is a object-graph where all childrens positions are relative to it's
 * parent.
 *
 * <ul>
 *   Good to know:
 *   <li>All measurements (offsets, widths, heights etc) in a components is expressed in millimeters
 *       to make it easier to define the positioning based on measurements of a physically printed
 *       FK pdf.
 *   <li>iText itself expects all values to be expressed in points, so conversion is needed when
 *       actually rendering a component e.g on a PdfContentByte.
 *   <li>To avoid having extensive constructor arguments, all but the core properties of a component
 *       are set using a builder pattern.
 * </ul>
 *
 * @param <T> The subtype
 */
public abstract class PdfComponent<T extends PdfComponent> {
   ...
   *
   * <p>Combinations of Rectangle.XXX constants are supported, such as Rectangle.TOP +
   * Rectangle.LEFT. Most common is to use Rectangle.BOX
   *
  ...
  /**
   * Render a PdfComponent. The upper left corner coordinates as expressed in mm. When actually
   * writing to the canvas, mm units must be converted to points. Also, the coordinate system of an
  ...
  /**
   * Draws a border (with the specified borderstyle and color) around the effective bounding box of
   * this component.
   *
  ...
  public Stream<PdfComponent<? extends PdfComponent>> flattened() {
    return Stream.concat(Stream.of(this), getChildren().stream().flatMap(PdfComponent::flattened));
  }
}
