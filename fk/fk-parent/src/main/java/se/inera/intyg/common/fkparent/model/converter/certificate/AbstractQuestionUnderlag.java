...
public abstract class AbstractQuestionUnderlag {
  ...
  public static CertificateDataElement toCertificate(
      ...
      UnderlagsTyp ignoredUnderlagsTyp,
      String questionId,
      String parentId,
      int index,
                ...
                .list(
                    List.of(
                        MedicalInvestigation.builder()
                            ...
                            .typeOptions(getTypeOptions(ignoredUnderlagsTyp))
  ...
  private static CertificateDataValueMedicalInvestigation getMedicalInvestigationValue(
      List<Underlag> underlag, int id) {
    return CertificateDataValueMedicalInvestigation.builder()
        .date(
            CertificateDataValueDate.builder()
                .id(UNDERLAG_SVAR_JSON_ID_4 + "[" + id + "].datum")
                .date(id < underlag.size() ? toLocalDate(underlag.get(id).getDatum()) : null)
                .build())
        .investigationType(
            CertificateDataValueCode.builder()
                .id(UNDERLAG_SVAR_JSON_ID_4 + "[" + id + "].typ")
                .code(
                    id < underlag.size() && underlag.get(id).getTyp() != null
                        ? underlag.get(id).getTyp().getId()
                        : null)
                .build())
        .informationSource(
            CertificateDataValueText.builder()
                .id(UNDERLAG_SVAR_JSON_ID_4 + "[" + id + "].hamtasFran")
                .text(id < underlag.size() ? underlag.get(id).getHamtasFran() : null)
                .build())
        .build();
  }
  ...
  public static List<Underlag> toInternal(Certificate certificate, String questionId) {
    ...
    final var value =
        (CertificateDataValueMedicalInvestigationList)
            certificate.getData().get(questionId).getValue();
    final var underlagList =
        value.getList().stream()
            .map(
                underlag ->
                    Underlag.create(
                        underlag.getInvestigationType().getCode() != null
                                && !underlag.getInvestigationType().getCode().isEmpty()
                            ? UnderlagsTyp.fromId(underlag.getInvestigationType().getCode())
                            : null,
                        underlag.getDate().getDate() != null
                            ? new InternalDate(underlag.getDate().getDate())
                            : null,
  ...
  private static boolean hasValue(Underlag underlag) {
    return underlag.getHamtasFran() != null
        || underlag.getDatum() != null
