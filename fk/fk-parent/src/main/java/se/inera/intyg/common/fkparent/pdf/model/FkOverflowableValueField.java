...
/**
 * Field component that can handle overflowing texts. Text that could not be written within the
 * components dimensions is truncated and the remainder is kept for later use (rendered on dynamic
 * pages).
 *
 * <p>Created by marced on 2016-10-20.
 */
public class FkOverflowableValueField extends PdfComponent<FkOverflowableValueField> {
  ...
  // Overflowing valuefields should hold the label even if it is displayed outside of this component
  // as a fkLabel.
  // If overflow occurs, we need to present the label when outputting the remaining part on the
  ...
  public void render(Document document, PdfWriter writer, float x, float y)
    ...
    // Fo some fields, we may wich to keep newline characters, such as an "Övrigt" field that has
    // contributions from
    ...
    if (showLabelOnTop) {
      ...
      ColumnText.showTextAligned(
          canvas, Element.ALIGN_LEFT, new Phrase(label, topLabelFont), labelX, labelY, 0);
    ...
    if (charsToWrite < textValue.length()) {
      ...
      if (charsToWrite > 0) {
        ...
        writeText(
            canvas,
            targetRect,
            textValue.substring(0, charsToWrite),
            SEE_APPENDIX_PAGE_TEXT,
            false);
  ...
  private int findFittingLength(
      PdfContentByte canvas, Rectangle boundingRect, String text, String overflowInfoText)
      throws DocumentException {

    int length = text.length();
    if (!ColumnText.hasMoreText(
        writeText(canvas, boundingRect, text.substring(0, length), overflowInfoText, true))) {
      return length;
    }
  ...
  private int reduceTextLength(
      int length,
      int step,
      PdfContentByte canvas,
      Rectangle boundingRect,
      String text,
      String overflowInfoText)
      throws DocumentException {

    while (length - step >= 0
        && ColumnText.hasMoreText(
            writeText(
                canvas, boundingRect, text.substring(0, length - step), overflowInfoText, true))) {
      length -= step;
    }
