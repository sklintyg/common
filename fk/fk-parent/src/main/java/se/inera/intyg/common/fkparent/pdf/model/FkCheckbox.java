...
public class FkCheckbox extends PdfComponent<FkCheckbox> {
  ...
  private static final Rectangle CHECKBOX_DIMENSIONS_IN_POINTS =
      new RectangleReadOnly(
          Utilities.millimetersToPoints(4.5f), Utilities.millimetersToPoints(4.5f));
  ...
  public void render(Document document, PdfWriter writer, float x, float y)
    ...
    table.setTotalWidth(
        new float[] {
          Utilities.millimetersToPoints(cellWidth), Utilities.millimetersToPoints(width - cellWidth)
        });
  ...
  private Image createCheckbox(PdfContentByte canvas, boolean isChecked)
      throws BadElementException {

    final PdfTemplate template =
        canvas.createTemplate(
            CHECKBOX_DIMENSIONS_IN_POINTS.getWidth(), CHECKBOX_DIMENSIONS_IN_POINTS.getHeight());
    Rectangle rect =
        new Rectangle(
            0,
            0,
            CHECKBOX_DIMENSIONS_IN_POINTS.getWidth(),
    ...
    if (isChecked) {
      ...
      template.moveTo(
          rect.getLeft() + CHECKBOX_INNER_PADDING, rect.getTop() - CHECKBOX_INNER_PADDING);
      template.lineTo(
          rect.getRight() - CHECKBOX_INNER_PADDING, rect.getBottom() + CHECKBOX_INNER_PADDING);
      template.moveTo(
          rect.getRight() - CHECKBOX_INNER_PADDING, rect.getTop() - CHECKBOX_INNER_PADDING);
      template.lineTo(
          rect.getLeft() + CHECKBOX_INNER_PADDING, rect.getBottom() + CHECKBOX_INNER_PADDING);
    }
