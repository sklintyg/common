...
/**
 * Decorates the "dynamic" overflow output pages with FK logo, borders etc common for all non-static
 * pages in a FK SIT type PDF. Given that all SIT-intyg types will follow the same rules for
 * handling overflowing fields, this event handler could be reused for all those types.
 */
...
public class FkDynamicPageDecoratorEventHandler extends PdfPageEventHelper {
  ...
  public FkDynamicPageDecoratorEventHandler(
      int startFromPage, float[] pageMargins, String intygsNamnRow1, String intygsNamnRow2) {
  ...
  private void drawIntygsNamn(PdfWriter writer, Document document) {
    ...
    ColumnText.showTextAligned(
        ...
        Element.ALIGN_LEFT,
        new Phrase(intygsNamnRow1, PdfConstants.FONT_FRAGERUBRIK),
        INTYG_NAME_X,
        document.getPageSize().getTop() - INTYG_NAME_ROW1_Y_OFFSET,
        0);
    ColumnText.showTextAligned(
        writer.getDirectContent(),
        Element.ALIGN_LEFT,
        new Phrase(intygsNamnRow2, PdfConstants.FONT_BOLD_9),
        INTYG_NAME_X,
        document.getPageSize().getTop() - INTYG_NAME_ROW2_Y_OFFSET,
        0);
  }
  ...
  private void drawBorder(PdfWriter writer, Document document) {
    ...
    Rectangle rect =
        new Rectangle(
            p.getLeft(pageMargins[0]),
            p.getBottom(pageMargins[3]),
            p.getRight(pageMargins[1]),
