 ...
 */
package se.inera.intyg.common.luae_fs.v1.utils;
...
public final class ScenarioFinder {
  ...
  private static final class FileBasedScenario implements Scenario {
      ...
      try {
        JAXBElement<RegisterCertificateType> rct =
            XmlMarshallerHelper.unmarshal(
                model(TRANSPORT_MODEL_PATH + getName() + TRANSPORT_MODEL_EXT));
        return rct.getValue();
      } catch (IOException e) {
    ...
    /** {@inheritDoc} */
    @Override
    public LuaefsUtlatandeV1 asInternalModel() throws ScenarioNotFoundException {
      try {
        return new CustomObjectMapper()
            .readValue(
                model(INTERNAL_MODEL_PATH + getName() + INTERNAL_MODEL_EXT),
