...
public class InternalDraftValidatorImpl implements InternalDraftValidator<LuaefsUtlatandeV1> {
      ...
      if (dateIsValid && checkIfNotFuture(utlatande.getKannedomOmPatient().asLocalDate())) {
        if (utlatande.getUndersokningAvPatienten() != null
          ...
          ValidatorUtil.addValidationErrorWithQuestionId(
              ...
              "luae_fs.validation.grund-for-mu.kannedom.after",
  ...
  void validateUnderlag(LuaefsUtlatandeV1 utlatande, List<ValidationMessage> validationMessages) {
    if (utlatande.getUnderlagFinns() == null) {
      ...
      // If the flag is null, we cant determine whether underlag should be a list or not, so we
      // can't do any
      // further validation..
      return;
    } else if (utlatande.getUnderlagFinns()
        && (utlatande.getUnderlag() == null || utlatande.getUnderlag().isEmpty())) {
      ValidatorUtil.addValidationErrorWithQuestionId(
          validationMessages,
          CATEGORY_GRUNDFORMU,
  ...
  void validateFunktionsnedsattning(
      LuaefsUtlatandeV1 utlatande, List<ValidationMessage> validationMessages) {
  ...
  void validateBlanksForOptionalFields(
      LuaefsUtlatandeV1 utlatande, List<ValidationMessage> validationMessages) {
