 ...
 */
package se.inera.intyg.common.luse.v1.testability;
...
import static se.inera.intyg.common.fkparent.testability.FkDataValueUtil.getDataValueMinimalDiagnosisListFk;
import static se.inera.intyg.common.fkparent.testability.FkDataValueUtil.getValueMedicalInvestigation;
...
public class LuseTestabilityCertificateTestdataProvider
  ...
  public Map<String, CertificateDataValue> getMinimumValues() {
    ...
    final var diagnos =
        getDataValueMinimalDiagnosisListFk(
            DIAGNOSES_LIST_ITEM_1_ID, Diagnos.create("A00", "ICD_10_SE", "Kolera", "Kolera"));
    ...
    final var nyBedomningDiagnosgrund =
        getDataValueBoolean(DIAGNOSGRUND_NY_BEDOMNING_SVAR_JSON_ID, false);
    ...
    final var funktionsnedsattningIntellektuell =
        getDataValueText(
            FUNKTIONSNEDSATTNING_INTELLEKTUELL_SVAR_JSON_ID, "funktionsnedsattning intellektuell");
    values.put(FUNKTIONSNEDSATTNING_INTELLEKTUELL_SVAR_ID, funktionsnedsattningIntellektuell);

    final var aktivitetsbegransning =
        getDataValueText(AKTIVITETSBEGRANSNING_SVAR_JSON_ID, "aktivitetsbegransning");
    values.put(AKTIVITETSBEGRANSNING_DELSVAR_ID, aktivitetsbegransning);

    final var medicinskaForutsattningarForArbete =
        getDataValueText(
  ...
  public Map<String, CertificateDataValue> getMaximumValues() {
    ...
    final var annat =
        getDataValueText(GRUNDFORMEDICINSKTUNDERLAG_BESKRIVNING_DELSVAR_JSON_ID_1, "annat");
    values.put(GRUNDFORMEDICINSKTUNDERLAG_ANNANBESKRIVNING_DELSVAR_ID_1, annat);

    final var motivering =
        getDataValueText(
            se.inera.intyg.common.luse.v1.model.converter.RespConstants
                .MOTIVERING_TILL_INTE_BASERAT_PA_UNDERLAG_ID_1,
            "motivering");
    values.put(MOTIVERING_TILL_INTE_BASERAT_PA_UNDERLAG_DELSVAR_ID_1, motivering);

    final var kannedomOmPatienten =
        getDataValueDate(RespConstants.KANNEDOM_SVAR_JSON_ID_2, LocalDate.now());
    ...
    final var underlag =
        CertificateDataValueMedicalInvestigationList.builder()
            .list(
                List.of(
                    getValueMedicalInvestigation(
                        UNDERLAG_SVAR_JSON_ID_4, 0, "NEUROPSYKIATRISKT", "neuropsykiatriskt"),
                    getValueMedicalInvestigation(
                        UNDERLAG_SVAR_JSON_ID_4, 1, "ARBETSTERAPEUT", "arbetsterapeut"),
                    getValueMedicalInvestigation(UNDERLAG_SVAR_JSON_ID_4, 2, "LOGOPED", "logoped")))
            .build();
    ...
    final var nyBedomningDiagnosgrund =
        getDataValueBoolean(DIAGNOSGRUND_NY_BEDOMNING_SVAR_JSON_ID, true);
    values.put(DIAGNOSGRUND_NY_BEDOMNING_SVAR_ID, nyBedomningDiagnosgrund);

    final var vilkaDiagnoser =
        getDataValueText(
    ...
    final var funktionsnedsattningKommunikation =
        getDataValueText(
            FUNKTIONSNEDSATTNING_KOMMUNIKATION_SVAR_JSON_ID, "funktionsnedsattning kommunikation");
    values.put(FUNKTIONSNEDSATTNING_KOMMUNIKATION_SVAR_ID, funktionsnedsattningKommunikation);

    final var funktionsnedsattningKoncentration =
        getDataValueText(
            FUNKTIONSNEDSATTNING_KONCENTRATION_SVAR_JSON_ID, "funktionsnedsattning koncentration");
    values.put(FUNKTIONSNEDSATTNING_KONCENTRATION_SVAR_ID, funktionsnedsattningKoncentration);

    final var funktionsnedsattningPsykisk =
        getDataValueText(FUNKTIONSNEDSATTNING_PSYKISK_SVAR_JSON_ID, "funktionsnedsattning psykisk");
    values.put(FUNKTIONSNEDSATTNING_PSYKISK_SVAR_ID, funktionsnedsattningPsykisk);

    final var funktionsnedsattningSynHorselTal =
        getDataValueText(
            FUNKTIONSNEDSATTNING_SYNHORSELTAL_SVAR_JSON_ID, "funktionsnedsattning synHorselTal");
    values.put(FUNKTIONSNEDSATTNING_SYNHORSELTAL_SVAR_ID, funktionsnedsattningSynHorselTal);

    final var funktionsnedsattningbalansKoordination =
        getDataValueText(
            FUNKTIONSNEDSATTNING_BALANSKOORDINATION_SVAR_JSON_ID,
            "funktionsnedsattning synHorselTal");
    values.put(
        FUNKTIONSNEDSATTNING_BALANSKOORDINATION_SVAR_ID, funktionsnedsattningbalansKoordination);

    final var funktionsnedsattningAnnan =
        getDataValueText(
            FUNKTIONSNEDSATTNING_ANNAN_SVAR_JSON_ID, "funktionsnedsattning synHorselTal");
    ...
    final var avslutadBehandling =
        getDataValueText(AVSLUTADBEHANDLING_SVAR_JSON_ID, "avslutad behandling");
    values.put(AVSLUTADBEHANDLING_DELSVAR_ID, avslutadBehandling);

    final var pagaendeBehandling =
        getDataValueText(PAGAENDEBEHANDLING_SVAR_JSON_ID, "pågående behandling");
    values.put(PAGAENDEBEHANDLING_DELSVAR_ID, pagaendeBehandling);

    final var planeradBehandling =
        getDataValueText(PLANERADBEHANDLING_SVAR_JSON_ID, "planerad behandling");
    ...
    final var trotsBegransningar =
        getDataValueText(FORMAGATROTSBEGRANSNING_SVAR_JSON_ID, "förmåga trots begränsningar");
    ...
    final var anledningTillKontakt =
        getDataValueText(ANLEDNING_TILL_KONTAKT_DELSVAR_JSON_ID, "anledning till kontakt");
