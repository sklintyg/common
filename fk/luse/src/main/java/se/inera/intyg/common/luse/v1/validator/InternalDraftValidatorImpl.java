...
public class InternalDraftValidatorImpl implements InternalDraftValidator<LuseUtlatandeV1> {
  ...
  private static final String CATEGORY_MEDICINSKAFORUTSATTNINGARFORARBETE =
  ...
  @Autowired ValidatorUtilFK validatorUtilFK;
  ...
  private void validateGrundForMU(
      LuseUtlatandeV1 utlatande, List<ValidationMessage> validationMessages) {
    ...
    if (utlatande.getAnnatGrundForMU() != null) {
      ValidatorUtilFK.validateGrundForMuDate(
          utlatande.getAnnatGrundForMU(), validationMessages, ValidatorUtilFK.GrundForMu.ANNAT);
    }
    ...
    if (utlatande.getKannedomOmPatient() == null) {
      ValidatorUtil.addValidationErrorWithQuestionId(
          validationMessages,
          CATEGORY_GRUNDFORMU,
          KANNEDOM_SVAR_JSON_ID_2,
          ValidationMessageType.EMPTY,
          KANNEDOM_SVAR_ID_2);
    } else {
      boolean dateIsValid =
          ValidatorUtil.validateDateAndCheckIfFuture(
              utlatande.getKannedomOmPatient(),
              validationMessages,
              CATEGORY_GRUNDFORMU,
              KANNEDOM_SVAR_JSON_ID_2,
      ...
      if (dateIsValid && checkIfNotFuture(utlatande.getKannedomOmPatient().asLocalDate())) {
        if (utlatande.getUndersokningAvPatienten() != null
            && utlatande.getUndersokningAvPatienten().isValidDate()
            && utlatande
                .getKannedomOmPatient()
                .asLocalDate()
                .isAfter(utlatande.getUndersokningAvPatienten().asLocalDate())) {
          ValidatorUtil.addValidationErrorWithQuestionId(
              validationMessages,
              CATEGORY_GRUNDFORMU,
              KANNEDOM_SVAR_JSON_ID_2,
              ValidationMessageType.OTHER,
              "luse.validation.grund-for-mu.kannedom.after",
              "KV_FKMU_0001.UNDERSOKNING.RBK",
              KANNEDOM_SVAR_ID_2);
        }
        if (utlatande.getAnhorigsBeskrivningAvPatienten() != null
  ...
  void validateUnderlag(LuseUtlatandeV1 utlatande, List<ValidationMessage> validationMessages) {
      ...
      if (underlag.getTyp() == null) {
        ValidatorUtil.addValidationErrorWithQuestionId(
            ...
            UNDERLAG_SVAR_ID_4);
              ...
              .getTyp()
              .getId()
              .equals(Underlag.UnderlagsTyp.NEUROPSYKIATRISKT_UTLATANDE.getId())
          && !underlag
              .getTyp()
              .getId()
              .equals(Underlag.UnderlagsTyp.UNDERLAG_FRAN_HABILITERINGEN.getId())
          && !underlag
              .getTyp()
              .getId()
              .equals(Underlag.UnderlagsTyp.UNDERLAG_FRAN_ARBETSTERAPEUT.getId())
          && !underlag
              .getTyp()
              .getId()
              .equals(Underlag.UnderlagsTyp.UNDERLAG_FRAN_FYSIOTERAPEUT.getId())
          ...
          && !underlag
              .getTyp()
              .getId()
              .equals(Underlag.UnderlagsTyp.UNDERLAG_FRANFORETAGSHALSOVARD.getId())
          && !underlag
              .getTyp()
              .getId()
              .equals(Underlag.UnderlagsTyp.UTREDNING_AV_ANNAN_SPECIALISTKLINIK.getId())
          && !underlag
              .getTyp()
              .getId()
              .equals(Underlag.UnderlagsTyp.UTREDNING_FRAN_VARDINRATTNING_UTOMLANDS.getId())
    ...
    if (utlatande.getUnderlag().size() > 1
        && !validateFirstUnderlagIsPresent(utlatande.getUnderlag())) {
      ValidatorUtil.addValidationErrorWithQuestionId(
          validationMessages,
          CATEGORY_GRUNDFORMU,
  ...
  private void validateSjukdomsforlopp(
    ...
    if (Strings.nullToEmpty(utlatande.getSjukdomsforlopp()).trim().isEmpty()) {
      ValidatorUtil.addValidationErrorWithQuestionId(
          validationMessages,
          CATEGORY_SJUKDOMSFORLOPP,
          SJUKDOMSFORLOPP_SVAR_JSON_ID,
          ValidationMessageType.EMPTY,
          SJUKDOMSFORLOPP_SVAR_ID);
    }
  }
  ...
  private void validateAktivitetsbegransning(
    ...
    if (Strings.nullToEmpty(utlatande.getAktivitetsbegransning()).trim().isEmpty()) {
      ValidatorUtil.addValidationErrorWithQuestionId(
          validationMessages,
          CATEGORY_AKTIVITETSBEGRANSNING,
          AKTIVITETSBEGRANSNING_SVAR_JSON_ID,
          ValidationMessageType.EMPTY,
          AKTIVITETSBEGRANSNING_DELSVAR_ID);
    }
  }

  private void validateMedicinskaForutsattningarForArbete(
      LuseUtlatandeV1 utlatande, List<ValidationMessage> validationMessages) {
    if (Strings.nullToEmpty(utlatande.getMedicinskaForutsattningarForArbete()).trim().isEmpty()) {
      ValidatorUtil.addValidationErrorWithQuestionId(
          validationMessages,
          CATEGORY_MEDICINSKAFORUTSATTNINGARFORARBETE,
  ...
  private void validateFunktionsnedsattning(
    ...
    if (Strings.nullToEmpty(utlatande.getFunktionsnedsattningAnnan()).trim().isEmpty()
        && Strings.nullToEmpty(utlatande.getFunktionsnedsattningBalansKoordination())
            .trim()
            .isEmpty()
      ...
      ValidatorUtil.addValidationErrorWithQuestionId(
          ...
          ValidationMessageType.EMPTY,
          "common.validation.funktionsnedsattning.empty",
          FUNKTIONSNEDSATTNING_CATEGORY_ID);
    }
  }

  private void validateDiagnosgrund(
      LuseUtlatandeV1 utlatande, List<ValidationMessage> validationMessages) {
    ...
    if (Strings.nullToEmpty(utlatande.getDiagnosgrund()).trim().isEmpty()) {
      ValidatorUtil.addValidationErrorWithQuestionId(
          validationMessages,
          CATEGORY_DIAGNOS,
          DIAGNOSGRUND_SVAR_JSON_ID,
          ValidationMessageType.EMPTY,
          DIAGNOSGRUND_SVAR_ID);
    }
    ...
    if (utlatande.getNyBedomningDiagnosgrund() == null) {
      ValidatorUtil.addValidationErrorWithQuestionId(
          validationMessages,
          CATEGORY_DIAGNOS,
          DIAGNOSGRUND_NY_BEDOMNING_SVAR_JSON_ID,
          ValidationMessageType.EMPTY,
          DIAGNOSGRUND_NY_BEDOMNING_SVAR_ID);
    }

    // R13
    if (utlatande.getNyBedomningDiagnosgrund() != null
        && utlatande.getNyBedomningDiagnosgrund()
  ...
  private void validateKontaktMedFk(
      LuseUtlatandeV1 utlatande, List<ValidationMessage> validationMessages) {
    // R11
    if ((utlatande.getKontaktMedFk() == null || !utlatande.getKontaktMedFk())
      ...
      ValidatorUtil.addValidationErrorWithQuestionId(
          validationMessages,
          CATEGORY_KONTAKT,
          KONTAKT_ONSKAS_SVAR_JSON_ID,
          ValidationMessageType.INCORRECT_COMBINATION,
          "luse.validation.kontakt.incorrect_combination",
          KONTAKT_ONSKAS_SVAR_ID);
    }
  }
