...
public class QuestionFunktionsnedsattningKoncentration {

    private static final short TEXT_LIMIT = (short) 4000;

    public static CertificateDataElement toCertificate(String funktionsnedsattningKoncentration, int index,
        CertificateTextProvider textProvider) {
        return CertificateDataElement.builder()
            .id(FUNKTIONSNEDSATTNING_KONCENTRATION_DELSVAR_ID_10)
            .parent(FUNKTIONSNEDSATTNING_CATEGORY_ID)
            .index(index)
            .config(
                CertificateDataConfigTextArea.builder()
                    .text(textProvider.get(FUNKTIONSNEDSATTNING_KONCENTRATION_TEXT_ID))
                    .description(textProvider.get(FUNKTIONSNEDSATTNING_KONCENTRATION_DESCRIPTION_ID))
                    .accordion(
                        Accordion.builder()
                            .openText(FUNKTIONSNEDSATTNING_ACCORDION_OPEN_TEXT)
                            .closeText(FUNKTIONSNEDSATTNING_ACCORDION_CLOSE_TEXT)
                            .header(textProvider.get(FUNKTIONSNEDSATTNING_KONCENTRATION_DELSVAR_TEXT_ID))
                            .build()
                    )
                    .id(FUNKTIONSNEDSATTNING_KONCENTRATION_SVAR_JSON_ID_10)
                    .build()
            )
            .value(
                CertificateDataTextValue.builder()
                    .id(FUNKTIONSNEDSATTNING_KONCENTRATION_SVAR_JSON_ID_10)
                    .text(funktionsnedsattningKoncentration)
                    .build()
            )
            .validation(
    ...
    public static String toInternal(Certificate certificate) {
        return ValueToolkit.textValue(certificate.getData(), FUNKTIONSNEDSATTNING_KONCENTRATION_DELSVAR_ID_10,
