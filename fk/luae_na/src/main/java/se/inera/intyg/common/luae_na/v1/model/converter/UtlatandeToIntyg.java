...
import static se.inera.intyg.common.luae_na.v1.model.converter.RespConstants.DIAGNOSGRUND_DELSVAR_ID_7;
...
import static se.inera.intyg.common.luae_na.v1.model.converter.RespConstants.GRUNDFORMEDICINSKTUNDERLAG_CODE_SYSTEM;
import static se.inera.intyg.common.luae_na.v1.model.converter.RespConstants.GRUNDFORMEDICINSKTUNDERLAG_DATUM_DELSVAR_ID_1;
...
import static se.inera.intyg.common.luae_na.v1.model.converter.RespConstants.UNDERLAG_CODE_SYSTEM;
import static se.inera.intyg.common.luae_na.v1.model.converter.RespConstants.UNDERLAG_DATUM_DELSVAR_ID_4;
import static se.inera.intyg.common.luae_na.v1.model.converter.RespConstants.UNDERLAG_HAMTAS_FRAN_DELSVAR_ID_4;
...
public final class UtlatandeToIntyg {
  ...
  public static Intyg convert(
      LuaenaUtlatandeV1 utlatande, WebcertModuleService webcertModuleService) {
  ...
  private static List<Svar> getSvar(
      LuaenaUtlatandeV1 source, WebcertModuleService webcertModuleService) {
    ...
    if (source.getAnhorigsBeskrivningAvPatienten() != null
        && source.getAnhorigsBeskrivningAvPatienten().isValidDate()) {
      svars.add(
          aSvar(GRUNDFORMEDICINSKTUNDERLAG_SVAR_ID_1, grundForMUInstans++)
              .withDelsvar(
                  GRUNDFORMEDICINSKTUNDERLAG_TYP_DELSVAR_ID_1,
                  aCV(
                      GRUNDFORMEDICINSKTUNDERLAG_CODE_SYSTEM,
                      RespConstants.ReferensTyp.ANHORIGSBESKRIVNING.transportId,
                      RespConstants.ReferensTyp.ANHORIGSBESKRIVNING.label))
              .withDelsvar(
                  GRUNDFORMEDICINSKTUNDERLAG_DATUM_DELSVAR_ID_1,
                  InternalConverterUtil.getInternalDateContent(
                      source.getAnhorigsBeskrivningAvPatienten()))
              .build());
    }
    ...
    if (source.getUnderlagFinns() != null) {
      svars.add(
          aSvar(UNDERLAGFINNS_SVAR_ID_3)
              .withDelsvar(UNDERLAGFINNS_DELSVAR_ID_3, source.getUnderlagFinns().toString())
              .build());
    }
      ...
      if (underlag.getDatum() != null && underlag.getDatum().isValidDate()) {
        svars.add(
            aSvar(UNDERLAG_SVAR_ID_4, underlagInstans++)
                .withDelsvar(
                    UNDERLAG_TYP_DELSVAR_ID_4,
                    underlag.getTyp() == null
                        ? null
                        : aCV(
                            UNDERLAG_CODE_SYSTEM,
                            underlag.getTyp().getId(),
                            underlag.getTyp().getLabel()))
                .withDelsvar(
                    UNDERLAG_DATUM_DELSVAR_ID_4,
                    InternalConverterUtil.getInternalDateContent(underlag.getDatum()))
                .withDelsvar(UNDERLAG_HAMTAS_FRAN_DELSVAR_ID_4, underlag.getHamtasFran())
                .build());
      }
    }
    ...
    addIfNotBlank(
        svars,
        SJUKDOMSFORLOPP_SVAR_ID_5,
        SJUKDOMSFORLOPP_DELSVAR_ID_5,
      ...
      if (source.getNyBedomningDiagnosgrund()) {
        svars.add(
            aSvar(NYDIAGNOS_SVAR_ID_45)
                .withDelsvar(
                    DIAGNOSGRUND_NYBEDOMNING_DELSVAR_ID_45,
                    source.getNyBedomningDiagnosgrund().toString())
                .withDelsvar(
                    DIAGNOS_FOR_NY_BEDOMNING_DELSVAR_ID_45, source.getDiagnosForNyBedomning())
                .build());
      } else {
        svars.add(
            aSvar(NYDIAGNOS_SVAR_ID_45)
                .withDelsvar(DIAGNOSGRUND_NYBEDOMNING_DELSVAR_ID_45, Boolean.FALSE.toString())
                .build());
      }
    }
    ...
    addIfNotBlank(
        ...
        FUNKTIONSNEDSATTNING_KOMMUNIKATION_DELSVAR_ID_9,
        source.getFunktionsnedsattningKommunikation());
    addIfNotBlank(
        svars,
        FUNKTIONSNEDSATTNING_KONCENTRATION_SVAR_ID_10,
        FUNKTIONSNEDSATTNING_KONCENTRATION_DELSVAR_ID_10,
        source.getFunktionsnedsattningKoncentration());
    addIfNotBlank(
        svars,
        FUNKTIONSNEDSATTNING_PSYKISK_SVAR_ID_11,
        FUNKTIONSNEDSATTNING_PSYKISK_DELSVAR_ID_11,
    ...
    addIfNotBlank(
        ...
        AKTIVITETSBEGRANSNING_DELSVAR_ID_17,
        source.getAktivitetsbegransning());
    addIfNotBlank(
        svars,
        AVSLUTADBEHANDLING_SVAR_ID_18,
        AVSLUTADBEHANDLING_DELSVAR_ID_18,
        source.getAvslutadBehandling());
    addIfNotBlank(
        svars,
        PAGAENDEBEHANDLING_SVAR_ID_19,
        PAGAENDEBEHANDLING_DELSVAR_ID_19,
        source.getPagaendeBehandling());
    addIfNotBlank(
        svars,
        PLANERADBEHANDLING_SVAR_ID_20,
        PLANERADBEHANDLING_DELSVAR_ID_20,
        source.getPlaneradBehandling());
    addIfNotBlank(
        svars, SUBSTANSINTAG_SVAR_ID_21, SUBSTANSINTAG_DELSVAR_ID_21, source.getSubstansintag());
    addIfNotBlank(
        svars,
        MEDICINSKAFORUTSATTNINGARFORARBETE_SVAR_ID_22,
    ...
    for (Tillaggsfraga tillaggsfraga : source.getTillaggsfragor()) {
      addIfNotBlank(
          svars, tillaggsfraga.getId(), tillaggsfraga.getId() + ".1", tillaggsfraga.getSvar());
    }
  ...
  private static String buildOvrigaUpplysningar(LuaenaUtlatandeV1 source) {
    ...
    if (!Strings.nullToEmpty(source.getMotiveringTillInteBaseratPaUndersokning())
        .trim()
        .isEmpty()) {
      motiveringTillInteBaseratPaUndersokning =
          "Motivering till varför utlåtandet inte baseras på undersökning av patienten: "
