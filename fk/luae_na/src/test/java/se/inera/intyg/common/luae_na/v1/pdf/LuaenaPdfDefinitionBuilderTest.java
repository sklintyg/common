 ...
 * for manual visual inspection of the resulting pdf files, as we don't have any way of
 * programmatically assert the content of the pdf.
 *
 * @author marced
 */
@ExtendWith(MockitoExtension.class)
class LuaenaPdfDefinitionBuilderTest {
  ...
  @Mock FkAbstractModuleEntryPoint entryPoint;
  ...
  void initTexts() throws IOException {
    ...
    IntygTextsLuaenaRepositoryTestHelper intygsTextRepositoryHelper =
        new IntygTextsLuaenaRepositoryTestHelper();
    ...
    intygList.add(
        objectMapper.readValue(
            new ClassPathResource("v1/PdfGeneratorTest/fullt_utlatande.json").getFile(),
            LuaenaUtlatandeV1.class));
    intygList.add(
        objectMapper.readValue(
            new ClassPathResource("v1/PdfGeneratorTest/overflow_utlatande.json").getFile(),
            LuaenaUtlatandeV1.class));
  }
  ...
  private void generate(
    ...
    byte[] generatorResult =
        PdfGenerator.generatePdf(
            luaenaPdfDefinitionBuilder.buildPdfDefinition(
                utlatandeV1,
                statuses,
                origin,
                intygTextsService.getIntygTextsPojo("luae_na", textVersion),
                utkastStatus,
                "printedByText"),
            TITLE);
  ...
  private void writePdfToFile(
      byte[] pdf, ApplicationOrigin origin, String scenarioName, String namingPrefix)
      throws IOException {
    String dir = "build/tmp";
    File file =
        new File(
            String.format(
                "%s/%s-%s-%s-%s", dir, origin.name(), scenarioName, namingPrefix, "luae_na.pdf"));
