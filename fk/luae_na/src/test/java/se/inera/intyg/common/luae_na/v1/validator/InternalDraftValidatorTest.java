...
public class InternalDraftValidatorTest {
  ...
  @InjectMocks ValidatorUtilFK validatorUtil;
  ...
  public void setUp() throws Exception {
    builderTemplate =
        LuaenaUtlatandeV1.builder()
            ...
            .setDiagnoser(
                List.of(Diagnos.create("S47", "ICD_10_SE", "Beskrivning", "diagnosDisplayName")))
  ...
  public void validateGrundForMUMissing() {
    ...
    assertEquals(
        GRUNDFORMEDICINSKTUNDERLAG_SVAR_ID_1, res.getValidationErrors().get(0).getQuestionId());
  }
  ...
  public void validateGrundForMUKannedomOmPatientAfterAnhorigsBeskrivning() {
    ...
    assertEquals(ValidationMessageType.OTHER, res.getValidationErrors().get(0).getType());
    assertEquals(
        RespConstants.KANNEDOM_SVAR_ID_2, res.getValidationErrors().get(0).getQuestionId());
  }
  ...
  public void validateGrundForMUAnnanGrundBeskrivningNotAnnanGrundDatum() {
    ...
    assertEquals(
        ...
        res.getValidationErrors().get(1).getQuestionId());
  }
  ...
  public void validateGrundForMUAnnanGrundBeskrivningMissing() {
    ...
    assertEquals(
        GRUNDFORMEDICINSKTUNDERLAG_ANNANBESKRIVNING_DELSVAR_ID_1,
        res.getValidationErrors().get(0).getQuestionId());
  }
  ...
  public void validateUnderlagOK() {
    LuaenaUtlatandeV1 utlatande =
        builderTemplate
            ...
            .setUnderlag(buildUnderlag("NEUROPSYKIATRISKT", "HABILITERING", "ARBETSTERAPEUT"))
            .build();
  ...
  public void validateUnderlagTooMany() {
    ...
    assertEquals(1, res.getValidationErrors().size());
    assertEquals(
        "luae_na.validation.underlag.too_many", res.getValidationErrors().get(0).getMessage());
    assertEquals(ValidationMessageType.OTHER, res.getValidationErrors().get(0).getType());
    assertEquals(UNDERLAG_SVAR_ID_4, res.getValidationErrors().get(0).getQuestionId());
  }
  ...
  public void validateUnderlagInformationMissing() {
    LuaenaUtlatandeV1 utlatande =
        builderTemplate
            ...
            .setUnderlag(
                List.of(
                    Underlag.create(Underlag.UnderlagsTyp.fromId("NEUROPSYKIATRISKT"), null, null)))
            .build();
    ...
    assertEquals(2, res.getValidationErrors().size());
    assertEquals(
        "luae_na.validation.underlag.date.missing", res.getValidationErrors().get(0).getMessage());
    assertEquals(ValidationMessageType.EMPTY, res.getValidationErrors().get(0).getType());
    assertEquals(UNDERLAG_SVAR_ID_4, res.getValidationErrors().get(0).getQuestionId());
    assertEquals(
  ...
  public void validateDiagnosMissing() {
    ...
    assertEquals(1, res.getValidationErrors().size());
    assertEquals(
        "common.validation.diagnos.missing", res.getValidationErrors().get(0).getMessage());
    assertEquals(ValidationMessageType.EMPTY, res.getValidationErrors().get(0).getType());
  }
  ...
  public void validateDiagnosDiagnosKodInvalid() {
    ...
    assertEquals(
        "common.validation.diagnos.length-3", res.getValidationErrors().get(0).getMessage());
    ...
    assertEquals(
        "common.validation.diagnos.psykisk.length-4",
        res.getValidationErrors().get(2).getMessage());
  ...
  public void validateAktivitetsbegransningMissing() {
    ...
    assertEquals(ValidationMessageType.EMPTY, res.getValidationErrors().get(0).getType());
    assertEquals(
        AKTIVITETSBEGRANSNING_DELSVAR_ID_17, res.getValidationErrors().get(0).getQuestionId());
  }
  ...
  public void validateFunktionsnedsattningMissing() {
    ...
    assertEquals(ValidationMessageType.EMPTY, res.getValidationErrors().get(0).getType());
    assertEquals(
        FUNKTIONSNEDSATTNING_CATEGORY_ID, res.getValidationErrors().get(0).getQuestionId());
  }
  ...
  public void validateNyBedomningDiagnosgrundBeskrivningMissing() {
    ...
    assertEquals(ValidationMessageType.EMPTY, res.getValidationErrors().get(0).getType());
    assertEquals(
        DIAGNOS_FOR_NY_BEDOMNING_DELSVAR_ID_45, res.getValidationErrors().get(0).getQuestionId());
  }
  ...
  private List<Underlag> buildUnderlag(String... underlagsTyper) {
    ...
    for (String typ : underlagsTyper) {
      underlag.add(
          Underlag.create(
              Underlag.UnderlagsTyp.fromId(typ),
