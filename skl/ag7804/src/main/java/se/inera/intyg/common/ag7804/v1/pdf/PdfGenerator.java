...
public class PdfGenerator {
  ...
  protected static final String MINIMAL_CERTIFICATE_FILE_PREFIX =
      "minimalt_" + CERTIFICATE_FILE_BASE_NAME;
  ...
  private static final String INFO_SIGNED_TEXT_COMPLETE =
      "Detta är en utskrift av ett elektroniskt intyg. Intyget har "
  ...
  public PdfResponse generatePdf(
    ...
    try {
      ...
      byte[] data =
          new UVRenderer().startRendering(printConfig, intygTexts, buildFilename(isPdfEmployer));
  ...
  private Map<String, String> buildModelPropReplacements(List<String> optionalFields) {
    ...
    if (optionalFields != null) {
      ...
      if (optionalFields.stream()
          .anyMatch(s -> s.equals(SKIP_SYMBOL + OPTIONAL_FIELD_FORMEDLA_DIAGNOSER))) {
  ...
  private String buildFilename(boolean pdfEmployer) {
    ...
    return (pdfEmployer ? MINIMAL_CERTIFICATE_FILE_PREFIX : CERTIFICATE_FILE_BASE_NAME)
