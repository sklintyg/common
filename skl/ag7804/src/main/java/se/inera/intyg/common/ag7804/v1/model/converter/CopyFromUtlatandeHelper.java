...
public final class CopyFromUtlatandeHelper {
  ...
  public static void copyFrom(LisjpUtlatandeV1 source, Ag7804UtlatandeV1.Builder target) {
    ...
    target
        .setAvstangningSmittskydd(source.getAvstangningSmittskydd())
   ...
   */
  private static List<se.inera.intyg.common.ag7804.model.internal.ArbetslivsinriktadeAtgarder>
      convertAtagarder(ImmutableList<ArbetslivsinriktadeAtgarder> arbetslivsinriktadeAtgarder) {
    return arbetslivsinriktadeAtgarder.stream()
        .map(
            source ->
                Arrays.stream(
                        se.inera.intyg.common.ag7804.model.internal.ArbetslivsinriktadeAtgarder
                            .ArbetslivsinriktadeAtgarderVal.values())
                    .filter(
                        arbetslivsinriktadeAtgarderVal ->
                            arbetslivsinriktadeAtgarderVal.getId().equals(source.getTyp().getId()))
                    .findFirst()
                    .orElse(null))
  ...
  private static se.inera.intyg.common.ag7804.model.internal.Prognos convertPrognos(
      Prognos prognos) {
    se.inera.intyg.common.ag7804.model.internal.PrognosDagarTillArbeteTyp dagTyp = null;
    if (prognos != null && prognos.getTyp() != null) {
      se.inera.intyg.common.ag7804.model.internal.PrognosTyp typ =
          se.inera.intyg.common.ag7804.model.internal.PrognosTyp.fromId(prognos.getTyp().getId());

      if (prognos.getDagarTillArbete() != null) {
        dagTyp =
            se.inera.intyg.common.ag7804.model.internal.PrognosDagarTillArbeteTyp.fromId(
                prognos.getDagarTillArbete().getId());
      }
  ...
  private static List<se.inera.intyg.common.ag7804.model.internal.Sjukskrivning>
      convertSjukSkrivningar(ImmutableList<Sjukskrivning> sjukskrivningar) {
    return sjukskrivningar.stream()
        .map(
            ss ->
                se.inera.intyg.common.ag7804.model.internal.Sjukskrivning.create(
                    se.inera.intyg.common.ag7804.model.internal.Sjukskrivning.SjukskrivningsGrad
                        .fromId(ss.getSjukskrivningsgrad().getId()),
                    ss.getPeriod()))
        .collect(Collectors.toList());
  }

  private static List<se.inera.intyg.common.fkparent.model.internal.Diagnos> convertDiagnos(
      ImmutableList<Diagnos> diagnoser) {
    return diagnoser.stream()
        .map(
            d ->
                se.inera.intyg.common.fkparent.model.internal.Diagnos.create(
                    d.getDiagnosKod(),
                    d.getDiagnosKodSystem(),
                    d.getDiagnosBeskrivning(),
                    d.getDiagnosDisplayName()))
        .collect(Collectors.toList());
  }
   ...
   */
  private static List<se.inera.intyg.common.ag7804.model.internal.Sysselsattning>
      convertSysselsattning(ImmutableList<Sysselsattning> sysselsattning) {
