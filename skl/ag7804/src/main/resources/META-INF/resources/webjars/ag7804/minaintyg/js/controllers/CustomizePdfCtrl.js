 ...
 */
angular.module('ag7804').controller('ag7804.CustomizePdfCtrl',
    ['$window', '$location', '$log', '$rootScope', '$state', '$stateParams', '$scope', 'ag7804.customizeViewstate',
      'common.IntygListService',
      'common.IntygService', 'common.dialogService', 'common.messageService', '$cookies', '$http',
      function($window, $location, $log, $rootScope, $state, $stateParams, $scope, customizeViewstate, listCertService, certificateService,
          dialogService, messageService, $cookies, $http) {
        'use strict';

        $scope.messageService = messageService;
        $scope.customizeViewstate = customizeViewstate;
        $scope.downloadAsPdfLink = '/moduleapi/certificate/ag7804/' + $stateParams.intygTypeVersion + '/' +
            $stateParams.certificateId + '/pdf/arbetsgivarutskrift';

        certificateService.getCertificate('ag7804', $stateParams.intygTypeVersion, $stateParams.certificateId, function(result) {
        ...
        $scope.$on('$stateChangeStart', function(event, toState, toParams, fromState, fromParams, options) {
          if (toState.name.indexOf('ag7804-customize') !== -1 || leaveConfirmed) {
            return;
          }
        ...
        $scope.backToViewCertificate = function() {
          ...
          $location.path('/ag7804/' + $stateParams.intygTypeVersion + '/view/' + $stateParams.certificateId);
        };
