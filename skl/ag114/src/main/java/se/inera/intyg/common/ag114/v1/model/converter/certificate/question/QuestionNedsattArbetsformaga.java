...
public class QuestionNedsattArbetsformaga {
    ...
    public static CertificateDataElement toCertificate(String nedsattArbetsformaga, int index, CertificateTextProvider textProvider) {
        return CertificateDataElement.builder()
            .id(NEDSATT_ARBETSFORMAGA_SVAR_ID)
            .parent(CATEGORY_ARBETSFORMAGA_ID)
            .index(index)
            .config(
                CertificateDataConfigTextArea.builder()
                    .id(NEDSATT_ARBETSFORMAGA_SVAR_JSON_ID)
                    .text(textProvider.get(NEDSATT_ARBETSFORMAGA_SVAR_TEXT_ID))
                    .description(textProvider.get(NEDSATT_ARBETSFORMAGA_SVAR_DESCRIPTION_ID))
                    .build()
            )
            .value(
            ...
            .validation(
                new CertificateDataValidation[]{
                    CertificateDataValidationMandatory.builder()
                        .questionId(NEDSATT_ARBETSFORMAGA_SVAR_ID)
                        .expression(singleExpression(NEDSATT_ARBETSFORMAGA_SVAR_JSON_ID))
                        .build()
                }
            )
            .build();
    ...
    public static String toInternal(Certificate certificate) {
        return textValue(certificate.getData(), NEDSATT_ARBETSFORMAGA_SVAR_ID, NEDSATT_ARBETSFORMAGA_SVAR_JSON_ID);
    }
}
