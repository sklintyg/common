...
public class QuestionSjukskrivningsperiodMessage {
            ...
            new CertificateDataValidation[] {
              CertificateDataValidationShow.builder()
                  ...
                  .expression(
                      moreThanOrEqual(
                          subtract(
                              to(SJUKSKRIVNINGSGRAD_PERIOD_JSON_ID),
                              from(SJUKSKRIVNINGSGRAD_PERIOD_JSON_ID)),
                          DATE_RANGE_LIMIT))
                  .build()
            })
        .build();
  ...
  private static Boolean moreThanOrEqualFourteenDays(
      InternalLocalDateInterval sjukskrivningsperiod) {
    if (sjukskrivningsperiod == null
        || sjukskrivningsperiod.getFrom() == null
        || sjukskrivningsperiod.getTom() == null) {
      return false;
    }

    return ChronoUnit.DAYS.between(
            sjukskrivningsperiod.fromAsLocalDate(), sjukskrivningsperiod.tomAsLocalDate())
        >= DATE_RANGE_LIMIT;
  }
}
