...
public class InternalDraftValidatorImpl implements InternalDraftValidator<Ag114UtlatandeV1> {
  ...
  protected static final String AG114_SJUKSKRIVNINGSGRAD_INVALID_PERCENT =
      "ag114.validation.sjukskrivningsgrad.invalid.percent";
  protected static final String COMMON_VALIDATION_DATE_PERIOD_INVALID_ORDER =
  ...
  @Autowired private ValidatorUtilSKL validatorUtilSKL;
  ...
  private void validateGrundForMU(
      Ag114UtlatandeV1 utlatande, List<ValidationMessage> validationMessages) {
    ...
    if (utlatande.getUndersokningAvPatienten() == null
      ...
      se.inera.intyg.common.support.validate.ValidatorUtil.addValidationErrorWithQuestionId(
          ...
          GRUNDFORMEDICINSKTUNDERLAG_SVAR_ID);
    }

    // R18 - no need to check. they are already separated as different attributes and cannot occur
    // twice.

    // R21 - no date in future
    if (utlatande.getUndersokningAvPatienten() != null) {
      validateGrundForMuDate(
    ...
    if (utlatande.getAnnatGrundForMU() != null
      ...
      se.inera.intyg.common.support.validate.ValidatorUtil.addValidationErrorWithQuestionId(
          validationMessages,
          CATEGORY_GRUNDFORMU,
          GRUNDFORMEDICINSKTUNDERLAG_BESKRIVNING_DELSVAR_JSON_ID_10_3,
          ValidationMessageType.EMPTY,
  ...
  private static void validateGrundForMuDate(
      InternalDate date, List<ValidationMessage> validationMessages, String jsonId) {
  ...
  private void validateBedomning(
    ...
    if (validatorUtilSKL.hasNoContent(utlatande.getSjukskrivningsgrad())) {
      ValidatorUtil.addValidationErrorWithQuestionId(
          validationMessages,
          RespConstants.CATEGORY_BEDOMNING,
          SJUKSKRIVNINGSGRAD_SVAR_JSON_ID_7_1,
          ValidationMessageType.EMPTY,
          SJUKSKRIVNINGSGRAD_SVAR_ID);
    } else {
      if (!validatorUtilSKL.isIntInRange(
          utlatande.getSjukskrivningsgrad(), SJUKSKRIVNINGSGRAD_FROM, SJUKSKRIVNINGSGRAD_TOM)) {
        ValidatorUtil.addValidationErrorWithQuestionId(
            validationMessages,
            RespConstants.CATEGORY_BEDOMNING,
            SJUKSKRIVNINGSGRAD_SVAR_JSON_ID_7_1,
            ValidationMessageType.OTHER,
            AG114_SJUKSKRIVNINGSGRAD_INVALID_PERCENT,
            SJUKSKRIVNINGSGRAD_SVAR_ID);
      }
    }
    ...
    if (utlatande.getSjukskrivningsperiod() == null) {
      ValidatorUtil.addValidationErrorWithQuestionId(
          validationMessages,
          RespConstants.CATEGORY_BEDOMNING,
          SJUKSKRIVNINGSGRAD_PERIOD_JSON_ID + ".period",
          ValidationMessageType.EMPTY,
          "common.validation.ue-sjukfranvaro.period.invalid",
          SJUKSKRIVNINGSGRAD_PERIOD_SVAR_ID);
    } else {

      boolean fromDateValid =
          ValidatorUtil.validateDate(
              utlatande.getSjukskrivningsperiod().getFrom(),
              validationMessages,
              RespConstants.CATEGORY_BEDOMNING,
              SJUKSKRIVNINGSGRAD_PERIOD_JSON_ID + ".from",
              null,
              SJUKSKRIVNINGSGRAD_PERIOD_SVAR_ID);

      boolean toDateValid =
          ValidatorUtil.validateDate(
              utlatande.getSjukskrivningsperiod().getTom(),
              validationMessages,
              RespConstants.CATEGORY_BEDOMNING,
              SJUKSKRIVNINGSGRAD_PERIOD_JSON_ID + ".tom",
              null,
              SJUKSKRIVNINGSGRAD_PERIOD_SVAR_ID);
      ...
      if (fromDateValid && toDateValid && !utlatande.getSjukskrivningsperiod().isValid()) {
        ValidatorUtil.addValidationErrorWithQuestionId(
            ...
            COMMON_VALIDATION_DATE_PERIOD_INVALID_ORDER,
            SJUKSKRIVNINGSGRAD_PERIOD_SVAR_ID);
      }
    }
  }
  ...
  private void validateSysselsattning(
    ...
    if (utlatande.getSysselsattning() == null
        ...
        || !utlatande
            ...
            .get(0)
            .getTyp()
            .equals(Sysselsattning.SysselsattningsTyp.NUVARANDE_ARBETE)) {
      ValidatorUtil.addValidationErrorWithQuestionId(
          validationMessages,
          RespConstants.CATEGORY_SYSSELSATTNING,
          TYP_AV_SYSSELSATTNING_SVAR_JSON_ID_1,
          ValidationMessageType.EMPTY,
          TYP_AV_SYSSELSATTNING_SVAR_ID);
    }
  ...
  private void validateDiagnos(
    ...
    if (utlatande.getOnskarFormedlaDiagnos() == null) {
      ValidatorUtil.addValidationErrorWithQuestionId(
          validationMessages,
          RespConstants.CATEGORY_DIAGNOS,
  ...
  private void validateNedsattArbetsformaga(
      Ag114UtlatandeV1 utlatande, List<ValidationMessage> validationMessages) {
    if (validatorUtilSKL.hasNoContent(utlatande.getNedsattArbetsformaga())) {
      ValidatorUtil.addValidationErrorWithQuestionId(
          validationMessages,
          RespConstants.CATEGORY_ARBETSFORMAGA,
          NEDSATT_ARBETSFORMAGA_SVAR_JSON_ID_5,
          ValidationMessageType.EMPTY,
          NEDSATT_ARBETSFORMAGA_SVAR_ID);
    }
  }

  private void validateArbetsformagaTrotsSjukdom(
      Ag114UtlatandeV1 utlatande, List<ValidationMessage> validationMessages) {
  ...
  private void validateKontakt(
    ...
    if (utlatande.getKontaktMedArbetsgivaren() != null
        && utlatande.getKontaktMedArbetsgivaren()
        && validatorUtilSKL.hasNoContent(utlatande.getAnledningTillKontakt())) {
      ValidatorUtil.addValidationErrorWithQuestionId(
          validationMessages,
          RespConstants.CATEGORY_KONTAKT,
