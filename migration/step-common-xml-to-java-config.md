...
| **C.12** | Delete all XML files + remove `@ImportResource` from intygstjänst                      | ✅ DONE | ✅        | All 47 Spring XML config files deleted from `src/main/resources` (1 `common-config.xml`, 16 `module-config.xml`, 15 `*-beans.xml`, 1 `fk7263-ws-stub.xml`, 3 `it-module-cxf-servlet.xml`, 2 `wc-module-cxf-servlet.xml`); `@ImportResource` removal is in intygstjänst (coordinate with that app)                                                                                                                 |
