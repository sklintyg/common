 ...
 */
package se.inera.intyg.common.doi.v1.testability;
...
import static se.inera.intyg.common.support.facade.util.DataValueUtil.getDataUncertainDateValue;
...
public class DoiTestabilityCertificateTestdataProvider
  ...
  public Map<String, CertificateDataValue> getMinimumValues() {
    ...
    final var dodsplatsBoende =
        getDataValueCode(DODSPLATS_BOENDE_DELSVAR_ID, DodsplatsBoende.ANNAN.name());
    ...
    final var terminalDodsorsak =
        getDataValueCauseOfDeath(
    ...
    final var dodsorsaksUppgifter =
        getDataValueCodeList(
  ...
  public Map<String, CertificateDataValue> getMaximumValues() {
    ...
    final var osakertDatum =
        getDataUncertainDateValue(DODSDATUM_JSON_ID, LocalDate.now().toString());
    values.put(DODSDATUM_OSAKERT_DELSVAR_ID, osakertDatum);

    final var antraffadDod =
        getDataValueDate(ANTRAFFAT_DOD_DATUM_JSON_ID, LocalDate.now().minusDays(10));
    ...
    final var forgiftningsOrsak =
        getDataValueCode(FORGIFTNING_ORSAK_JSON_ID, ForgiftningOrsak.OLYCKSFALL.name());
    ...
    final var forgiftningUppkommelse =
        getDataValueText(FORGIFTNING_UPPKOMMELSE_JSON_ID, uppkommelseForgiftning);
  ...
  private CertificateDataValueCauseOfDeath getDataValueCauseOfDeath(
      String id,
      String descriptionId,
      String datumId,
      String descriptionText,
      LocalDate date,
      String specifikationName) {
    return CertificateDataValueCauseOfDeath.builder()
        .id(id)
        .description(
            CertificateDataValueText.builder().id(descriptionId).text(descriptionText).build())
        .debut(CertificateDataValueDate.builder().id(datumId).date(date).build())
        .specification(
