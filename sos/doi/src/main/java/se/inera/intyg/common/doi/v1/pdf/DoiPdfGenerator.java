...
public class DoiPdfGenerator extends AbstractSoSPdfGenerator {
  ...
  // Type TEXT
  private static final String FIELD_LAND_OM_EJ_STADIGVARANDE_BOSATT_I_SVERIGE =
  ...
  // Type TEXT
  private static final String FIELD_OM_DODSDATUM_EJ_SAKERT_AR_MAN_DAG_ANTRAFFAD_DOD =
      "Om dödsdatum ej säkert År mån dag anträffad död";

  // Type TEXT
  private static final String FIELD_KOMMUN_OM_OKAND_DODSPLATS_KOMMUNEN_DAR_KROPPEN_PATRAFFADES =
  ...
  private static final String FIELD_DODSORSAK_FIRSTROW_BESKRIVNING =
      "Den terminala dödsorsaken var_%s";
  private static final String FIELD_DODSORSAK_ROW_BESKRIVNING = "Den terminala dödsorsaken var_%s";
  private static final String FIELD_DODSORSAK_ROW_UNGEFARLIG_DEBUT =
      "Ungefärlig debut år mån dag_%s";
  ...
  private static final String FIELD_BIDRAGANDE_DODSORSAK_ROW_BESKRIVNING =
      "Andra sjukdomar eller skador som bidragit till" + " dödsfalletRow%d";
  private static final String FIELD_BIDRAGANDE_DODSORSAK_ROW_UNGEFARLIG_DEBUT =
      "Ungefärlig debut år mån dag_%d";
  ...
  // Type TEXT
  private static final String FIELD_TILLSTAND_SOM_FORANLEDDE_INGREPPET =
  ...
  // Type TEXT
  private static final String FIELD_DATUM_FOR_SKADAFORGIFTNING_AR_MAN_DAG =
      "Datum för skadaförgiftning år mån dag";
  // Type TEXT
  private static final String FIELD_KORT_BESKRIVNING_AV_HUR_SKADANFORGIFTNINGEN_UPPKOM =
      "Kort beskrivning av hur skadanförgiftningen"
  ...
  // Type TEXT
  private static final String FIELD_LAKARENS_EFTERNAMN_OCH_FORNAMN =
  ...
  public DoiPdfGenerator(
      DoiUtlatandeV1 intyg, IntygTexts intygTexts, List<Status> statuses, UtkastStatus utkastStatus)
      throws SoSPdfGeneratorException {
    this(intyg, intygTexts, statuses, utkastStatus, true);
  }

  protected DoiPdfGenerator(
    ...
    try {
      ...
      // If not removing Adobe Reader Usage rights signature, you get a warning when opening i Adobe
      // Reader.
      ...
      if (!isUtkast && !isLocked) {
        ...
        createRightMarginText(
            pdfStamper, pdfReader.getNumberOfPages(), utlatande.getId(), WEBCERT_MARGIN_TEXT);
      }
  ...
  private void fillAcroformFields() throws IOException, DocumentException {
    ...
    fillSignature(
        doiUtlatandeV1.getGrundData().getSigneringsdatum(),
        doiUtlatandeV1.getGrundData().getSkapadAv());
  }
  ...
  protected void fillPatientDetails() throws IOException, DocumentException {
    ...
    fillText(
        ...
        truncateTextIfNeeded(
            doiUtlatandeV1.getGrundData().getPatient().getPostadress(), FIELD_BOSTADSADRESS));
    fillText(FIELD_POSTNUMMER, doiUtlatandeV1.getGrundData().getPatient().getPostnummer());
    fillText(
        FIELD_POSTORT,
        truncateTextIfNeeded(
            doiUtlatandeV1.getGrundData().getPatient().getPostort(), FIELD_POSTORT));
  ...
  private void fillDateOfDeath() {
    ...
    adjustAndFill(
        FIELD_OM_DODSDATUM_EJ_SAKERT_AR_MAN_DAG_ANTRAFFAD_DOD,
        doiUtlatandeV1.getAntraffatDodDatum(),
        ADJUST_BY_5);
  }
  ...
  private void fillPlaceOfDeath() {
    ...
    adjustAndFill(
        FIELD_KOMMUN_OM_OKAND_DODSPLATS_KOMMUNEN_DAR_KROPPEN_PATRAFFADES,
        doiUtlatandeV1.getDodsplatsKommun(),
        ADJUST_BY_9);
  }
  ...
  private void fillCauseOfDeath() throws DocumentException, IOException {
    ...
    if (doiUtlatandeV1.getFoljd() != null) {
      final var drDictionary =
          fields.getFieldItem(fieldDescriptionMainCause).getMerged(0).getAsDict(PdfName.DR);
      for (int i = 0; i < doiUtlatandeV1.getFoljd().size(); i++) {
        final var fieldDescriptionSecondaryCause =
            String.format(FIELD_DODSORSAK_ROW_BESKRIVNING, 2 + i);
        setFieldFont(drDictionary, fieldDescriptionSecondaryCause);
        fillCauseOfDeathRow(
            2 + i, fieldDescriptionSecondaryCause, doiUtlatandeV1.getFoljd().get(i));
      }
    }
  }

  private void fillCauseOfDeathRow(int index, String fieldDescription, Dodsorsak orsak)
  ...
  private void fillContributingCauseRow(int index, Dodsorsak cause)
    ...
    final var fieldDebut =
        String.format(FIELD_BIDRAGANDE_DODSORSAK_ROW_UNGEFARLIG_DEBUT, FIELD_INDEX_4 + index);
    ...
    final var fieldNotAvailable =
        String.format(FIELD_DODSORSAK_ROW_INGEN_UPPGIFT, FIELD_INDEX_4 + index);
  ...
  private void setCauseOfDeathFields(
      Dodsorsak cause,
    ...
    if (cause != null) {
      ...
      checkCheckboxField(
          fieldAcute, Specifikation.PLOTSLIG == cause.getSpecifikation() ? "Ja" : "");
      checkCheckboxField(
          fieldKronisk, Specifikation.KRONISK == cause.getSpecifikation() ? "Ja" : "");
      checkCheckboxField(
          fieldNotAvailable, Specifikation.UPPGIFT_SAKNAS == cause.getSpecifikation() ? "Ja" : "");
    }
  }
  ...
  private void fillSurgery() {
    ...
    adjustAndFill(
        FIELD_TILLSTAND_SOM_FORANLEDDE_INGREPPET,
        doiUtlatandeV1.getOperationAnledning(),
        ADJUST_BY_4);
  }
  ...
  private void fillInjuryPoisoning() {
    ...
    adjustAndFill(
        FIELD_KORT_BESKRIVNING_AV_HUR_SKADANFORGIFTNINGEN_UPPKOM,
  ...
  private void fillBasisForCauseOfDeath() {
    checkCheckboxField(
        FIELD_UNDERSOKNING_FORE_DODEN,
        hasDodsorsaksGrund(Dodsorsaksgrund.UNDERSOKNING_FORE_DODEN) ? "Ja" : "");
    checkCheckboxField(
        FIELD_YTTRE_UNDERSOKNING_EFTER_DODEN,
        hasDodsorsaksGrund(Dodsorsaksgrund.UNDERSOKNING_EFTER_DODEN) ? "Ja" : "");
    checkCheckboxField(
        FIELD_KLINISK_OBDUKTION, hasDodsorsaksGrund(Dodsorsaksgrund.KLINISK_OBDUKTION) ? "Ja" : "");
    checkCheckboxField(
        FIELD_RATTSMEDICINSK_OBDUKTION,
        hasDodsorsaksGrund(Dodsorsaksgrund.RATTSMEDICINSK_OBDUKTION) ? "Ja" : "");
    checkCheckboxField(
        FIELD_RATTSMEDICINSK_LIKBESIKTNING,
        hasDodsorsaksGrund(Dodsorsaksgrund.RATTSMEDICINSK_BESIKTNING) ? "Ja" : "");
  }
  ...
  private void fillSignature(LocalDateTime signeringsDatum, HoSPersonal skapadAv)
    ...
    if (signeringsDatum != null) {
      ...
      adjustAndFill(
          FIELD_LAKARENS_EFTERNAMN_OCH_FORNAMN, skapadAv.getFullstandigtNamn(), ADJUST_BY_6);
      adjustAndFill(
          FIELD_BEFATTNING, String.join(", ", skapadAv.getBefattningarAsCode()), ADJUST_BY_6);
    }
    adjustAndFill(
        FIELD_TJANSTESTALLE,
        String.join(
            ", ",
            skapadAv.getVardenhet().getEnhetsnamn()
                + ", "
                + skapadAv.getVardenhet().getVardgivare().getVardgivarnamn()),
        ADJUST_BY_6);
    adjustAndFill(
        FIELD_UTDELNINGSADRESS,
        truncateTextIfNeeded(skapadAv.getVardenhet().getPostadress(), FIELD_UTDELNINGSADRESS),
        ADJUST_BY_6);
    adjustAndFill(FIELD_POSTNUMMER_2, skapadAv.getVardenhet().getPostnummer(), ADJUST_BY_3);
    adjustAndFill(
        FIELD_POSTORT_2,
        truncateTextIfNeeded(skapadAv.getVardenhet().getPostort(), FIELD_POSTORT_2),
        ADJUST_BY_3);
    adjustAndFill(
        FIELD_TELEFON_INKL_RIKTNUMMER, skapadAv.getVardenhet().getTelefonnummer(), ADJUST_BY_6);
  ...
  private void setFieldFont(PdfDictionary drDictionary, String fieldDescriptionSecondaryCause) {
    ...
    if (drDictionary != null && drDictionary.isDictionary()) {
      fields
          .getFieldItem(fieldDescriptionSecondaryCause)
          .getMerged(0)
          .put(PdfName.DR, drDictionary);
    }
  }

  private void setCauseOfDeathFieldFontSize(String fieldDescription, String text)
      throws DocumentException, IOException {
  ...
  private float calculateFontSize(String fieldDescription, String text)
      throws DocumentException, IOException {
    if (text == null) {
    ...
    final var leadingFactor =
        baseFont.getFontDescriptor(BaseFont.BBOXURY, 1)
            - baseFont.getFontDescriptor(BaseFont.BBOXLLY, 1);
  ...
  private Rectangle getFieldRectangle(PdfArray fieldCoordinates) {
    return new Rectangle(
        fieldCoordinates.getAsNumber(0).floatValue(),
        fieldCoordinates.getAsNumber(1).floatValue(),
        fieldCoordinates.getAsNumber(2).floatValue(),
        fieldCoordinates.getAsNumber(FIELD_INDEX_3).floatValue() - 1f);
  }
