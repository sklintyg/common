...
public class QuestionBidragandeSjukdomar {
  ...
  public static CertificateDataElement toCertificate(
      List<Dodsorsak> bidragandeSjukdomar, int index, CertificateTextProvider texts) {
  ...
  private static CertificateDataValueCauseOfDeath getCaseOfDeathValue(
      String id, Dodsorsak bidragandeSjukdom) {
    return CertificateDataValueCauseOfDeath.builder()
        .id(id)
        .description(getDescription(bidragandeSjukdom, id))
        .debut(getDebut(bidragandeSjukdom, id))
        .specification(getSpecification(bidragandeSjukdom))
        .build();
  }
  ...
  private static CertificateDataValueDate getDebut(Dodsorsak bidragandeSjukdom, String id) {
    return CertificateDataValueDate.builder()
        ...
        .date(
            bidragandeSjukdom != null && bidragandeSjukdom.getDatum() != null
                ? bidragandeSjukdom.getDatum().asLocalDate()
                : null)
        .build();
  }
  ...
  private static CertificateDataValueCode getSpecification(Dodsorsak bidragandeSjukdom) {
    final var name =
        bidragandeSjukdom != null && bidragandeSjukdom.getSpecifikation() != null
            ? bidragandeSjukdom.getSpecifikation().name()
            : null;
    return CertificateDataValueCode.builder().id(name).code(name).build();
  }
  ...
  public static List<Dodsorsak> toInternal(Certificate certificate) {
    ...
    final var causeOfDeathListValue =
        (CertificateDataValueCauseOfDeathList)
            certificate.getData().get(BIDRAGANDE_SJUKDOM_OM_DELSVAR_ID).getValue();
    final var dodsorsakList =
        causeOfDeathListValue.getList().stream()
            .map(
                causeOfDeath ->
                    Dodsorsak.create(
