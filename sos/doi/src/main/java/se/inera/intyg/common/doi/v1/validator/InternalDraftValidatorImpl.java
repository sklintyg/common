...
public class InternalDraftValidatorImpl implements InternalDraftValidator<DoiUtlatandeV1> {
  ...
  @Override
  public ValidateDraftResponse validateDraft(
      DoiUtlatandeV1 utlatande, TypeAheadProvider typeAheadProvider) {
  ...
  private void validateDodsorsak(
      DoiUtlatandeV1 utlatande, List<ValidationMessage> validationMessages) {
    if (utlatande.getTerminalDodsorsak() == null
        || Strings.nullToEmpty(utlatande.getTerminalDodsorsak().getBeskrivning())
            .trim()
            .isEmpty()) {
      ValidatorUtil.addValidationErrorWithQuestionId(
          validationMessages,
          "utlatandeOrsak",
          TERMINAL_DODSORSAK_JSON_ID,
    ...
    if (utlatande.getTerminalDodsorsak() != null
      ...
      if (ValidatorUtil.validateDateAndCheckIfFuture(
          utlatande.getTerminalDodsorsak().getDatum(),
          validationMessages,
          "utlatandeOrsak",
          validationField,
        ...
        if (ValidatorUtil.isNotNullTrue(utlatande.getDodsdatumSakert())
            ...
            && utlatande.getDodsdatum().isValidDate()
            && ValidatorUtil.isDateAfter(
                utlatande.getTerminalDodsorsak().getDatum(), utlatande.getDodsdatum())) {
          ValidatorUtil.addValidationErrorWithQuestionId(
              validationMessages,
              "utlatandeOrsak",
              validationField,
              ValidationMessageType.INCORRECT_COMBINATION,
              "doi.validation.terminalDodsorsak.datum.efterDodsdatum",
              DODSORSAK_DELSVAR_ID);
        } else if (ValidatorUtil.isNotNullFalse(utlatande.getDodsdatumSakert())
            // R22-2 - får inte infalla efter datumet då man anträffade den döde, om dödsdatumet är
            // ej säkert.
            && utlatande.getAntraffatDodDatum() != null
            && utlatande.getAntraffatDodDatum().isValidDate()
            && ValidatorUtil.isDateAfter(
                utlatande.getTerminalDodsorsak().getDatum(), utlatande.getAntraffatDodDatum())) {
          ValidatorUtil.addValidationErrorWithQuestionId(
              validationMessages,
              "utlatandeOrsak",
              validationField,
  ...
  private void validateFoljd(DoiUtlatandeV1 utlatande, List<ValidationMessage> validationMessages) {
    ...
    // From beginning its dodsdatum and then each foljd in instance order should be before or equal
    // to the previous dates.
    LocalDate minDate =
        (utlatande.getTerminalDodsorsak() != null
                && utlatande.getTerminalDodsorsak().getDatum() != null
                && utlatande.getTerminalDodsorsak().getDatum().isValidDate())
      ...
      if (foljd.getDatum() != null) {
        ...
        Boolean validDate =
            ValidatorUtil.validateDate(
                foljd.getDatum(), validationMessages, "utlatandeOrsak", validationField, null);
  ...
  private void validateBidragandeSjukdomar(
      DoiUtlatandeV1 utlatande, List<ValidationMessage> validationMessages) {
    if (utlatande.getBidragandeSjukdomar().size() > MAX_BIDRAGANDE_SJUKDOMAR) {
      ValidatorUtil.addValidationError(
          validationMessages,
          "utlatandeOrsak",
          BIDRAGANDE_SJUKDOM_JSON_ID,
          ValidationMessageType.INCORRECT_COMBINATION);
    }
    for (int i = 0; i < utlatande.getBidragandeSjukdomar().size(); i++) {
      ...
      if (Strings.nullToEmpty(bidragandeSjukdom.getBeskrivning()).trim().isEmpty()) {
        ValidatorUtil.addValidationErrorWithQuestionId(
            validationMessages,
            "utlatandeOrsak",
            BIDRAGANDE_SJUKDOM_JSON_ID + "[" + i + "]." + DODSORSAK_OM_JSON_ID,
            ValidationMessageType.EMPTY,
      ...
      if (bidragandeSjukdom.getDatum() != null) {
        ...
        if (bidragandeSjukdom.getDatum().isValidDate()
          ...
          ValidatorUtil.addValidationErrorWithQuestionId(
            ...
            && ValidatorUtil.isDateAfter(
                bidragandeSjukdom.getDatum(), utlatande.getAntraffatDodDatum())) {
          ValidatorUtil.addValidationErrorWithQuestionId(
              validationMessages,
              "utlatandeOrsak",
              validationField,
      ...
      if (ValidatorUtil.validateDate(
        ...
        if (ValidatorUtil.isDateAfter(utlatande.getOperationDatum(), utlatande.getDodsdatum())) {
          ValidatorUtil.addValidationErrorWithQuestionId(
              ...
              OPERATION_DATUM_DELSVAR_ID);
        } else if (ValidatorUtil.isDateAfter(
            utlatande.getOperationDatum(), utlatande.getAntraffatDodDatum())) {
          ValidatorUtil.addValidationErrorWithQuestionId(
              validationMessages,
              "operation",
              OPERATION_DATUM_JSON_ID,
              ...
              OPERATION_DATUM_DELSVAR_ID);
        } else if (utlatande.getDodsdatumSakert() != null
            && utlatande.getDodsdatumSakert()
            && ValidatorUtil.isDateAfter(
                utlatande.getDodsdatum(),
                new InternalDate(
                    utlatande.getOperationDatum().asLocalDate().plusDays(FOUR_WEEKS_IN_DAYS)))) {
          ValidatorUtil.addValidationErrorWithQuestionId(
              validationMessages,
              "operation",
              OPERATION_DATUM_JSON_ID,
  ...
  private void validateForgiftning(
      DoiUtlatandeV1 utlatande, List<ValidationMessage> validationMessages) {
    if (utlatande.getForgiftning() == null) {
      ValidatorUtil.addValidationErrorWithQuestionId(
          validationMessages,
          "forgiftning",
          FORGIFTNING_OM_JSON_ID,
          ValidationMessageType.EMPTY,
          FORGIFTNING_OM_DELSVAR_ID);
      ...
      if (utlatande.getForgiftningOrsak() == null) {
        ValidatorUtil.addValidationErrorWithQuestionId(
            validationMessages,
            "forgiftning",
            FORGIFTNING_ORSAK_JSON_ID,
            ValidationMessageType.EMPTY,
            FORGIFTNING_ORSAK_DELSVAR_ID);
      }
      // R16
      if (ValidatorUtil.validateDate(
          utlatande.getForgiftningDatum(),
          validationMessages,
          "forgiftning",
          FORGIFTNING_DATUM_JSON_ID,
          null,
        ...
        if (ValidatorUtil.isDateAfter(utlatande.getForgiftningDatum(), utlatande.getDodsdatum())) {
          ValidatorUtil.addValidationErrorWithQuestionId(
            ...
            utlatande.getForgiftningDatum(), utlatande.getAntraffatDodDatum())) {
          ValidatorUtil.addValidationErrorWithQuestionId(
              validationMessages,
              "forgiftning",
              FORGIFTNING_DATUM_JSON_ID,
  ...
  private void validateGrunder(
    ...
    if (utlatande.getGrunder().isEmpty()) {
      ...
      ValidatorUtil.addValidationErrorWithQuestionId(
          ...
          ValidationMessageType.OTHER,
          GRUNDER_DELSVAR_ID);
    } else if (utlatande.getGrunder().size()
        != utlatande.getGrunder().stream().distinct().count()) {
      ValidatorUtil.addValidationErrorWithQuestionId(
          validationMessages,
          "dodsorsakgrund",
          GRUNDER_JSON_ID,
