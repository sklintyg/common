...
public class CategoryKompletterandePatientuppgifter {

    public static CertificateDataElement toCertificate(int index, CertificateTextProvider texts) {
        return CertificateDataElement.builder()
            .id(KOMPLETTERANDE_PATIENTUPPGIFTER_CATEGORY_ID)
            .index(index)
            .config(
                CertificateDataConfigCategory.builder()
                    .text(texts.get(KOMPLETTERANDE_PATIENTUPPGIFTER_TEXT_ID))
                    .build()
            )
            .build();
    }
