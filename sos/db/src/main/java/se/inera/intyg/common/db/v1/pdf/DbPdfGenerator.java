...
public class DbPdfGenerator extends AbstractSoSPdfGenerator {
  ...
  // Type TEXT
  private static final String FIELD_PERSONNUMMERSAMORDNINGSNUMMER_12_SIFFROR =
      "Personnummersamordningsnummer 12 siffror";
  // Type TEXT
  ...
  private static final String FIELD_OM_DODSDATUM_EJ_SAKERT_AR_MAN_DAG_ANTRAFFAD_DOD =
      "År mån dag Om dödsdatum";

  // Type TEXT
  private static final String FIELD_KOMMUN_OM_OKAND_DODSPLATS =
  ...
  public DbPdfGenerator(
      DbUtlatandeV1 intyg, IntygTexts intygTexts, List<Status> statuses, UtkastStatus utkastStatus)
  ...
  private void fillAcroformFields() throws IOException, DocumentException {
    ...
    fillPoliceReport();
    fillSignature(
        dbUtlatandeV1.getGrundData().getSigneringsdatum(),
        dbUtlatandeV1.getGrundData().getSkapadAv());
  }
  ...
  private void fillExternalExamination() {
    ...
    if (dbUtlatandeV1.getUndersokningYttre() != null) {
      ...
      } else if (dbUtlatandeV1.getUndersokningYttre()
    ...
    adjustAndFill(
        FIELD_YTTRE_UNDERSOKNING_AR_MAN_DAG, dbUtlatandeV1.getUndersokningDatum(), ADJUST_BY_5);
  }
  ...
  private void fillDateOfDeath() {
    ...
    fillText(
        ...
        dbUtlatandeV1.getAntraffatDodDatum());
  }
  ...
  protected void fillPatientDetails() throws IOException, DocumentException {
    adjustAndFill(
        FIELD_PERSONNUMMERSAMORDNINGSNUMMER_12_SIFFROR,
        dbUtlatandeV1.getGrundData().getPatient().getPersonId().getPersonnummer(),
        ADJUST_BY_9);
    adjustAndFill(
        FIELD_EFTERNAMN, dbUtlatandeV1.getGrundData().getPatient().getEfternamn(), ADJUST_BY_6);
    adjustAndFill(
        FIELD_FORNAMN, dbUtlatandeV1.getGrundData().getPatient().getFornamn(), ADJUST_BY_6);
    adjustAndFill(
        FIELD_BOSTADSADRESS,
        truncateTextIfNeeded(
            dbUtlatandeV1.getGrundData().getPatient().getPostadress(), FIELD_BOSTADSADRESS),
        ADJUST_BY_6);
    adjustAndFill(
        FIELD_POSTNUMMER, dbUtlatandeV1.getGrundData().getPatient().getPostnummer(), ADJUST_BY_6);
    adjustAndFill(
        FIELD_POSTORT,
        truncateTextIfNeeded(dbUtlatandeV1.getGrundData().getPatient().getPostort(), FIELD_POSTORT),
        ADJUST_BY_6);
  ...
  // Adjusts the position of a checkbox field, Y-indexes are 1 (bottom) and 3 (top), X-indexes are 0
  // and 2.
  private void adjustAndFillCheckBox(String fieldId, String value) {
    if (value != null) {
      PdfArray fieldCoordinates = getFieldCoordinates(fieldId);
      fieldCoordinates.set(
          1, new PdfNumber(fieldCoordinates.getAsNumber(1).doubleValue() - CHECKBOX_Y_ADJUST));
      //noinspection CheckStyle
      fieldCoordinates.set(
          3, new PdfNumber(fieldCoordinates.getAsNumber(3).doubleValue() - CHECKBOX_Y_ADJUST));
      fieldCoordinates.set(
          0, new PdfNumber(fieldCoordinates.getAsNumber(0).doubleValue() - CHECKBOX_X_ADJUST));
      fieldCoordinates.set(
          2, new PdfNumber(fieldCoordinates.getAsNumber(2).doubleValue() - CHECKBOX_X_ADJUST));
