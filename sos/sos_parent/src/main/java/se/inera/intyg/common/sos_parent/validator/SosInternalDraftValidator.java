...
public final class SosInternalDraftValidator {
  ...
  public static void validateIdentitetStyrkt(
      SosUtlatande utlatande, List<ValidationMessage> validationMessages) {
    if (Strings.nullToEmpty(utlatande.getIdentitetStyrkt()).trim().isEmpty()) {
      ValidatorUtil.addValidationErrorWithQuestionId(
          validationMessages,
          "personuppgifter",
          IDENTITET_STYRKT_JSON_ID,
          ValidationMessageType.EMPTY,
          IDENTITET_STYRKT_DELSVAR_ID);
    }
  }

  public static void validateDodsdatum(
      SosUtlatande utlatande, List<ValidationMessage> validationMessages, String prefix) {
    if (utlatande.getDodsdatumSakert() == null) {
      ValidatorUtil.addValidationErrorWithQuestionId(
          validationMessages,
          "dodsdatumOchdodsPlats",
          DODSDATUM_SAKERT_JSON_ID,
          ValidationMessageType.EMPTY,
          DODSDATUM_SAKERT_DELSVAR_ID);
      return;
    }
    final var patientBirthDateFromPersonId =
    ...
    if (utlatande.getDodsdatum() == null) {
      if (utlatande.getDodsdatumSakert()) {
        ...
        ValidatorUtil.addValidationErrorWithQuestionId(
            ...
            DODSDATUM_JSON_ID,
            ValidationMessageType.EMPTY,
            "common.validation.ue-vaguedate.empty.year",
        ...
        ValidatorUtil.addValidationErrorWithQuestionId(
            ...
            DODSDATUM_JSON_ID,
            ValidationMessageType.EMPTY,
            "common.validation.ue-vaguedate.empty.month",
            DODSDATUM_OSAKERT_DELSVAR_ID);
      }
      ...
      if (ValidatorUtil.validateDate(
        ...
        if (!utlatande.getDodsdatum().isBeforeNumDays(-1)) {
          ValidatorUtil.addValidationErrorWithQuestionId(
            ...
            && utlatande.getDodsdatum().isBefore(patientBirthDateFromPersonId)) {
          ValidatorUtil.addValidationErrorWithQuestionId(
              validationMessages,
              "dodsdatumOchdodsPlats",
              DODSDATUM_JSON_ID,
    ...
    if (!utlatande.getDodsdatumSakert()) {
      ...
      if (ValidatorUtil.validateDate(
          utlatande.getAntraffatDodDatum(),
          validationMessages,
          "dodsdatumOchdodsPlats",
          ANTRAFFAT_DOD_DATUM_JSON_ID,
          null,
        ...
        if (!utlatande.getAntraffatDodDatum().isBeforeNumDays(-1)) {
          ...
          ValidatorUtil.addValidationErrorWithQuestionId(
              ...
              ANTRAFFAT_DOD_DATUM_DELSVAR_ID);
        } else if (utlatande.getDodsdatum() != null
            && utlatande
                .getDodsdatum()
          ...
          ValidatorUtil.addValidationErrorWithQuestionId(
              ...
              prefix + ".validation.datum.innanDodsdatum",
              ANTRAFFAT_DOD_DATUM_DELSVAR_ID);
  ...
  public static void validateDodsplats(
      SosUtlatande utlatande,
    ...
    if (Strings.nullToEmpty(utlatande.getDodsplatsKommun()).trim().isEmpty()) {
      ValidatorUtil.addValidationErrorWithQuestionId(
          validationMessages,
          "dodsdatumOchdodsPlats",
          DODSPLATS_KOMMUN_JSON_ID,
          ValidationMessageType.EMPTY,
          DODSPLATS_KOMMUN_DELSVAR_ID);
      ...
      if (!validMunicipalityValues.contains(value)) {
        ValidatorUtil.addValidationErrorWithQuestionId(
            ...
            DODSPLATS_KOMMUN_JSON_ID,
            ValidationMessageType.INVALID_FORMAT,
            "common.validation.ue-typeahead.invalid",
            DODSPLATS_KOMMUN_DELSVAR_ID);
      }
    }
    ...
    if (utlatande.getDodsplatsBoende() == null) {
      ValidatorUtil.addValidationErrorWithQuestionId(
          validationMessages,
          "dodsdatumOchdodsPlats",
          DODSPLATS_BOENDE_JSON_ID,
          ValidationMessageType.EMPTY,
          DODSPLATS_BOENDE_DELSVAR_ID);
    }
  }

  public static void validateBarn(
      SosUtlatande utlatande, List<ValidationMessage> validationMessages) {
    if (utlatande.getBarn() == null) {
      ...
      if (utlatande.getBarn()
          && utlatande
              .getDodsdatum()
              .asLocalDate()
              .isAfter(patientBirthDate.plus(BARN_SOM_AVLIDIT_INOM_28_DAGAR, ChronoUnit.DAYS))) {
        ValidatorUtil.addValidationErrorWithQuestionId(
            validationMessages,
            "barnSomAvlidit",
            BARN_JSON_ID,
