...
public abstract class AbstractSoSPdfGenerator {
  ...
  protected PdfReader getPdfReader(
      SosUtlatande utlatande, IntygTexts intygTexts, String defaultTemplatePath) throws Exception {
  ...
  protected String getPdfPathFromIntygVersion(
      Utlatande utlatande, IntygTexts intygTexts, String defaultPath)
  ...
  /**
   * Simple util method that helps translade a Boolean instance state to 3 possible value, one for
   * null and others for true/false.
   */
  protected String getRadioValueFromBoolean(
      Boolean booleanValue, String whenNull, String whenTrue, String whenFalse) {
  ...
  protected void fillText(String fieldId, InternalDate date) {
    try {
      ...
      throw new IllegalArgumentException(
          "Could not fill field '" + fieldId + "' with value '" + date + "'", e);
    }
  }
  ...
  protected void adjustAndFill(String fieldId, InternalDate date, double adjustment) {
    if (date != null) {
      ...
      fieldCoordinates.set(
          3, new PdfNumber(fieldCoordinates.getAsNumber(3).doubleValue() - adjustment));
  ...
  protected double getFieldWidth(String fieldId) {
    PdfArray fieldCoordinates = getFieldCoordinates(fieldId);
    return fieldCoordinates.getAsNumber(2).doubleValue()
        - fieldCoordinates.getAsNumber(0).doubleValue();
  }

  protected String truncateTextIfNeeded(String text, String fieldId)
    ...
    if (text != null && !text.isEmpty()) {
      ...
      float symbolWidth =
          font.getCalculatedBaseFont(true).getWidthPoint(" ... ", font.getCalculatedSize());
      while (textWidth > fieldWidth) {
        rightIndex--;
        textWidth =
            symbolWidth
                + font.getCalculatedBaseFont(true)
                    .getWidthPoint(text.substring(0, rightIndex), font.getCalculatedSize());
      }
  ...
  protected void addIntygStateWatermark(
