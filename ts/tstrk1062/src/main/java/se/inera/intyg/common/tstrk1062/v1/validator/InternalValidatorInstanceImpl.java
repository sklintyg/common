...
public class InternalValidatorInstanceImpl implements InternalDraftValidator<TsTrk1062UtlatandeV1> {
  ...
  public ValidateDraftResponse validateDraft(TsTrk1062UtlatandeV1 utlatande) {
    ...
    // Kategori 3 - Allmänt
    validateDiagnosRegistrering(
        utlatande.getDiagnosRegistrering(),
  ...
  private void validateIntygAvser(
      final IntygAvser intygAvser, List<ValidationMessage> validationMessages) {
    if (isNull(intygAvser)) {
      addValidationError(
          validationMessages,
          INTYG_AVSER_CATEGORY,
          INTYG_AVSER_SVAR_JSON_ID,
  ...
  private void validateIdKontroll(
      IdKontroll idKontroll, List<ValidationMessage> validationMessages) {
  ...
  private void validateDiagnosRegistrering(
      DiagnosRegistrering diagnosRegistrering,
    ...
    } else if (diagnosRegistrering.getTyp()
        == DiagnosRegistrering.DiagnosRegistreringsTyp.DIAGNOS_FRITEXT) {
      validateDiagnosFritext(diagnosFritext, validationMessages);
    }
  }

  private void validateDiagnosFritext(
      DiagnosFritext diagnosFritext, List<ValidationMessage> validationMessages) {
    if (isNull(diagnosFritext)) {
      addValidationError(
          validationMessages,
          ALLMANT_KATEGORI,
          ALLMANT_DIAGNOSKOD_FRITEXT_SVAR_JSON_ID
              + PUNKT
              + ALLMANT_DIAGNOSKOD_FRITEXT_FRITEXT_DELSVAR_JSON_ID,
          ValidationMessageType.EMPTY);
      return;
    }

    if (isNull(diagnosFritext.getDiagnosFritext())
        || diagnosFritext.getDiagnosFritext().isEmpty()) {
      addValidationError(
          validationMessages,
          ALLMANT_KATEGORI,
          ALLMANT_DIAGNOSKOD_FRITEXT_SVAR_JSON_ID
    ...
    if (isNull(diagnosFritext.getDiagnosArtal()) || diagnosFritext.getDiagnosArtal().isEmpty()) {
      addValidationError(
          ...
          ALLMANT_DIAGNOSKOD_FRITEXT_SVAR_JSON_ID
              + PUNKT
              + ALLMANT_DIAGNOSKOD_FRITEXT_ARTAL_DELSVAR_JSON_ID,
          ValidationMessageType.EMPTY,
  ...
  private void validateDiagnosKodad(
      ImmutableList<DiagnosKodad> diagnosKodad, List<ValidationMessage> validationMessages) {
    int diagnosNr = 0;
    boolean ignoreDiagnoseOne = false;

    if (!diagnosKodad.isEmpty()
        && !validateFirstDiagnoseIsPresent(diagnosKodad)
        && diagnosKodad.size() > 1) {
  ...
  private void validateLakemedelsbehandling(
      Lakemedelsbehandling lakemedelsbehandling, List<ValidationMessage> validationMessages) {
    ...
    if (lakemedelsbehandling.getHarHaft()) {
      ...
      if (lakemedelsbehandling.getPagar()) {
        if (isNull(lakemedelsbehandling.getAktuell())
            || lakemedelsbehandling.getAktuell().isEmpty()) {
          addValidationError(
              validationMessages,
              LAKEMEDELSBEHANDLING_KATEGORI,
        ...
        if (isNull(lakemedelsbehandling.getFoljsamhet())) {
          addValidationError(
              validationMessages,
              LAKEMEDELSBEHANDLING_KATEGORI,
              LAKEMEDELSBEHANDLING_JSON_ID
                  + PUNKT
                  + LAKEMEDELSBEHANDLING_FOLJSAMHET_DELSVAR_JSON_ID,
              ValidationMessageType.EMPTY);
        }
      } else {
        if (isNull(lakemedelsbehandling.getAvslutadTidpunkt())
            || lakemedelsbehandling.getAvslutadTidpunkt().isEmpty()) {
          addValidationError(
              validationMessages,
              LAKEMEDELSBEHANDLING_KATEGORI,
        ...
        if (isNull(lakemedelsbehandling.getAvslutadOrsak())
            || lakemedelsbehandling.getAvslutadOrsak().isEmpty()) {
          addValidationError(
              validationMessages,
              LAKEMEDELSBEHANDLING_KATEGORI,
              LAKEMEDELSBEHANDLING_JSON_ID
                  + PUNKT
                  + LAKEMEDELSBEHANDLING_AVSLUTAD_ORSAK_DELSVAR_JSON_ID,
              ValidationMessageType.EMPTY);
        }
      }
    }
  ...
  private void validateSymptom(
      String bedomningAvSymptom, List<ValidationMessage> validationMessages) {
  ...
  private void validatePrognos(
      PrognosTillstand prognosTillstand, List<ValidationMessage> validationMessages) {
  ...
  private void validateBedomning(Bedomning bedomning, List<ValidationMessage> validationMessages) {
    if (isNull(bedomning)
        || isNull(bedomning.getUppfyllerBehorighetskrav())
        || bedomning.getUppfyllerBehorighetskrav().isEmpty()) {
      addValidationError(
          validationMessages,
          BEDOMNING_KATEGORI,
      ...
      final Set<Bedomning.BehorighetsTyp> behorighetsTypSet =
          bedomning.getUppfyllerBehorighetskrav();
