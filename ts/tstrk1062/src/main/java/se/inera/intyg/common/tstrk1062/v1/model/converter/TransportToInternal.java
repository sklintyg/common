...
public final class TransportToInternal {
  ...
  private static void setSvar(TsTrk1062UtlatandeV1.Builder utlatande, Intyg source)
      throws ConverterException {
    final EnumSet<IntygAvser.BehorighetsTyp> intygAvserList =
        EnumSet.noneOf(IntygAvser.BehorighetsTyp.class);
    ...
    final Set<Bedomning.BehorighetsTyp> bedomningUppfyllerBehorighetskrav =
        EnumSet.noneOf(Bedomning.BehorighetsTyp.class);
    ...
    if (diagnosKodadList.size() > 0) {
      utlatande.setDiagnosRegistrering(
          DiagnosRegistrering.create(DiagnosRegistrering.DiagnosRegistreringsTyp.DIAGNOS_KODAD));
      utlatande.setDiagnosKodad(diagnosKodadList);
    } else if (diagnosFritext != null) {
      utlatande.setDiagnosRegistrering(
          DiagnosRegistrering.create(DiagnosRegistrering.DiagnosRegistreringsTyp.DIAGNOS_FRITEXT));
  ...
  private static IntygAvser.BehorighetsTyp getIntygAvser(List<Delsvar> delsvarList)
      throws ConverterException {
  ...
  private static DiagnosKodad getDiagnosKodad(List<Delsvar> delsvarList) throws ConverterException {
    ...
    if (diagnosCVValue != null) {
      ...
      diagnosKodSystem =
          Diagnoskodverk.getEnumByCodeSystem(diagnosCVValue.getCodeSystem()).toString();
      diagnosDisplayName = diagnosCVValue.getDisplayName();
    }

    final String diagnosBeskrivning =
        getStringValue(delsvarList, ALLMANT_DIAGNOSKOD_KODAD_KOD_TEXT_DELSVAR_ID);

    String diagnosArtal = null;
    final InternalDate partialDateArtal =
        getPartialDateValue(delsvarList, ALLMANT_DIAGNOSKOD_KODAD_KOD_ARTAL_DELSVAR_ID);
  ...
  private static DiagnosFritext getDiagnosFritext(List<Delsvar> delsvarList)
      throws ConverterException {
    String diagnosText = getStringValue(delsvarList, ALLMANT_DIAGNOSKOD_FRITEXT_FRITEXT_DELSVAR_ID);

    String diagnosArtal = null;
    final InternalDate partialDateArtal =
        getPartialDateValue(delsvarList, ALLMANT_DIAGNOSKOD_FRITEXT_ARTAL_DELSVAR_ID);
    if (partialDateArtal != null) {
      diagnosArtal = partialDateArtal.toString();
    }
  ...
  private static PrognosTillstand getPrognosTillstand(List<Delsvar> delsvarList)
      throws ConverterException {
    ...
    for (Delsvar delsvar : delsvarList) {
      if (delsvar.getId().equalsIgnoreCase(SYMPTOM_PROGNOS_DELSVAR_ID)) {
        if (!isStringContent(delsvar)) {
          if (PrognosTillstandTyp.KANEJBEDOMA
              .getCode()
              .equals(getCVSvarContent(delsvar).getCode())) {
  ...
  private static Bedomning.BehorighetsTyp getBehorighetsTyp(List<Delsvar> delsvarList)
      throws ConverterException {
  ...
  private static InternalDate getPartialDateValue(List<Delsvar> delsvarList, String delsvarId)
      throws ConverterException {
  ...
  private static CVType getCVValue(List<Delsvar> delsvarList, String delsvarId)
      throws ConverterException {
