...
import static se.inera.intyg.common.ts_bas.v6.codes.RespConstantsV6.OTILLRACKLIG_RORELSEFORMAGA_SVAR_TEXT_ID;
...
class QuestionOtillrackligRorelseFormogaTest {
  ...
  class IncludeValueViewText extends ValueViewTextTest<Funktionsnedsattning> {

    @Override
    protected CertificateDataElement getElement(Funktionsnedsattning expectedValue) {
      return QuestionOtillrackligRorelseFormoga.toCertificate(expectedValue, 0, textProvider);
    }

    @Override
    protected List<InputExpectedValuePair<Funktionsnedsattning, CertificateDataValueViewText>>
        inputExpectedValuePairList() {
      return List.of(
          new InputExpectedValuePair<>(
              Funktionsnedsattning.builder().build(),
              CertificateDataValueViewText.builder().text("Ej angivet").build()),
          new InputExpectedValuePair<>(
              Funktionsnedsattning.builder().setOtillrackligRorelseformaga(true).build(),
              CertificateDataValueViewText.builder().text("Ja").build()),
          new InputExpectedValuePair<>(
              Funktionsnedsattning.builder().setOtillrackligRorelseformaga(false).build(),
