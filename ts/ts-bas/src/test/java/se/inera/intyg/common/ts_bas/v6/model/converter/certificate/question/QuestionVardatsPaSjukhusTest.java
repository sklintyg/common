...
import static se.inera.intyg.common.ts_bas.v6.codes.RespConstantsV6.FOREKOMST_VARD_SJUKHUS_KONTAKT_LAKARE_DELSVAR_TEXT_ID;
...
class QuestionVardatsPaSjukhusTest {
  ...
  class IncludeValueViewText extends ValueViewTextTest<Sjukhusvard> {

    @Override
    protected CertificateDataElement getElement(Sjukhusvard expectedValue) {
      return QuestionVardatsPaSjukhus.toCertificate(expectedValue, 0, textProvider);
    }

    @Override
    protected List<InputExpectedValuePair<Sjukhusvard, CertificateDataValueViewText>>
        inputExpectedValuePairList() {
      return List.of(
          new InputExpectedValuePair<>(
              null, CertificateDataValueViewText.builder().text("Ej angivet").build()),
          new InputExpectedValuePair<>(
              Sjukhusvard.builder().setSjukhusEllerLakarkontakt(true).build(),
              CertificateDataValueViewText.builder().text("Ja").build()),
          new InputExpectedValuePair<>(
              Sjukhusvard.builder().setSjukhusEllerLakarkontakt(false).build(),
