...
import static se.inera.intyg.common.ts_bas.v7.codes.RespConstantsV7.NYSTAGMUS_SVAR_TEXT_ID;
    ...
    class ToInternal {
        ...
        class IncludeInternalBooleanValueTest extends InternalBooleanValueTest {
            ...
            protected CertificateDataElement getElement(Boolean expectedValue) {
                final var syn = Syn.builder().setNystagmus(expectedValue).build();
                return QuestionNystagmus.toCertificate(syn, 0, textProvider);
            }

            @Override
            protected Boolean toInternalBooleanValue(Certificate certificate) {
                return QuestionNystagmus.toInternal(certificate);
            }
        }
    }
}
