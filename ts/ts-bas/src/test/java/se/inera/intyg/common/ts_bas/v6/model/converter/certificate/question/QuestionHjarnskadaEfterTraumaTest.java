...
class QuestionHjarnskadaEfterTraumaTest {
  ...
  class IncludeValueViewText extends ValueViewTextTest<HjartKarl> {
    ...
    protected CertificateDataElement getElement(HjartKarl expectedValue) {
      return QuestionHjarnskadaEfterTrauma.toCertificate(expectedValue, 0, textProvider);
    }

    @Override
    protected List<InputExpectedValuePair<HjartKarl, CertificateDataValueViewText>>
        inputExpectedValuePairList() {
      return List.of(
          new InputExpectedValuePair<>(
              HjartKarl.builder().build(),
              CertificateDataValueViewText.builder().text("Ej angivet").build()),
          new InputExpectedValuePair<>(
              HjartKarl.builder().setHjarnskadaEfterTrauma(true).build(),
              CertificateDataValueViewText.builder().text("Ja").build()),
          new InputExpectedValuePair<>(
              HjartKarl.builder().setHjarnskadaEfterTrauma(false).build(),
