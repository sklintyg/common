...
import static se.inera.intyg.common.ts_bas.v7.codes.RespConstantsV7.HAR_DIABETES_SVAR_TEXT_ID;
...
class QuestionHarDiabetesTest {
  ...
  class IncludeValueViewText extends ValueViewTextTest<Diabetes> {
    ...
    protected CertificateDataElement getElement(Diabetes expectedValue) {
      return QuestionHarDiabetes.toCertificate(expectedValue, 0, textProvider);
    }
    ...
    protected List<InputExpectedValuePair<Diabetes, CertificateDataValueViewText>>
      ...
      return List.of(
          ...
          new InputExpectedValuePair<>(
              Diabetes.builder().setHarDiabetes(true).build(),
              CertificateDataValueViewText.builder().text("Ja").build()),
          new InputExpectedValuePair<>(
              Diabetes.builder().setHarDiabetes(false).build(),
