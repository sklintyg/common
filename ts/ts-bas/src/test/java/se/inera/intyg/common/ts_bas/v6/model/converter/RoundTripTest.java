...
class RoundTripTest {
  ...
  void testRoundTripInternalFirst(String name, Scenario scenario) throws Exception {
    ...
    Diff diff =
        DiffBuilder.compare(Input.fromString(actual.toString()))
    ...
    TsBasUtlatandeV6 expectedInternal =
        objectMapper.readValue(
            getClass()
                .getResourceAsStream("/v6/scenarios/internal/roundtripjson/" + name + ".json"),
            TsBasUtlatandeV6.class);
  ...
  void testRoundTripTransportFirst(String name, Scenario scenario) throws Exception {
    CustomObjectMapper objectMapper = new CustomObjectMapper();
    TsBasUtlatandeV6 internal =
        TransportToInternal.convert(scenario.asRivtaV3TransportModel().getIntyg());
