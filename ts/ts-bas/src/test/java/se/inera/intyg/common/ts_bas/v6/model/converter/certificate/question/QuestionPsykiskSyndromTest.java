...
class QuestionPsykiskSyndromTest {
  ...
  class IncludeCommonElementTests extends CommonElementTest {
    ...
    protected CertificateDataElement getElement() {
      return QuestionPsykiskSyndrom.toCertificate(null, 0, textProvider);
    }
    ...
    protected String getParent() {
      return PSYKISK_UTVECKLINGSSTORNING_CATEGORY_ID;
    }
  ...
  class IncludeValueViewText extends ValueViewTextTest<Utvecklingsstorning> {

    @Override
    protected CertificateDataElement getElement(Utvecklingsstorning expectedValue) {
      return QuestionPsykiskSyndrom.toCertificate(expectedValue, 0, textProvider);
    }

    @Override
    protected List<InputExpectedValuePair<Utvecklingsstorning, CertificateDataValueViewText>>
        inputExpectedValuePairList() {
      return List.of(
          new InputExpectedValuePair<>(
              null, CertificateDataValueViewText.builder().text("Ej angivet").build()),
          new InputExpectedValuePair<>(
              Utvecklingsstorning.builder().setHarSyndrom(true).build(),
              CertificateDataValueViewText.builder().text("Ja").build()),
          new InputExpectedValuePair<>(
              Utvecklingsstorning.builder().setHarSyndrom(false).build(),
