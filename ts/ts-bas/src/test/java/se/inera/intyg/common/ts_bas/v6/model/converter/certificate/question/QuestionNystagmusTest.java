...
import static se.inera.intyg.common.ts_bas.v6.codes.RespConstantsV6.NYSTAGMUS_SVAR_TEXT_ID;
...
class QuestionNystagmusTest {
  ...
  class IncludeValueViewText extends ValueViewTextTest<Syn> {
    ...
    protected CertificateDataElement getElement(Syn expectedValue) {
      return QuestionNystagmus.toCertificate(expectedValue, 0, textProvider);
    }
    ...
    protected List<InputExpectedValuePair<Syn, CertificateDataValueViewText>>
      ...
      return List.of(
          ...
          new InputExpectedValuePair<>(
              Syn.builder().setNystagmus(false).build(),
              CertificateDataValueViewText.builder().text("Nej").build()),
          new InputExpectedValuePair<>(
              Syn.builder().setNystagmus(true).build(),
