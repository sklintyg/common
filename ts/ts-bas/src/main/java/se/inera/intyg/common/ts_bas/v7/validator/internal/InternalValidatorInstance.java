...
public class InternalValidatorInstance {
  ...
  /**
   * Validates an internal draft of an {@link TsBasUtlatandeV7} (this means the object being
  ...
  private void validateUtvecklingsstorning(Utvecklingsstorning utvecklingsstorning) {
    ...
    if (utvecklingsstorning.getHarSyndrom() == null) {
      ValidatorUtil.addValidationErrorWithQuestionId(
          validationMessages,
          CATEGORY_UTVECKLINGSSTORNING,
          "utvecklingsstorning.harSyndrom",
          ValidationMessageType.EMPTY,
    ...
    if (utvecklingsstorning.getPsykiskUtvecklingsstorning() == null) {
      ValidatorUtil.addValidationErrorWithQuestionId(
          validationMessages,
          CATEGORY_UTVECKLINGSSTORNING,
          "utvecklingsstorning.psykiskUtvecklingsstorning",
          ValidationMessageType.EMPTY,
          PSYKISK_UTVECKLINGSSTORNING_DELSVAR_ID);
    }
  }
  ...
  private void validatePsykiskt(Psykiskt psykiskt) {
    if (psykiskt == null) {
      ValidatorUtil.addValidationErrorWithQuestionId(
          ...
          PSYKISK_SJUKDOM_STORNING_DELSVAR_ID);
      return;
    }
  ...
  private void validateSomnVakenhet(SomnVakenhet somnVakenhet) {
    ...
    if (somnVakenhet.getTeckenSomnstorningar() == null) {
      ValidatorUtil.addValidationErrorWithQuestionId(
          validationMessages,
          CATEGORY_SOMN_VAKENHET,
          "somnVakenhet.teckenSomnstorningar",
          ValidationMessageType.EMPTY,
          TECKEN_SOMN_ELLER_VAKENHETSSTORNING_SVAR_ID);
    }
  }
  ...
  private void validateSjukhusvard(Sjukhusvard sjukhusvard) {
    ...
    if (sjukhusvard.getSjukhusEllerLakarkontakt() == null) {
      ValidatorUtil.addValidationErrorWithQuestionId(
          validationMessages,
          CATEGORY_SJUKHUSVARD,
          "sjukhusvard.sjukhusEllerLakarkontakt",
          ValidationMessageType.EMPTY,
          FOREKOMST_VARD_SJUKHUS_KONTAKT_LAKARE_DELSVAR_ID);
      return;
    }
    ...
    if (sjukhusvard.getSjukhusEllerLakarkontakt()) {
      ValidatorUtil.assertDescriptionNotEmptyWithQuestionId(
          validationMessages,
          sjukhusvard.getTidpunkt(),
          CATEGORY_SJUKHUSVARD,
          "sjukhusvard.tidpunkt",
          TIDPUNKT_VARD_SJUKHUS_KONTAKT_LAKARE_DELSVAR_ID);
      ValidatorUtil.assertDescriptionNotEmptyWithQuestionId(
          validationMessages,
          sjukhusvard.getVardinrattning(),
          CATEGORY_SJUKHUSVARD,
          "sjukhusvard.vardinrattning",
          PLATS_VARD_SJUKHUS_KONTAKT_LAKARE_DELSVAR_ID);
      ValidatorUtil.assertDescriptionNotEmptyWithQuestionId(
          validationMessages,
          sjukhusvard.getAnledning(),
          CATEGORY_SJUKHUSVARD,
  ...
  private void validateBedomning(final Bedomning bedomning) {
    ...
    if (bedomning == null) {
      ValidatorUtil.addValidationErrorWithQuestionId(
          ...
          LAMPLIGHET_INNEHA_BEHORIGHET_SVAR_ID);
      return;
    }
  ...
  private void validateDiabetes(final Diabetes diabetes) {
    ...
    if (diabetes.getHarDiabetes()) {
      ...
      if (diabetes.getDiabetesTyp() == null) {
        ...
        if (isNullOrFalse(diabetes.getInsulin())
            && isNullOrFalse(diabetes.getKost())
            && isNullOrFalse(diabetes.getTabletter())) {
          ValidatorUtil.addValidationErrorWithQuestionId(
              validationMessages,
              CATEGORY_DIABETES,
              "diabetes.behandlingsTyp",
              ValidationMessageType.EMPTY,
              BEHANDLING_DIABETES_SVAR_ID);
        }
      }
    }
  }
  ...
  private void validateFunktionsnedsattning(final Funktionsnedsattning funktionsnedsattning) {
    ...
    if (funktionsnedsattning.getFunktionsnedsattning() == null) {
      ...
      ValidatorUtil.assertDescriptionNotEmptyWithQuestionId(
          validationMessages,
          funktionsnedsattning.getBeskrivning(),
          CATEGORY_FUNKTIONSNEDSATTNING,
  ...
  private void validateHjartKarl(final HjartKarl hjartKarl) {
    ...
    if (hjartKarl == null) {
      ValidatorUtil.addValidationErrorWithQuestionId(
          ...
          HJART_ELLER_KARLSJUKDOM_SVAR_ID);
      return;
    }
    ...
    if (hjartKarl.getHjarnskadaEfterTrauma() == null) {
      ValidatorUtil.addValidationErrorWithQuestionId(
          validationMessages,
          CATEGORY_HJART_KARL,
          "hjartKarl.hjarnskadaEfterTrauma",
          ValidationMessageType.EMPTY,
          TECKEN_PA_HJARNSKADA_SVAR_ID);
    }
    ...
    if (hjartKarl.getRiskfaktorerStroke() == null) {
      ValidatorUtil.addValidationErrorWithQuestionId(
          validationMessages,
          CATEGORY_HJART_KARL,
          "hjartKarl.riskfaktorerStroke",
          ValidationMessageType.EMPTY,
          RISKFAKTORER_STROKE_SVAR_ID);
      ...
      ValidatorUtil.assertDescriptionNotEmptyWithQuestionId(
          ...
          CATEGORY_HJART_KARL,
          "hjartKarl.beskrivningRiskfaktorer",
  ...
  private void validateHorselBalans(final HorselBalans horselBalans) {
    ...
    if (horselBalans == null) {
      ValidatorUtil.addValidationErrorWithQuestionId(
          validationMessages,
          CATEGORY_HORSEL_BALANS,
          "horselBalans",
          ValidationMessageType.EMPTY,
          "ts-bas.validation.horselBalans.missing",
          BALANSRUBBNINGAR_YRSEL_SVAR_ID);
      return;
    }
  ...
  private void validateKognitivt(final Kognitivt kognitivt) {
    ...
    if (kognitivt == null) {
      ValidatorUtil.addValidationErrorWithQuestionId(
          ...
          TECKEN_SVIKTANDE_KOGNITIV_FUNKTION_SVAR_ID);
      return;
    }
    ...
    if (kognitivt.getSviktandeKognitivFunktion() == null) {
      ValidatorUtil.addValidationErrorWithQuestionId(
          validationMessages,
          CATEGORY_KOGNITIVT,
          "kognitivt.sviktandeKognitivFunktion",
          ValidationMessageType.EMPTY,
          TECKEN_SVIKTANDE_KOGNITIV_FUNKTION_SVAR_ID);
    }
  }
  ...
  private void validateMedicinering(final Medicinering medicinering) {
    ...
    if (medicinering.getStadigvarandeMedicinering() == null) {
      ...
      ValidatorUtil.assertDescriptionNotEmptyWithQuestionId(
          validationMessages,
          medicinering.getBeskrivning(),
          CATEGORY_MEDICINERING,
          "medicinering.beskrivning",
  ...
  private void validateNarkotikaLakemedel(final NarkotikaLakemedel narkotikaLakemedel) {
    ...
    if (narkotikaLakemedel.getForemalForVardinsats() == null) {
      ValidatorUtil.addValidationErrorWithQuestionId(
          ...
          ValidationMessageType.EMPTY,
          VARDINSATSER_MISSBRUK_BEROENDE_DELSVAR_ID);
    }

    if ((narkotikaLakemedel.getTeckenMissbruk() != null && narkotikaLakemedel.getTeckenMissbruk())
        || (narkotikaLakemedel.getForemalForVardinsats() != null
            && narkotikaLakemedel.getForemalForVardinsats())) {
    ...
    if (narkotikaLakemedel.getLakarordineratLakemedelsbruk() == null) {
      ValidatorUtil.addValidationErrorWithQuestionId(
          ...
          ValidationMessageType.EMPTY,
          REGELBUNDET_LAKARORDINERAT_BRUK_LAKEMEDEL_DELSVAR_ID);

    } else if (narkotikaLakemedel.getLakarordineratLakemedelsbruk()) {
      ValidatorUtil.assertDescriptionNotEmptyWithQuestionId(
          validationMessages,
          narkotikaLakemedel.getLakemedelOchDos(),
          CATEGORY_NARKOTIKA_LAKEMEDEL,
          "narkotikaLakemedel.lakemedelOchDos",
  ...
  private void validateMedvetandestorning(final Medvetandestorning medvetandestorning) {
    ...
    if (medvetandestorning == null) {
      ValidatorUtil.addValidationErrorWithQuestionId(
          validationMessages,
          CATEGORY_MEDVETANDESTORNING,
          "medvetandestorning",
          ValidationMessageType.EMPTY,
          "ts-bas.validation.medvetandestorning.missing",
          MEDVETANDESTORNING_SVAR_ID);
      return;
    }
  ...
  private void validateSyn(final TsBasUtlatandeV7 utlatande) {
    ...
    if (syn.getNattblindhet() == null) {
      ValidatorUtil.addValidationErrorWithQuestionId(
          ...
          "syn.nattblindhet",
          ValidationMessageType.EMPTY,
          SEENDE_NEDSATT_BELYSNING_SVAR_ID);
    }
    ...
    if (syn.getProgressivOgonsjukdom() == null) {
      ValidatorUtil.addValidationErrorWithQuestionId(
          validationMessages,
          CATEGORY_SYN,
          "syn.progressivOgonsjukdom",
          ValidationMessageType.EMPTY,
          PROGRESSIV_OGONSJUKDOM_SVAR_ID);
    }
    ...
    if (syn.getSynskarpaSkickasSeparat() == null || !syn.getSynskarpaSkickasSeparat()) { // R37
      if (syn.getHogerOga() == null) {
        ValidatorUtil.addValidationErrorWithQuestionId(
            ...
            VARDEN_FOR_SYNSKARPA_ID);
      } else {
        if (syn.getHogerOga().getUtanKorrektion() == null) {
          ValidatorUtil.addValidationErrorWithQuestionId(
              validationMessages,
              CATEGORY_SYN,
              "syn.hogerOga.utanKorrektion",
              ValidationMessageType.EMPTY,
              VARDEN_FOR_SYNSKARPA_ID);

        } else if (syn.getHogerOga().getUtanKorrektion() < 0.0
            || syn.getHogerOga().getUtanKorrektion() > 2.0) {
          ValidatorUtil.addValidationErrorWithQuestionId(
              validationMessages,
              CATEGORY_SYN,
              "syn.hogerOga.utanKorrektion",
        ...
        if (syn.getHogerOga().getMedKorrektion() != null) {
          if (syn.getHogerOga().getMedKorrektion() < 0.0
              || syn.getHogerOga().getMedKorrektion() > 2.0) {
            ValidatorUtil.addValidationErrorWithQuestionId(
                validationMessages,
                CATEGORY_SYN,
      ...
      if (syn.getVansterOga() == null) {
        ...
        } else if (syn.getVansterOga().getUtanKorrektion() < 0.0
            || syn.getVansterOga().getUtanKorrektion() > 2.0) {
          ValidatorUtil.addValidationErrorWithQuestionId(
              validationMessages,
              CATEGORY_SYN,
              "syn.vansterOga.utanKorrektion",
        ...
        if (syn.getVansterOga().getMedKorrektion() != null) {
          if (syn.getVansterOga().getMedKorrektion() < 0.0
              || syn.getVansterOga().getMedKorrektion() > 2.0) {
            ValidatorUtil.addValidationErrorWithQuestionId(
                validationMessages,
                CATEGORY_SYN,
      ...
      if (syn.getBinokulart() == null) {
        ...
        } else if (syn.getBinokulart().getUtanKorrektion() < 0.0
            || syn.getBinokulart().getUtanKorrektion() > 2.0) {
          ValidatorUtil.addValidationErrorWithQuestionId(
              validationMessages,
              CATEGORY_SYN,
              "syn.binokulart.utanKorrektion",
        ...
        if (syn.getBinokulart().getMedKorrektion() != null) {
          if (syn.getBinokulart().getMedKorrektion() < 0.0
              || syn.getBinokulart().getMedKorrektion() > 2.0) {
            ValidatorUtil.addValidationErrorWithQuestionId(
                validationMessages,
                CATEGORY_SYN,
      ...
      if (syn.getVansterOga() != null) {
        if (syn.getVansterOga().getUtanKorrektion() != null
            ...
            || (syn.getVansterOga().getKontaktlins() != null
                && syn.getVansterOga().getKontaktlins())) {
          ValidatorUtil.addValidationErrorWithQuestionId(
              validationMessages,
              CATEGORY_SYN,
              "syn.vansterOga",
  ...
  private void validateSynKorrektionsRegler(TsBasUtlatandeV7 utlatande, Syn syn) {
    ...
    if (syn.getBinokulart() != null
        && syn.getHogerOga() != null
        && syn.getVansterOga() != null
        && utlatande.getIntygAvser() != null
        && utlatande.getIntygAvser().getKorkortstyp() != null) {
      // R33
      if (utlatande.getIntygAvser().getKorkortstyp().contains(IntygAvserKategori.IAV10)
        ...
        // R34
              ...
              IntygAvserKategori.getNormalCategories(), utlatande.getIntygAvser().getKorkortstyp())
          && (syn.getHogerOga().getUtanKorrektion() != null
              && syn.getVansterOga().getUtanKorrektion() != null)
          && (syn.getHogerOga().getUtanKorrektion() < 0.8
              && syn.getVansterOga().getUtanKorrektion() < 0.8)
          && (syn.getHogerOga().getUtanKorrektion() >= 0.1
              && syn.getVansterOga().getUtanKorrektion() >= 0.1)) {
        if (syn.getHogerOga().getMedKorrektion() == null) {
          ValidatorUtil.addValidationErrorWithQuestionId(
              validationMessages,
              CATEGORY_SYN,
