...
public final class TransportToInternal {
  ...
  private static void setSvar(TsBasUtlatandeV7.Builder utlatande, Intyg source)
  ...
  private static void handleIntygAvser(
      TsBasUtlatandeV7.Builder utlatande, Svar svar, EnumSet<IntygAvserKategori> intygAvserSet)
      throws ConverterException {
    for (Delsvar delsvar : svar.getDelsvar()) {
      switch (delsvar.getId()) {
        case INTYG_AVSER_DELSVAR_ID_1:
          intygAvserSet.add(
              IntygAvserKategori.valueOf(
                  IntygAvserKod.fromCode(getCVSvarContent(delsvar).getCode()).getCode()));
          break;
        default:
  ...
  private static void handleIdentitetStyrktGenom(TsBasUtlatandeV7.Builder utlatande, Svar svar)
      throws ConverterException {
    for (Delsvar delsvar : svar.getDelsvar()) {
      switch (delsvar.getId()) {
        case IDENTITET_STYRKT_GENOM_ID_2:
          utlatande.setVardkontakt(
              Vardkontakt.create(
                  VARDKONTAKT_TYP,
  ...
  private static void handleSjukdomFunktionsnedsattning(
      Funktionsnedsattning.Builder funktionsnedsattning, Svar svar) {
    for (Delsvar delsvar : svar.getDelsvar()) {
      switch (delsvar.getId()) {
  ...
  private static void handleOtillrackligRorelseformaga(
      Funktionsnedsattning.Builder funktionsnedsattning, Svar svar) {
    for (Delsvar delsvar : svar.getDelsvar()) {
      switch (delsvar.getId()) {
  ...
  private static void handleTypAvDiabetes(Diabetes.Builder diabetes, Svar svar)
      throws ConverterException {
    for (Delsvar delsvar : svar.getDelsvar()) {
      switch (delsvar.getId()) {
  ...
  private static void handleTeckenNeurologiskSjukdom(
      TsBasUtlatandeV7.Builder utlatande, Svar svar) {
    for (Delsvar delsvar : svar.getDelsvar()) {
      switch (delsvar.getId()) {
  ...
  private static void handleMedvetandestorning(TsBasUtlatandeV7.Builder utlatande, Svar svar) {
  ...
  private static void handleNedsattNjurfunktion(TsBasUtlatandeV7.Builder utlatande, Svar svar) {
    for (Delsvar delsvar : svar.getDelsvar()) {
      switch (delsvar.getId()) {
  ...
  private static void handleTeckenSviktandeKognitivFunktion(
      TsBasUtlatandeV7.Builder utlatande, Svar svar) {
    for (Delsvar delsvar : svar.getDelsvar()) {
      switch (delsvar.getId()) {
  ...
  private static void handleTeckenSomnEllerVakenhetsstorning(
      TsBasUtlatandeV7.Builder utlatande, Svar svar) {
    for (Delsvar delsvar : svar.getDelsvar()) {
      switch (delsvar.getId()) {
  ...
  private static void handleRegelbundetLakarordineratBrukLakemedel(
      NarkotikaLakemedel.Builder narkotika, Svar svar) {
    for (Delsvar delsvar : svar.getDelsvar()) {
      switch (delsvar.getId()) {
  ...
  private static void handlePsykiskSjukdomStorning(TsBasUtlatandeV7.Builder utlatande, Svar svar) {
    for (Delsvar delsvar : svar.getDelsvar()) {
      switch (delsvar.getId()) {
  ...
  private static void handlePsykiskUtvecklingsstorning(
      Utvecklingsstorning.Builder utvecklingsstorning, Svar svar) {
    for (Delsvar delsvar : svar.getDelsvar()) {
      switch (delsvar.getId()) {
  ...
  private static void handleAdhdAddDampAspergersTourettes(
      Utvecklingsstorning.Builder utvecklingsstorning, Svar svar) {
    for (Delsvar delsvar : svar.getDelsvar()) {
      switch (delsvar.getId()) {
  ...
  private static void handleVardSjukhusKontaktLakare(
      TsBasUtlatandeV7.Builder utlatande, Svar svar) {
  ...
  private static void handleStadigvarandeMedicinering(
      TsBasUtlatandeV7.Builder utlatande, Svar svar) {
  ...
  private static void handleOvrigaKommentarer(TsBasUtlatandeV7.Builder utlatande, Svar svar) {
    for (Delsvar delsvar : svar.getDelsvar()) {
      switch (delsvar.getId()) {
  ...
  private static void handleUppfyllerKravForBehorighet(
      Bedomning.Builder bedomning, Svar svar, EnumSet<BedomningKorkortstyp> bedomningsSet)
      throws ConverterException {
    for (Delsvar delsvar : svar.getDelsvar()) {
      switch (delsvar.getId()) {
        case UPPFYLLER_KRAV_FOR_BEHORIGHET_DELSVAR_ID_33:
          ...
          BedomningKorkortstyp bedomningKorkortstyp =
              BedomningKorkortstyp.valueOf(korkortsbehorighetKod.getCode());
  ...
  private static void handleBorUndersokasAvSpecialistlakare(
      Bedomning.Builder bedomning, Svar svar) {
    for (Delsvar delsvar : svar.getDelsvar()) {
      switch (delsvar.getId()) {
