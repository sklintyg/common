...
public class InternalValidatorInstance {
  ...
  /**
   * Validates an internal draft of an {@link TsBasUtlatandeV6} (this means the object being
   * validated is not necessarily complete).
   *
   * @param utlatande an internal {@link TsBasUtlatandeV6}
   * @return a ValidateDraftResponseHolder with a status and a list of validationErrors
   */
  public ValidateDraftResponse validate(TsBasUtlatandeV6 utlatande) {
  ...
  private void validateIdentitetStyrkt(Vardkontakt vardkontakt) {
    if (vardkontakt == null) {
      ValidatorUtil.addValidationError(
          validationMessages, CATEGORY_IDENTITET, "vardkontakt", ValidationMessageType.EMPTY);
      return;
    }
  ...
  private void validateSjukhusvard(Sjukhusvard sjukhusvard) {
    ...
    if (sjukhusvard.getSjukhusEllerLakarkontakt()) {
      ValidatorUtil.assertDescriptionNotEmpty(
          validationMessages,
  ...
  private void validateSyn(final TsBasUtlatandeV6 utlatande) {
    ...
    if (syn.getSynfaltsdefekter() == null) {
      ValidatorUtil.addValidationError(
          validationMessages, CATEGORY_SYN, "syn.synfaltsdefekter", ValidationMessageType.EMPTY);
    }

    if (syn.getNattblindhet() == null) {
      ValidatorUtil.addValidationError(
          validationMessages, CATEGORY_SYN, "syn.nattblindhet", ValidationMessageType.EMPTY);
    }
    ...
    if (syn.getDiplopi() == null) {
      ValidatorUtil.addValidationError(
          validationMessages, CATEGORY_SYN, "syn.diplopi", ValidationMessageType.EMPTY);
    }

    if (syn.getNystagmus() == null) {
      ValidatorUtil.addValidationError(
          validationMessages, CATEGORY_SYN, "syn.nystagmus", ValidationMessageType.EMPTY);
    }
