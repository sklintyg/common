...
public final class TransportToInternal {
  ...
  public static TsBasUtlatandeV6 convert(Intyg source) throws ConverterException {
    ...
    utlatande.setGrundData(TransportConverterUtil.getGrundData(source, PatientInfo.EXTENDED));
  ...
  private static void setSvar(TsBasUtlatandeV6.Builder utlatande, Intyg source)
  ...
  private static void handleIntygAvser(
      TsBasUtlatandeV6.Builder utlatande, Svar svar, EnumSet<IntygAvserKategori> intygAvserSet)
      throws ConverterException {
        ...
        case INTYG_AVSER_DELSVAR_ID_1:
          intygAvserSet.add(
              IntygAvserKategori.valueOf(
                  IntygAvserKod.fromCode(getCVSvarContent(delsvar).getCode()).name()));
          break;
        default:
  ...
  private static void handleIdentitetStyrktGenom(TsBasUtlatandeV6.Builder utlatande, Svar svar)
      throws ConverterException {
    for (Delsvar delsvar : svar.getDelsvar()) {
      switch (delsvar.getId()) {
  ...
  private static void handleTeckenNeurologiskSjukdom(
      TsBasUtlatandeV6.Builder utlatande, Svar svar) {
    for (Delsvar delsvar : svar.getDelsvar()) {
      switch (delsvar.getId()) {
      ...
      switch (delsvar.getId()) {
        case UPPFYLLER_KRAV_FOR_BEHORIGHET_DELSVAR_ID_33:
          ...
          BedomningKorkortstyp bedomningKorkortstyp =
              BedomningKorkortstyp.valueOf(korkortsbehorighetKod.name());
