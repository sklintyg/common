...
import static se.inera.intyg.common.ts_bas.v7.codes.RespConstantsV7.OVRIGA_KOMMENTARER_CATEGORY_TEXT_ID;
...
public class CategoryOvrigt {
    public static CertificateDataElement toCertificate(int index, CertificateTextProvider texts) {
        return CertificateDataElement.builder()
            .id(OVRIGA_KOMMENTARER_CATEGORY_ID)
            .index(index)
            .config(
                CertificateDataConfigCategory.builder()
                    .text(texts.get(OVRIGA_KOMMENTARER_CATEGORY_TEXT_ID))
                    .build()
            )
            .build();
    }
