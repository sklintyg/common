...
import se.inera.intyg.common.ts_diabetes_2.model.converter.InternalToTransport;
import se.inera.intyg.common.ts_diabetes_2.model.converter.TransportToInternal;
import se.inera.intyg.common.ts_diabetes_2.model.converter.UtlatandeToIntyg;
import se.inera.intyg.common.ts_diabetes_2.model.internal.TsDiabetes2Utlatande;
import se.inera.intyg.common.ts_diabetes_2.pdf.PdfGenerator;
...
public class TsDiabetes2ModuleApi implements ModuleApi {
    ...
    @Autowired
    protected WebcertModelFactory<TsDiabetes2Utlatande> webcertModelFactory;

    @Autowired
    private InternalDraftValidator<TsDiabetes2Utlatande> internalDraftValidator;

    @Autowired
    private ObjectMapper objectMapper;

    @Autowired
    private ModelCompareUtil<TsDiabetes2Utlatande> modelCompareUtil;
    ...
    public PdfResponse pdf(String internalModel, List<Status> statuses, ApplicationOrigin applicationOrigin, UtkastStatus utkastStatus)
        ...
        TsDiabetes2Utlatande tsDIabetes2Intyg = getInternal(internalModel);
        IntygTexts texts = getTexts(TsDiabetes2EntryPoint.MODULE_ID, tsDIabetes2Intyg.getTextVersion());

        Personnummer personId = tsDIabetes2Intyg.getGrundData().getPatient().getPersonId();
        return new PdfGenerator().generatePdf(tsDIabetes2Intyg.getId(), internalModel, personId, texts, statuses, applicationOrigin,
                utkastStatus);
    }
    ...
    private String getSchematronFileName() {
        return TsDiabetes2EntryPoint.SCHEMATRON_FILE;
    }

    private RegisterCertificateType internalToTransport(TsDiabetes2Utlatande utlatande) throws ConverterException {
        return InternalToTransport.convert(utlatande);
    }

    private TsDiabetes2Utlatande transportToInternal(se.riv.clinicalprocess.healthcond.certificate.v3.Intyg intyg)
            throws ConverterException {
        return TransportToInternal.convert(intyg);
    }

    private se.riv.clinicalprocess.healthcond.certificate.v3.Intyg utlatandeToIntyg(TsDiabetes2Utlatande utlatande)
            throws ConverterException {
        return UtlatandeToIntyg.convert(utlatande);
    }

    private TsDiabetes2Utlatande decorateWithSignature(TsDiabetes2Utlatande utlatande, String base64EncodedSignatureXml) {
        return utlatande.toBuilder().setSignature(base64EncodedSignatureXml).build();
    }

    @Override
    public String getAdditionalInfo(se.riv.clinicalprocess.healthcond.certificate.v3.Intyg intyg) throws ModuleException {
        return null;
    }
    ...
    public String createRenewalFromTemplate(CreateDraftCopyHolder draftCopyHolder, Utlatande template)
        ...
        try {
            ...
            TsDiabetes2Utlatande internal = (TsDiabetes2Utlatande) template;

            // Null out applicable fields
            TsDiabetes2Utlatande renewCopy = internal.toBuilder()
                    .build();
    ...
    @Override
    public se.riv.clinicalprocess.healthcond.certificate.v3.Intyg getIntygFromUtlatande(Utlatande utlatande) throws ModuleException {
        try {
            return utlatandeToIntyg(TsDiabetes2Utlatande.class.cast(utlatande));
        } catch (Exception e) {
    ...
    protected TsDiabetes2Utlatande getInternal(String internalModel) throws ModuleException {
        try {
            return objectMapper.readValue(internalModel, TsDiabetes2Utlatande.class);
        } catch (IOException e) {
    ...
    private String toInternalModelResponse(TsDiabetes2Utlatande internalModel) throws ModuleException {
        try {
    ...
    private CertificateResponse convert(GetCertificateResponseType response) throws ModuleException {
        try {
            TsDiabetes2Utlatande utlatande = transportToInternal(response.getIntyg());
    ...
    private String updateInternal(String internalModel, HoSPersonal hosPerson, LocalDateTime signingDate)
            throws ModuleException {
        try {
            TsDiabetes2Utlatande utlatande = getInternal(internalModel);
    ...
    private String updateInternalAfterSigning(String internalModel, String base64EncodedSignatureXml)
            throws ModuleException {
        try {
            TsDiabetes2Utlatande utlatande = decorateWithSignature(getInternal(internalModel), base64EncodedSignatureXml);
            return toInternalModelResponse(utlatande);
        } catch (ModuleException e) {
