...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
  ...
  <xsl:template name="utlatandeHeader">
    ...
    <p:utlatande-id root="1.2.752.129.2.1.2.1">
      <xsl:attribute name="extension" select="ns2:intygs-id/ns3:extension"/>
    </p:utlatande-id>
    ...
    <xsl:if test="ns2:svar[@id='32']">
      <p:kommentar>
        <xsl:value-of select="ns2:svar[@id='32']/ns2:delsvar[@id='32.1']"/>
      </p:kommentar>
    </xsl:if>
  </xsl:template>
  ...
  <xsl:template name="grundData">
    ...
    <p:signeringsdatum>
      <xsl:value-of select="ns2:signeringstidpunkt"/>
    </p:signeringsdatum>
    <p:patient>
      <p:person-id root="1.2.752.129.2.1.3.1">
        <xsl:attribute name="extension">
          <xsl:value-of select="concat(substring(ns2:patient/ns2:person-id/ns3:extension, 0,9), '-', substring(ns2:patient/ns2:person-id/ns3:extension, 9,13))"/>
        </xsl:attribute>
      </p:person-id>
      <p:fornamn>
        <xsl:value-of select="ns2:patient/ns2:fornamn"/>
      </p:fornamn>
      <p:efternamn>
        <xsl:value-of select="ns2:patient/ns2:efternamn"/>
      </p:efternamn>
      <p:postadress>
        <xsl:value-of select="ns2:patient/ns2:postadress"/>
      </p:postadress>
      <p:postnummer>
        <xsl:value-of select="ns2:patient/ns2:postnummer"/>
      </p:postnummer>
      <p:postort>
        <xsl:value-of select="ns2:patient/ns2:postort"/>
      </p:postort>
    </p:patient>
    <p:skapadAv>
      <p:personal-id root="1.2.752.129.2.1.4.1">
        <xsl:attribute name="extension">
          <xsl:value-of select="ns2:skapadAv/ns2:personal-id/ns3:extension"/>
        </xsl:attribute>
      </p:personal-id>
      <p:fullstandigtNamn>
        <xsl:value-of select="ns2:skapadAv/ns2:fullstandigtNamn"/>
      </p:fullstandigtNamn>
      <xsl:for-each select="ns2:skapadAv/ns2:befattning">
        <p:befattning>
      ...
      <p:enhet>
        <p:enhets-id root="1.2.752.129.2.1.4.1">
          <xsl:attribute name="extension">
            <xsl:value-of select="ns2:skapadAv/ns2:enhet/ns2:enhets-id/ns3:extension"/>
          </xsl:attribute>
        </p:enhets-id>
        <p:enhetsnamn>
          <xsl:value-of select="ns2:skapadAv/ns2:enhet/ns2:enhetsnamn"/>
        </p:enhetsnamn>
        <p:postadress>
          <xsl:value-of select="ns2:skapadAv/ns2:enhet/ns2:postadress"/>
        </p:postadress>
        <p:postnummer>
          <xsl:value-of select="ns2:skapadAv/ns2:enhet/ns2:postnummer"/>
        </p:postnummer>
        <p:postort>
          <xsl:value-of select="ns2:skapadAv/ns2:enhet/ns2:postort"/>
        </p:postort>
        <p:telefonnummer>
          <xsl:value-of select="ns2:skapadAv/ns2:enhet/ns2:telefonnummer"/>
        </p:telefonnummer>
        <p:vardgivare>
          <p:vardgivare-id root="1.2.752.129.2.1.4.1">
            <xsl:attribute name="extension">
              <xsl:value-of select="ns2:skapadAv/ns2:enhet/ns2:vardgivare/ns2:vardgivare-id/ns3:extension"/>
            </xsl:attribute>
          </p:vardgivare-id>
          <p:vardgivarnamn>
            <xsl:value-of select="ns2:skapadAv/ns2:enhet/ns2:vardgivare/ns2:vardgivarnamn"/>
          </p:vardgivarnamn>
        </p:vardgivare>
      </p:enhet>
  ...
  <xsl:template name="vardKontakt">
    <p:vardkontakt>
      ...
      <p:idKontroll codeSystem="e7cc8f30-a353-4c42-b17a-a189b6876647" codeSystemName="kv_id_kontroll">
        <xsl:attribute name="code" select="ns2:svar[@id='2']/ns2:delsvar[@id='2.1']/ns3:cv/ns3:code"/>
      </p:idKontroll>
    </p:vardkontakt>
  </xsl:template>
  ...
  <xsl:template name="ogatsRorlighetObservation">
    <p:observation>
      ...
      <p:forekomst>
        <xsl:value-of select="ns2:svar[@id='6']/ns2:delsvar[@id='6.1']"/>
      </p:forekomst>
    </p:observation>
  </xsl:template>
  ...
  <xsl:template name="synfunktionObservation">
    ...
    <p:observation>
      ...
      <p:varde>
        <xsl:attribute name="value" select="ns2:svar[@id='8']/ns2:delsvar[@id='8.1']"/>
      </p:varde>
    ...
    <p:observation>
      <p:observationskod code="420050001" codeSystem="{$id_snomed-ct}" codeSystemName="SNOMED-CT"/>
      <p:varde>
        <xsl:attribute name="value" select="ns2:svar[@id='8']/ns2:delsvar[@id='8.2']"/>
      </p:varde>
      <xsl:element name="{$ns-prefix}:lateralitet" namespace="{$ns-namespace}">
    ...
    <p:observation>
      <p:observationskod code="420050001" codeSystem="{$id_snomed-ct}" codeSystemName="SNOMED-CT"/>
      <p:varde>
        <xsl:attribute name="value" select="ns2:svar[@id='8']/ns2:delsvar[@id='8.3']"/>
      </p:varde>
      <xsl:element name="{$ns-prefix}:lateralitet" namespace="{$ns-namespace}">
    ...
    <!-- Med korrektion -->
    <!-- Höger öga -->
    <xsl:if test="ns2:svar[@id='8']/ns2:delsvar[@id='8.4']">
      <p:observation>
        <p:observationskod code="397535007" codeSystem="{$id_snomed-ct}" codeSystemName="SNOMED-CT"/>
        <p:varde>
          <xsl:attribute name="value" select="ns2:svar[@id='8']/ns2:delsvar[@id='8.4']"/>
        </p:varde>
        <xsl:element name="{$ns-prefix}:lateralitet" namespace="{$ns-namespace}">
          <xsl:attribute name="code" select="'24028007'"/>
          <xsl:attribute name="codeSystem" select="$id_snomed-ct"/>
    ...
    <!-- Vänster öga -->
    <xsl:if test="ns2:svar[@id='8']/ns2:delsvar[@id='8.5']">
      <p:observation>
        <p:observationskod code="397535007" codeSystem="{$id_snomed-ct}" codeSystemName="SNOMED-CT"/>
        <p:varde>
          <xsl:attribute name="value" select="ns2:svar[@id='8']/ns2:delsvar[@id='8.5']"/>
        </p:varde>
        <xsl:element name="{$ns-prefix}:lateralitet" namespace="{$ns-namespace}">
          <xsl:attribute name="code" select="'7771000'"/>
          <xsl:attribute name="codeSystem" select="$id_snomed-ct"/>
    ...
    <xsl:if test="ns2:svar[@id='8']/ns2:delsvar[@id='8.4']">
      <p:observation>
        <p:observationskod code="397535007" codeSystem="{$id_snomed-ct}" codeSystemName="SNOMED-CT"/>
        <p:varde>
          <xsl:attribute name="value" select="ns2:svar[@id='8']/ns2:delsvar[@id='8.6']"/>
        </p:varde>
        <xsl:element name="{$ns-prefix}:lateralitet" namespace="{$ns-namespace}">
          <xsl:attribute name="code" select="'51440002'"/>
          <xsl:attribute name="codeSystem" select="$id_snomed-ct"/>
