...
class InternalToCertificateTest {
  ...
  void shallIncludeQuestionDiagnosAr() {
    final var actualCertificate = internalToCertificate.convert(internalCertificate, textProvider);
    assertEquals(
        5, actualCertificate.getData().get(ALLMANT_DIABETES_DIAGNOS_AR_SVAR_ID).getIndex());
  }
  ...
  void shallIncludeQuestionDiabetesBehandlingInsulinperiod() {
    final var actualCertificate = internalToCertificate.convert(internalCertificate, textProvider);
    assertEquals(
        9, actualCertificate.getData().get(ALLMANT_BEHANDLING_INSULIN_DELSVAR_ID).getIndex());
  }
  ...
  void shallIncludeQuestionDiabetesBehandlingAnnan() {
    final var actualCertificate = internalToCertificate.convert(internalCertificate, textProvider);
    assertEquals(
        10, actualCertificate.getData().get(ALLMANT_BEHANDLING_ANNAN_DELSVAR_ID).getIndex());
  }
  ...
  void shallIncludeQuestionEgenkontrollBlodsocker() {
    ...
    assertEquals(
        13,
        actualCertificate.getData().get(HYPOGLYKEMI_EGENKONTROLL_BLODSOCKER_SVAR_ID).getIndex());
  }
  ...
  void shallIncludeQuestionTeckenNedsattHjarnfunktion() {
    ...
    assertEquals(
        ...
        actualCertificate
            .getData()
            .get(HYPOGLYKEMI_TECKEN_NEDSATT_HJARNFUNKTION_SVAR_ID)
            .getIndex());
  }
  ...
  void shallIncludeQuestionHypoglykemiGodtagbarKontroll() {
    final var actualCertificate = internalToCertificate.convert(internalCertificate, textProvider);
    assertEquals(
        15, actualCertificate.getData().get(HYPOGLYKEMI_GODTAGBAR_KONTROLL_SVAR_ID).getIndex());
  }
  ...
  void shallIncludeQuestionFormagaKannaVarningstecken() {
    final var actualCertificate = internalToCertificate.convert(internalCertificate, textProvider);
    assertEquals(
        16, actualCertificate.getData().get(HYPOGLYKEMI_FORMOGA_VARNINGSTECKEN_SVAR_ID).getIndex());
  }
  ...
  void shallIncludeQuestionHypoglykemiAterkommandeSenasteAretTrafikTidpunkt() {
    ...
    assertEquals(
        ...
        actualCertificate
            .getData()
            .get(HYPOGLYKEMI_ATERKOMMANDE_SENASTE_ARET_TRAFIK_TIDPUNKT_DELSVAR_ID)
            .getIndex());
  }
  ...
  void shallIncludeQuestionSynMisstankeOgonsjukdomMessage() {
    ...
    assertEquals(
        25,
        actualCertificate.getData().get(SYN_VARDEN_MISSTANKE_OGONSJUKDOM_MESSAGE_ID).getIndex());
  }
  ...
  void shallIncludeQuestionSynSeparatOgonlakarintyg() {
    final var actualCertificate = internalToCertificate.convert(internalCertificate, textProvider);
    assertEquals(
        27, actualCertificate.getData().get(SYN_SEPARAT_OGONLAKARINTYG_SVAR_ID).getIndex());
  }
  ...
  void shallIncludeQuestionBedomningKorkortstyp() {
    ...
    assertEquals(
        32,
        actualCertificate.getData().get(BEDOMNING_UPPFYLLER_BEHORIGHETSKRAV_DELSVAR_ID).getIndex());
  }
  ...
  void shallIncludeQuestionBedomningLakareSpecialKompetens() {
    final var actualCertificate = internalToCertificate.convert(internalCertificate, textProvider);
    assertEquals(
        34, actualCertificate.getData().get(BEDOMNING_LAKARE_SPECIAL_KOMPETENS_SVAR_ID).getIndex());
  }
}
