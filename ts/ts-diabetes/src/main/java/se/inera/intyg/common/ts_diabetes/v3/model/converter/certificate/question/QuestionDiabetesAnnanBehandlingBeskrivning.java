...
public class QuestionDiabetesAnnanBehandlingBeskrivning {

    public static CertificateDataElement toCertificate(Allmant allmant, int index, CertificateTextProvider textProvider) {
        final var annanBehandlingBeskrivning =
            allmant != null && allmant.getBehandling() != null ? allmant.getBehandling().getAnnanBehandlingBeskrivning() : null;
        return CertificateDataElement.builder()
            .id(ALLMANT_BEHANDLING_ANNAN_DELSVAR_ID)
            .parent(ALLMANT_BEHANDLING_SVAR_ID)
            .index(index)
            .config(
                CertificateDataConfigViewText.builder()
                    .text(textProvider.get(ALLMANT_BEHANDLING_ANNAN_TEXT_ID))
                    .build()
            )
            .value(
                CertificateDataValueViewText.builder()
                    .text(stringValue(annanBehandlingBeskrivning))
                    .build()
            )
            .build();
    }
