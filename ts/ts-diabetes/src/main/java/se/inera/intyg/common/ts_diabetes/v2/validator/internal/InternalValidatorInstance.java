...
public class InternalValidatorInstance {
  ...
  /**
   * Validates an internal draft of an {@link TsDiabetesUtlatandeV2} (this means the object being
  ...
  private void validateHypoglykemi(Hypoglykemier hypoglykemier) {
    ...
    if (hypoglykemier.getTeckenNedsattHjarnfunktion() == null) {
      ValidatorUtil.addValidationError(
          validationMessages,
          CATEGORY_HYPOGLYKEMIER,
          "hypoglykemier.teckenNedsattHjarnfunktion",
          ValidationMessageType.EMPTY);
    }
    ...
    if (ValidatorUtil.isNotNullTrue(hypoglykemier.getTeckenNedsattHjarnfunktion())) {
      if (hypoglykemier.getSaknarFormagaKannaVarningstecken() == null) {
        ValidatorUtil.addValidationError(
            validationMessages,
            CATEGORY_HYPOGLYKEMIER,
            "hypoglykemier.saknarFormagaKannaVarningstecken",
            ValidationMessageType.EMPTY);
      }
      ...
      if (hypoglykemier.getAllvarligForekomst() == null) {
        ValidatorUtil.addValidationError(
            validationMessages,
            CATEGORY_HYPOGLYKEMIER,
            "hypoglykemier.allvarligForekomst",
            ValidationMessageType.EMPTY);
      }
      ...
      if (hypoglykemier.getAllvarligForekomstTrafiken() == null) {
        ValidatorUtil.addValidationError(
            validationMessages,
            CATEGORY_HYPOGLYKEMIER,
            "hypoglykemier.allvarligForekomstTrafiken",
            ValidationMessageType.EMPTY);
      }
    }
    ...
    if (context.isHogreBehorighetContext()) {
      if (hypoglykemier.getEgenkontrollBlodsocker() == null) {
        ValidatorUtil.addValidationError(
            validationMessages,
            CATEGORY_HYPOGLYKEMIER,
            "hypoglykemier.egenkontrollBlodsocker",
            ValidationMessageType.EMPTY);
      }
      ...
      if (hypoglykemier.getAllvarligForekomstVakenTid() == null) {
        ValidatorUtil.addValidationError(
            validationMessages,
            CATEGORY_HYPOGLYKEMIER,
            "hypoglykemier.allvarligForekomstVakenTid",
            ValidationMessageType.EMPTY);
      }
    }
  }
  ...
  private void validateBedomning(final Bedomning bedomning) {
    ...
    if (bedomning == null) {
      ValidatorUtil.addValidationError(
          validationMessages, CATEGORY_BEDOMNING, "bedomning", ValidationMessageType.EMPTY);
      return;
    }
    if (bedomning.getKorkortstyp().isEmpty()
        && (bedomning.getKanInteTaStallning() == null
            || ValidatorUtil.isNotNullFalse(bedomning.getKanInteTaStallning()))) {
      ValidatorUtil.addValidationError(
          validationMessages,
          CATEGORY_BEDOMNING,
          "bedomning",
    ...
    if (context.isHogreBehorighetContext()) {
      if (bedomning.getLamplighetInnehaBehorighet() == null) {
        ValidatorUtil.addValidationError(
            validationMessages,
            CATEGORY_BEDOMNING,
            "bedomning.lamplighetInnehaBehorighet",
            ValidationMessageType.EMPTY);
      }
    }
  }
  ...
  private void validateDiabetes(final Diabetes diabetes, final Patient patient) {
    ...
    if (diabetes.getObservationsperiod() == null) {
      ...
      if (!STRING_VALIDATOR.validateStringIsYear(diabetes.getObservationsperiod())) {
        ...
        if (ValidatorUtil.isYearBeforeBirth(
            diabetes.getObservationsperiod(), patient.getPersonId())) {
          ValidatorUtil.addValidationError(
              validationMessages,
              CATEGORY_DIABETES,
              "diabetes.observationsperiod",
    ...
    boolean annanBehandling =
        diabetes.getAnnanBehandlingBeskrivning() != null
            && !diabetes.getAnnanBehandlingBeskrivning().isEmpty();
    if (!(ValidatorUtil.isNotNullTrue(diabetes.getEndastKost())
        || ValidatorUtil.isNotNullTrue(diabetes.getTabletter())
        || ValidatorUtil.isNotNullTrue(diabetes.getInsulin())
        || annanBehandling)) {
      ValidatorUtil.addValidationError(
          validationMessages,
          CATEGORY_DIABETES,
  ...
  private void validateSyn(final Syn syn) {
    ...
    if (syn.getSeparatOgonlakarintyg() == null) {
      ...
      if (syn.getVanster() == null || syn.getVanster().getUtanKorrektion() == null) {
        ...
        if (syn.getVanster().getUtanKorrektion() < 0.0
            || syn.getVanster().getUtanKorrektion() > 2.0) {
          ValidatorUtil.addValidationError(
              validationMessages,
              CATEGORY_SYN,
              "syn.vanster.utanKorrektion",
        ...
        if (syn.getVanster().getMedKorrektion() != null) {
          if (syn.getVanster().getMedKorrektion() < 0.0
              || syn.getVanster().getMedKorrektion() > 2.0) {
            ValidatorUtil.addValidationError(
                validationMessages,
                CATEGORY_SYN,
      ...
      if (syn.getBinokulart() == null || syn.getBinokulart().getUtanKorrektion() == null) {
        ...
        if (syn.getBinokulart().getUtanKorrektion() < 0.0
