...
public class QuestionSynSeparatOgonlakarintyg {

  public static CertificateDataElement toCertificate(
      Synfunktion synfunktion, int index, CertificateTextProvider textProvider) {
    final var skickasSeparat = synfunktion != null ? synfunktion.getSkickasSeparat() : null;
    return CertificateDataElement.builder()
        .id(SYN_SEPARAT_OGONLAKARINTYG_SVAR_ID)
        .parent(SYN_VARDEN_SYNSKARPA_MESSAGE_ID)
        .index(index)
        .config(
            CertificateDataConfigViewText.builder()
                .text(textProvider.get(SYN_SEPARAT_OGONLAKARINTYG_SVAR_TEXT_ID))
                .build())
        .value(CertificateDataValueViewText.builder().text(booleanValue(skickasSeparat)).build())
        .build();
  }
}
