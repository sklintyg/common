...
public final class UtlatandeToIntyg {
  ...
  private static List<Svar> getSvar(TsDiabetesUtlatandeV4 source) {
    ...
    if (source.getIntygAvser() != null
      ...
      for (IntygAvserKategori intygAvserKategori : source.getIntygAvser().getKategorier()) {
        ...
        svars.add(
            aSvar(INTYG_AVSER_SVAR_ID, intygAvserInstans++)
                .withDelsvar(
                    INTYG_AVSER_DELSVAR_ID,
                    aCV(
  ...
  private static void buildAllmant(Allmant allmant, List<Svar> svars) {
    if (allmant.getPatientenFoljsAv() != null) {
      svars.add(
          aSvar(ALLMANT_PATIENTEN_FOLJS_AV_SVAR_ID)
              .withDelsvar(
                  ALLMANT_PATIENTEN_FOLJS_AV_DELSVAR_ID,
                  aCV(
                      KV_VARDNIVA_CODE_SYSTEM,
                      allmant.getPatientenFoljsAv().getCode(),
    ...
    if (allmant.getMedicineringForDiabetes() != null) {
      svars.add(
          aSvar(ALLMANT_MEDICINERING_FOR_DIABETES_SVAR_ID)
              .withDelsvar(
                  ALLMANT_MEDICINERING_FOR_DIABETES_DELSVAR_ID,
    ...
    if (allmant.getMedicineringMedforRiskForHypoglykemi() != null) {
      svars.add(
          aSvar(ALLMANT_MEDICINERING_MEDFOR_RISK_FOR_HYPOGYKEMI_SVAR_ID)
              .withDelsvar(
                  ALLMANT_MEDICINERING_MEDFOR_RISK_FOR_HYPOGYKEMI_DELSVAR_ID,
                  InternalConverterUtil.getBooleanContent(
                      allmant.getMedicineringMedforRiskForHypoglykemi()))
              .build());
    }

    if (allmant.getBehandling() != null
        && TRUE.equals(allmant.getMedicineringMedforRiskForHypoglykemi())) {
      // Here we rely on withDelsvar not adding a delsvar if content is null
      final var behandlingSvar =
          aSvar(ALLMANT_BEHANDLING_SVAR_ID)
              ...
              .withDelsvar(
                  ALLMANT_BEHANDLING_ANNAN_DELSVAR_ID,
    ...
    if (allmant.getMedicineringMedforRiskForHypoglykemiTidpunkt() != null) {
      svars.add(
          aSvar(ALLMANT_MEDICINERING_MEDFOR_RISK_FOR_HYPOGYKEMI_TIDPUNKT_SVAR_ID)
              .withDelsvar(
                  ALLMANT_MEDICINERING_MEDFOR_RISK_FOR_HYPOGYKEMI_TIDPUNKT_DELSVAR_ID,
                  aPartialDate(
  ...
  private static void buildHypoglykemi(Hypoglykemi hypoglykemi, List<Svar> answers) {
    ...
    if (hasControl != null) {
      ...
      answerBuilder.withDelsvar(
          HYPOGLYKEMI_KONTROLL_SJUKDOMSTILLSTAND_DELSVAR_ID,
          InternalConverterUtil.getBooleanContent(hasControl));
    ...
    InternalConverterUtil.addIfNotNull(
        ...
        HYPOGLYKEMI_FORSTAR_RISKER_MED_HYPOGLYKEMI_DELSVAR_ID,
        hypoglykemi.getForstarRiskerMedHypoglykemi());

    InternalConverterUtil.addIfNotNull(
        answers,
        HYPOGLYKEMI_FORMAGA_KANNA_VARNINGSTECKEN_SVAR_ID,
        HYPOGLYKEMI_FORMAGA_KANNA_VARNINGSTECKEN_DELSVAR_ID,
        hypoglykemi.getFormagaKannaVarningstecken());

    InternalConverterUtil.addIfNotNull(
        answers,
        HYPOGLYKEMI_VIDTA_ADEKVATA_ATGARDER_SVAR_ID,
        HYPOGLYKEMI_VIDTA_ADEKVATA_ATGARDER_DELSVAR_ID,
        hypoglykemi.getVidtaAdekvataAtgarder());
    ...
    if (recurringThisYear != null) {
      ...
      answerBuilder.withDelsvar(
          HYPOGLYKEMI_ATERKOMMANDE_SENASTE_ARET_DELSVAR_ID,
          InternalConverterUtil.getBooleanContent(recurringThisYear));
      ...
      if (recurringThisYear && hypoglykemi.getAterkommandeSenasteAretTidpunkt() != null) {
        answerBuilder.withDelsvar(
            ...
            aPartialDate(
                PartialDateTypeFormatEnum.YYYY_MM_DD,
                hypoglykemi.getAterkommandeSenasteAretTidpunkt().asLocalDate()));
      }
      if (recurringThisYear && hypoglykemi.getAterkommandeSenasteAretKontrolleras() != null) {
        answerBuilder.withDelsvar(
            ...
            InternalConverterUtil.getBooleanContent(
                hypoglykemi.getAterkommandeSenasteAretKontrolleras()));
      }
      if (recurringThisYear && hypoglykemi.getAterkommandeSenasteAretTrafik() != null) {
        answerBuilder.withDelsvar(
            ...
            InternalConverterUtil.getBooleanContent(
                hypoglykemi.getAterkommandeSenasteAretTrafik()));
      }
      answers.add(answerBuilder.build());
    }
    ...
    if (recurringAwakeLastTwelveMonths != null) {
      ...
      answerBuilder.withDelsvar(
          HYPOGLYKEMI_ATERKOMMANDE_VAKET_SENASTE_TOLV_DELSVAR_ID,
          InternalConverterUtil.getBooleanContent(recurringAwakeLastTwelveMonths));
      ...
      if (recurringAwakeLastTwelveMonths && recurringAwakeLastThreeMonths != null) {
        answerBuilder.withDelsvar(
            ...
            InternalConverterUtil.getBooleanContent(recurringAwakeLastThreeMonths));

        final var recurringAwakeLastThreeMonthsTimepoint =
        ...
        if (recurringAwakeLastThreeMonths && recurringAwakeLastThreeMonthsTimepoint != null) {
          answerBuilder.withDelsvar(
              ...
              aPartialDate(
                  PartialDateTypeFormatEnum.YYYY_MM_DD,
                  recurringAwakeLastThreeMonthsTimepoint.asLocalDate()));
        }
      }
      answers.add(answerBuilder.build());
    }
    ...
    if (seriousLastTwelveMonths != null) {
      ...
      answerBuilder.withDelsvar(
          HYPOGLYKEMI_ALLVARLIG_SENASTE_TOLV_MANADERNA_DELSVAR_ID,
          InternalConverterUtil.getBooleanContent(seriousLastTwelveMonths));

      final var seriousLastTwelveMonthsTimepoint =
      ...
      if (seriousLastTwelveMonths && seriousLastTwelveMonthsTimepoint != null) {
        answerBuilder.withDelsvar(
            ...
            aPartialDate(
                PartialDateTypeFormatEnum.YYYY_MM_DD,
                seriousLastTwelveMonthsTimepoint.asLocalDate()));
      }
      answers.add(answerBuilder.build());
    }
    ...
    InternalConverterUtil.addIfNotNull(
        ...
        HYPOGLYKEMI_REGELBUNDNA_BLODSOCKERKONTROLLER_DELSVAR_ID,
        hypoglykemi.getRegelbundnaBlodsockerkontroller());
  }
  ...
  private static void buildOvrigt(Ovrigt ovrigt, List<Svar> answers) {
    ...
    if (hasComplications != null) {
      ...
      answerBuilder.withDelsvar(
          OVRIGT_KOMPLIKATIONER_AV_SJUKDOMEN_DELSVAR_ID,
          InternalConverterUtil.getBooleanContent(hasComplications));
    ...
    final var resolvedShouldBeExamined =
        !Strings.nullToEmpty(shouldBeExamined).trim().isEmpty() ? shouldBeExamined : null;
    InternalConverterUtil.addIfNotBlank(
        answers,
        OVRIGT_BOR_UNDERSOKAS_AV_SPECIALIST_SVAR_ID,
        OVRIGT_BOR_UNDERSOKAS_AV_SPECIALIST_DELSVAR_ID,
        resolvedShouldBeExamined);
  }
  ...
  private static void buildBedomning(Bedomning bedomning, List<Svar> svars) {
    ...
    InternalConverterUtil.addIfNotBlank(
        ...
        BEDOMNING_OVRIGA_KOMMENTARER_DELSVAR_ID,
        resolvedComments);
  }
}
