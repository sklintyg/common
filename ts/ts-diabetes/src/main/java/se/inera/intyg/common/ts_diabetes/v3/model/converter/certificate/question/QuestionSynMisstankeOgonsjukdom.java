...
public class QuestionSynMisstankeOgonsjukdom {

  public static CertificateDataElement toCertificate(
      Synfunktion synfunktion, int index, CertificateTextProvider textProvider) {
    final var misstankeOgonsjukdom =
        synfunktion != null ? synfunktion.getMisstankeOgonsjukdom() : null;
    return CertificateDataElement.builder()
        .id(SYN_UTAN_ANMARKNING_SVAR_ID)
        .parent(SYN_CATEGORY_ID)
        .index(index)
        .config(
            CertificateDataConfigViewText.builder()
                .text(textProvider.get(SYN_UTAN_ANMARKNING_SVAR_TEXT_ID))
                .build())
        .value(
            CertificateDataValueViewText.builder().text(booleanValue(misstankeOgonsjukdom)).build())
        .build();
  }
}
