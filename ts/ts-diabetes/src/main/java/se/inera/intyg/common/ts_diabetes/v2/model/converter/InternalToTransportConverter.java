...
public final class InternalToTransportConverter {
  ...
  public static RegisterTSDiabetesType convert(TsDiabetesUtlatandeV2 utlatande) {
    ...
    if (utlatande.getSyn().getSeparatOgonlakarintyg() != null
        && !utlatande.getSyn().getSeparatOgonlakarintyg()) {
    ...
    if (!Strings.nullToEmpty(utlatande.getTextVersion()).trim().isEmpty()) {
      List<String> versionInfo =
          Splitter.onPattern(DELIMITER_REGEXP).splitToList(utlatande.getTextVersion());
  ...
  private static IdentitetStyrkt readIdentitetStyrkt(Vardkontakt vardkontakt) {
    ...
    result.setIdkontroll(
        IdentifieringsVarden.fromValue(
            IdKontrollKod.valueOf(vardkontakt.getIdkontroll()).getCode()));
    return result;
  }
  ...
  private static SynfunktionDiabetes readSynfunktionDiabetes(Syn syn) {
    ...
    result.setFinnsProvningOgatsRorlighet(
        syn.getProvningOgatsRorlighet() != null && syn.getProvningOgatsRorlighet());
    return result;
  }
  ...
  private static SynskarpaMedKorrektion readMedKorrektion(Syn syn) {
    ...
    result.setBinokulart(
        syn.getBinokulart() != null ? syn.getBinokulart().getMedKorrektion() : null);
  ...
  private static IntygsAvserTypDiabetes readIntygAvser(IntygAvser intygAvser) {
    ...
    for (IntygAvserKategori kat : intygAvser.getKorkortstyp()) {
      KorkortsbehorighetTsDiabetes bh =
          KorkortsbehorighetTsDiabetes.fromValue(Korkortsbehorighet.fromValue(kat.name()));
  ...
  private static Hypoglykemier readHypoglykemier(
      se.inera.intyg.common.ts_diabetes.v2.model.internal.Hypoglykemier hypoglykemier) {
    Hypoglykemier result = new Hypoglykemier();
    result.setHarKunskapOmAtgarder(
        hypoglykemier.getKunskapOmAtgarder() != null && hypoglykemier.getKunskapOmAtgarder());
    result.setHarTeckenNedsattHjarnfunktion(
        hypoglykemier.getTeckenNedsattHjarnfunktion() != null
            && hypoglykemier.getTeckenNedsattHjarnfunktion());

    if (hypoglykemier.getSaknarFormagaKannaVarningstecken() != null) {
      result.setSaknarFormagaKannaVarningstecken(
    ...
    if (hypoglykemier.getAllvarligForekomstTrafiken() != null) {
      ...
      result.setAllvarligForekomstTrafikBeskrivning(
    ...
    if (hypoglykemier.getAllvarligForekomstVakenTid() != null) {
      ...
      result.setAllvarligForekomstVakenTidAr(
          hypoglykemier.getAllvarligForekomstVakenTidObservationstid() != null
              ? hypoglykemier.getAllvarligForekomstVakenTidObservationstid().getDate()
              : null);
    }
    return result;
  }

  private static Diabetes readDiabetes(
      se.inera.intyg.common.ts_diabetes.v2.model.internal.Diabetes diabetes) {
    ...
    if (diabetes.getDiabetestyp() != null) {
      result
          .getDiabetesTyp()
          .add(
              InternalToTransportUtil.convertDiabetesTyp(
                  DiabetesKod.valueOf(diabetes.getDiabetestyp())));
    }
    return result;
  }
  ...
  private static BedomningTypDiabetes readBedomning(Bedomning bedomning) {
    ...
    for (BedomningKorkortstyp typ : bedomning.getKorkortstyp()) {
      result
          .getKorkortstyp()
          .add(KorkortsbehorighetTsDiabetes.fromValue(Korkortsbehorighet.fromValue(typ.name())));
    }
