...
public final class TransportToInternal {
  ...
  private static void setSvar(TsDiabetesUtlatandeV4.Builder utlatande, Intyg source)
  ...
  private static void handleIntygAvser(Set<IntygAvserKategori> intygAvserSet, Svar svar)
      throws ConverterException {
      ...
      switch (delsvar.getId()) {
        ...
        default:
          throw new IllegalArgumentException(
              "Invalid delsvar id received, expected id IntigetAvser.");
      }
    }
  }

  private static void handleIdentitetStyrkt(TsDiabetesUtlatandeV4.Builder utlatande, Svar svar)
      throws ConverterException {
    for (Delsvar delsvar : svar.getDelsvar()) {
      switch (delsvar.getId()) {
        case RespConstants.IDENTITET_STYRKT_GENOM_DELSVAR_ID:
          utlatande.setIdentitetStyrktGenom(
              IdKontroll.create(KvIdKontroll.fromCode(getCVSvarContent(delsvar).getCode())));
          break;
        default:
          throw new IllegalArgumentException(
              "Unexpected delsvar id '"
                  + delsvar.getId()
  ...
  private static void handleAllmantPatientenFoljsAv(Allmant.Builder allmant, Svar svar)
      throws ConverterException {
    for (Delsvar delsvar : svar.getDelsvar()) {
      switch (delsvar.getId()) {
  ...
  private static void handleAllmantTypAvDiabetes(Allmant.Builder allmant, Svar svar)
      throws ConverterException {
    for (Delsvar delsvar : svar.getDelsvar()) {
      switch (delsvar.getId()) {
  ...
  private static void handleAllmantMedicineringMedforRiskForHypoglykemi(
      Allmant.Builder allmant, Svar svar) {
    for (Delsvar delsvar : svar.getDelsvar()) {
      switch (delsvar.getId()) {
  ...
  private static void handleAllmantMedicineringMedforRiskForHypoglykemiTidpunkt(
      Allmant.Builder allmant, Svar svar) throws ConverterException {
    for (Delsvar delsvar : svar.getDelsvar()) {
      switch (delsvar.getId()) {
  ...
  private static void handleHypoglykemiKontrollSjukdomstillstand(
      Hypoglykemi.Builder hypoglykemi, Svar svar) {
    for (Delsvar delsvar : svar.getDelsvar()) {
      switch (delsvar.getId()) {
  ...
  private static void handleHypoglykemiForstarRiskerMedHypoglykemi(
      Hypoglykemi.Builder hypoglykemi, Svar svar) {
    for (Delsvar delsvar : svar.getDelsvar()) {
      switch (delsvar.getId()) {
  ...
  private static void handleHypoglykemiFormagaKannaVarningstecken(
      Hypoglykemi.Builder hypoglykemi, Svar svar) {
    for (Delsvar delsvar : svar.getDelsvar()) {
      switch (delsvar.getId()) {
  ...
  private static void handleHypoglykemiVidtaAdekvataAtgarder(
      Hypoglykemi.Builder hypoglykemi, Svar svar) {
    for (Delsvar delsvar : svar.getDelsvar()) {
      switch (delsvar.getId()) {
  ...
  private static void handleHypoglykemiAterkommandeSenasteAret(
      Hypoglykemi.Builder hypoglykemi, Svar svar) throws ConverterException {
    for (Delsvar delsvar : svar.getDelsvar()) {
      switch (delsvar.getId()) {
        ...
        case RespConstants.HYPOGLYKEMI_ATERKOMMANDE_SENASTE_ARET_TIDPUNKT_DELSVAR_ID:
          hypoglykemi.setAterkommandeSenasteAretTidpunkt(
              new InternalDate(getPartialDateContent(delsvar).getValue().toString()));
          break;
  ...
  private static void handleHypoglykemiAterkommandeVaketSenasteTolv(
      Hypoglykemi.Builder hypoglykemi, Svar svar) throws ConverterException {
    for (Delsvar delsvar : svar.getDelsvar()) {
      switch (delsvar.getId()) {
        ...
        case RespConstants.HYPOGLYKEMI_ATERKOMMANDE_VAKET_SENASTE_TRE_TIDPUNKT_DELSVAR_ID:
          hypoglykemi.setAterkommandeVaketSenasteTreTidpunkt(
              new InternalDate(getPartialDateContent(delsvar).getValue().toString()));
          break;
        default:
          throw new IllegalArgumentException(
  ...
  private static void handleHypoglykemiAllvarligSenasteTolvManaderna(
      Hypoglykemi.Builder hypoglykemi, Svar svar) throws ConverterException {
    for (Delsvar delsvar : svar.getDelsvar()) {
      switch (delsvar.getId()) {
        ...
        case RespConstants.HYPOGLYKEMI_ALLVARLIG_SENASTE_TOLV_MANADERNA_TIDPUNKT_DELSVAR_ID:
          hypoglykemi.setAllvarligSenasteTolvManadernaTidpunkt(
              new InternalDate(getPartialDateContent(delsvar).getValue().toString()));
          break;
        default:
          throw new IllegalArgumentException(
  ...
  private static void handleHypoglykemiRegelbundnaBlodsockerkontroller(
      Hypoglykemi.Builder hypoglykemi, Svar svar) {
    for (Delsvar delsvar : svar.getDelsvar()) {
      switch (delsvar.getId()) {
  ...
  private static void handleBedomning(
      Set<BedomningKorkortstyp> bedomningUppfyllerBehorighetskrav, Svar svar)
      throws ConverterException {
    for (Delsvar delsvar : svar.getDelsvar()) {
      switch (delsvar.getId()) {
        case RespConstants.BEDOMNING_UPPFYLLER_BEHORIGHETSKRAV_DELSVAR_ID:
          bedomningUppfyllerBehorighetskrav.add(
              BedomningKorkortstyp.valueOf(getCVSvarContent(delsvar).getCode()));
          break;
        default:
          throw new IllegalArgumentException(
              "Unexpected delsvar id '"
