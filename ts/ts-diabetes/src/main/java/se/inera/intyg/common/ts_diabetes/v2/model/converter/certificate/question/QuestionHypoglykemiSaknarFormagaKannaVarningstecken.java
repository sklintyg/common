...
public class QuestionHypoglykemiSaknarFormagaKannaVarningstecken {

    public static CertificateDataElement toCertificate(Boolean kannaVarningstecken, int index, CertificateTextProvider textProvider) {
        return CertificateDataElement.builder()
            .id(HYPOGLYKEMI_SAKNAR_FORMAGA_KANNA_VARNINGSTECKEN_SVAR_ID)
            .parent(HYPOGLYKEMI_CATEGORY_ID)
            .index(index)
            .config(
                CertificateDataConfigViewText.builder()
                    .text(textProvider.get(HYPOGLYKEMI_SAKNAR_FORMAGA_KANNA_VARNINGSTECKEN_SVAR_TEXT_ID))
                    .build()
            )
            .value(
                CertificateDataValueViewText.builder()
                    .text(booleanValue(kannaVarningstecken))
                    .build()
            )
            .build();
    }
