...
public final class UtlatandeToIntyg {
  ...
  private static void buildDiabetesSvar(Diabetes source, List<Svar> svars) {
    ...
    if (observationsperiod != null) {
      svars.add(
          aSvar(AR_FOR_DIABETESDIAGNOS_SVAR_ID_35)
              .withDelsvar(
                  AR_FOR_DIABETESDIAGNOS_DELSVAR_ID_35,
    ...
    if (source.getEndastKost() != null) {
      diabetesBehandling.withDelsvar(
          KOSTBEHANDLING_DELSVAR_ID_19, source.getEndastKost().toString());
    }
    ...
    if (insulinBehandlingsperiod != null) {
      diabetesBehandling.withDelsvar(
          INSULINBEHANDLING_SEDAN_AR_DELSVAR_ID_19,
    ...
    if (!Strings.nullToEmpty(source.getAnnanBehandlingBeskrivning()).trim().isEmpty()) {
      diabetesBehandling.withDelsvar(
          ANNAN_BEHANDLING_DELSVAR_ID_19, source.getAnnanBehandlingBeskrivning());
    }
  ...
  private static void buildHypoglykemierSvar(Hypoglykemier source, List<Svar> svars) {
    ...
    addIfNotNull(
        svars,
        KUNSKAP_ATGARDER_HYPOGLYKEMI_SVAR_ID_36,
        KUNSKAP_ATGARDER_HYPOGLYKEMI_DELSVAR_ID_36,
        source.getKunskapOmAtgarder());
    addIfNotNull(
        svars,
        FOREKOMST_HYPOGLYKEMIER_SVAR_ID_37,
        FOREKOMST_HYPOGLYKEMIER_DELSVAR_ID_37,
        source.getTeckenNedsattHjarnfunktion());
    addIfNotNull(
        svars,
        SAKNAR_FORMAGA_KANNA_VARNINGSTECKEN_SVAR_ID_38,
        SAKNAR_FORMAGA_KANNA_VARNINGSTECKEN_DELSVAR_ID_38,
        source.getSaknarFormagaKannaVarningstecken());
    InternalConverterUtil.SvarBuilder allvarligHypoglykemi =
        aSvar(ALLVARLIG_HYPOGLYKEMI_SVAR_ID_39);
    if (source.getAllvarligForekomst() != null) {
      allvarligHypoglykemi.withDelsvar(
          FOREKOMST_ALLVARLIG_HYPOGLYKEMI_DELSVAR_ID_39, source.getAllvarligForekomst().toString());
    }
    if (!Strings.nullToEmpty(source.getAllvarligForekomstBeskrivning()).trim().isEmpty()) {
      allvarligHypoglykemi.withDelsvar(
          ANTAL_EPISODER_ALLVARLIG_HYPOGLYKEMI_DELSVAR_ID_39,
          source.getAllvarligForekomstBeskrivning());
    }
    if (!allvarligHypoglykemi.delSvars.isEmpty()) {
      svars.add(allvarligHypoglykemi.build());
    }
    InternalConverterUtil.SvarBuilder allvarligHypoglykemiTrafiken =
        aSvar(ALLVARLIG_HYPOGLYKEMI_I_TRAFIKEN_SVAR_ID_40);
    if (source.getAllvarligForekomstTrafiken() != null) {
      allvarligHypoglykemiTrafiken.withDelsvar(
          FOREKOMST_ALLVARLIG_HYPOGLYKEMI_I_TRAFIKEN_DELSVAR_ID_40,
    ...
    if (!Strings.nullToEmpty(source.getAllvarligForekomstTrafikBeskrivning()).trim().isEmpty()) {
      allvarligHypoglykemiTrafiken.withDelsvar(
          ANTAL_EPISODER_TIDPUNKT_ALLVARLIG_HYPOGLYKEMI_I_TRAFIKEN_DELSVAR_ID_40,
    ...
    addIfNotNull(
        svars,
        EGENKONTROLLER_BLODSOCKER_SVAR_ID_41,
        EGENKONTROLLER_BLODSOCKER_DELSVAR_ID_41,
        source.getEgenkontrollBlodsocker());
    InternalConverterUtil.SvarBuilder allvarligHypoglykemiVakenTid =
        aSvar(ALLVARLIG_HYPOGLYKEMI_UNDER_VAKEN_TID_SVAR_ID_42);
    if (source.getAllvarligForekomstVakenTid() != null) {
      allvarligHypoglykemiVakenTid.withDelsvar(
          FOREKOMST_ALLVARLIG_HYPOGLYKEMI_UNDER_VAKEN_TID_DELSVAR_ID_42,
    ...
    if (source.getAllvarligForekomstVakenTidObservationstid() != null
      ...
      try {
        allvarligHypoglykemiVakenTid.withDelsvar(
            TIDPUNKT_ALLVARLIG_HYPOGLYKEMI_UNDER_VAKEN_TID_DELSVAR_ID_42,
  ...
  private static void buildSynSvar(Syn source, List<Svar> svars) {
    ...
    addIfNotNull(
        svars,
        OGONLAKARINTYG_SKICKAS_SEPARAT_SVAR_ID_43,
        OGONLAKARINTYG_SKICKAS_SEPARAT_DELSVAR_ID_43,
        source.getSeparatOgonlakarintyg());
    addIfNotNull(
        svars,
        SYNFALTSPROVNING_DONDER_SVAR_ID_44,
        SYNFALTSPROVNING_DONDER_DELSVAR_ID_44,
    ...
    if (source.getHoger() != null) {
      if (source.getHoger().getUtanKorrektion() != null) {
        synskarpa.withDelsvar(
            ...
            source.getHoger().getUtanKorrektion().toString());
      }
      if (source.getHoger().getMedKorrektion() != null) {
        synskarpa.withDelsvar(
            HOGER_OGA_MED_KORREKTION_DELSVAR_ID_8, source.getHoger().getMedKorrektion().toString());
      }
    }
    if (source.getVanster() != null) {
      if (source.getVanster().getUtanKorrektion() != null) {
        synskarpa.withDelsvar(
            ...
            source.getVanster().getUtanKorrektion().toString());
      }
      if (source.getVanster().getMedKorrektion() != null) {
        synskarpa.withDelsvar(
            VANSTER_OGA_MED_KORREKTION_DELSVAR_ID_8,
            source.getVanster().getMedKorrektion().toString());
      }
    }
  ...
  private static void buildBedomningSvar(Bedomning source, List<Svar> svars) {
    ...
    addIfNotNull(
        svars,
        LAMPLIGHET_INNEHA_BEHORIGHET_SVAR_ID_45,
        LAMPLIGHET_INNEHA_BEHORIGHET_DELSVAR_ID_45,
