...
public class QuestionHypoglykemiTeckenNedsattHjarnfunktion {

  public static CertificateDataElement toCertificate(
      Hypoglykemier hypoglykemier, int index, CertificateTextProvider textProvider) {
    final var nedsattHjarnfunktion =
        hypoglykemier != null ? hypoglykemier.getNedsattHjarnfunktion() : null;
    return CertificateDataElement.builder()
        .id(HYPOGLYKEMI_TECKEN_NEDSATT_HJARNFUNKTION_SVAR_ID)
        .parent(HYPOGLYKEMI_CATEGORY_ID)
        .index(index)
        .config(
            CertificateDataConfigViewText.builder()
                .text(textProvider.get(HYPOGLYKEMI_TECKEN_NEDSATT_HJARNFUNKTION_SVAR_TEXT_ID))
                .build())
        .value(
            CertificateDataValueViewText.builder().text(booleanValue(nedsattHjarnfunktion)).build())
        .build();
  }
}
