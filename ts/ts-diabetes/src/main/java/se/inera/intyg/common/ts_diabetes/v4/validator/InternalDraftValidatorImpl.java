...
public class InternalDraftValidatorImpl implements InternalDraftValidator<TsDiabetesUtlatandeV4> {
  ...
  protected static final String BEHANDLING_ROOT_FIELD_PATH =
  ...
  private static final Set<KorkortsbehorighetKod> HIGHER_LICENCE_TYPES =
      Set.of(
  ...
  private static boolean eligibleForRule8(TsDiabetesUtlatandeV4 utlatande) {
    return utlatande.getHypoglykemi() != null
        && isTrue(utlatande.getHypoglykemi().getAterkommandeSenasteAret());
  }

  // R9
  private static boolean eligibleForRule9(TsDiabetesUtlatandeV4 utlatande) {
    return utlatande.getHypoglykemi() != null
        && isTrue(utlatande.getHypoglykemi().getAterkommandeVaketSenasteTolv());
  }

  // R18
  private static boolean eligibleForRule18(TsDiabetesUtlatandeV4 utlatande) {
    return utlatande.getAllmant() != null
        && utlatande.getAllmant().getBehandling() != null
  ...
  private static boolean eligibleForRule29(TsDiabetesUtlatandeV4 utlatande) {
    ...
    return ovrigt != null
        && ovrigt.getKomplikationerAvSjukdomen() != null
        && isTrue(ovrigt.getKomplikationerAvSjukdomen());
  }

  private static boolean eligibleForRule30(TsDiabetesUtlatandeV4 utlatande) {
    final var allmant = utlatande.getAllmant();
    return allmant != null
        && allmant.getMedicineringMedforRiskForHypoglykemi() != null
  ...
  private static boolean eligibleForRule32(TsDiabetesUtlatandeV4 utlatande) {
    return utlatande.getAllmant() != null
        && isTrue(utlatande.getAllmant().getMedicineringForDiabetes());
  }
  ...
  private static boolean eligibleForRule35(TsDiabetesUtlatandeV4 utlatande) {
    ...
    final var selectedBehorigheter =
        getLicenceTypes(utlatande.getBedomning().getUppfyllerBehorighetskrav());
  ...
  public ValidateDraftResponse validateDraft(TsDiabetesUtlatandeV4 utlatande) {
    ...
    if (utlatande.getAllmant() != null) {
      ...
      addValidationError(
          validationMessages, ALLMANT_CATEGORY_ID, ALLMANT_JSON_ID, ValidationMessageType.EMPTY);
    }
    ...
    if (utlatande.getOvrigt() != null) {
      ...
      addValidationError(
          validationMessages, OVRIGT_CATEGORY_ID, OVRIGT_CATEGORY_ID, ValidationMessageType.EMPTY);
    }
  ...
  private void validateIntygetAvser(
      TsDiabetesUtlatandeV4 utlatande, List<ValidationMessage> validationMessages) {
    final var intygAvser =
        utlatande.getIntygAvser() == null ? null : utlatande.getIntygAvser().getKategorier();
    if (intygAvser == null || intygAvser.isEmpty()) {
      addValidationErrorWithQuestionId(
          validationMessages,
          INTYG_AVSER_CATEGORY_ID,
          INTYG_AVSER_SVAR_JSON_ID + ".kategorier",
          ValidationMessageType.EMPTY,
          INTYG_AVSER_SVAR_ID);
    }
  }

  private void validateIdentitetStyrkt(
      TsDiabetesUtlatandeV4 utlatande, List<ValidationMessage> validationMessages) {
    if (utlatande.getIdentitetStyrktGenom() == null) {
      addValidationErrorWithQuestionId(
          validationMessages,
          IDENTITET_CATEGORY_ID,
          IDENTITET_STYRKT_GENOM_JSON_ID + ".typ",
          ValidationMessageType.EMPTY,
          IDENTITET_STYRKT_GENOM_SVAR_ID);
    }
  }

  private void validatePatientenFoljsAv(
      TsDiabetesUtlatandeV4 utlatande, List<ValidationMessage> validationMessages) {
    ...
    if (allmant.getPatientenFoljsAv() == null) {
      addValidationErrorWithQuestionId(
          validationMessages,
          ALLMANT_CATEGORY_ID,
          ALLMANT_JSON_ID + "." + ALLMANT_PATIENTEN_FOLJS_AV_JSON_ID,
          ValidationMessageType.EMPTY,
          ALLMANT_PATIENTEN_FOLJS_AV_SVAR_ID);
    }
  }

  private void validateDiagnosAr(
      TsDiabetesUtlatandeV4 utlatande, List<ValidationMessage> validationMessages) {
    final var diabetesSedanArFieldPath =
        ALLMANT_JSON_ID + "." + ALLMANT_DIABETES_DIAGNOS_AR_JSON_ID;
    final var allmant = Objects.requireNonNull(utlatande.getAllmant());
    ...
    if (cleanedDiabetesDiagnosAr.isEmpty()) {
      addValidationErrorWithQuestionId(
          validationMessages,
          ALLMANT_CATEGORY_ID,
          diabetesSedanArFieldPath,
          ValidationMessageType.EMPTY,
          B_02B,
          ALLMANT_DIABETES_DIAGNOS_AR_SVAR_ID);
      return;
    }
    ...
    // R2
    if (ValidatorUtil.isYearBeforeBirth(
        parsedYear, utlatande.getGrundData().getPatient().getPersonId())) {
      addValidationErrorWithQuestionId(
          validationMessages,
          ALLMANT_CATEGORY_ID,
          diabetesSedanArFieldPath,
  ...
  private void validateTypAvDiabetes(
      TsDiabetesUtlatandeV4 utlatande, List<ValidationMessage> validationMessages) {
    final var typAvDiabetesFieldPath = ALLMANT_JSON_ID + "." + ALLMANT_TYP_AV_DIABETES_JSON_ID;
    final var annanTypAvDiabetesBeskrivningFieldPath =
    ...
    if (allmant.getTypAvDiabetes() == null) {
      addValidationErrorWithQuestionId(
          validationMessages,
          ALLMANT_CATEGORY_ID,
          typAvDiabetesFieldPath,
          ValidationMessageType.EMPTY,
          ALLMANT_TYP_AV_DIABETES_SVAR_ID);
    }

    if (eligibleForRule3(utlatande)
        && Strings.nullToEmpty(allmant.getBeskrivningAnnanTypAvDiabetes()).trim().isEmpty()) {
      addValidationErrorWithQuestionId(
          validationMessages,
          ALLMANT_CATEGORY_ID,
          annanTypAvDiabetesBeskrivningFieldPath,
          ValidationMessageType.EMPTY,
          ALLMANT_BESKRIVNING_ANNAN_TYP_AV_DIABETES_DELSVAR_ID);
    } else if (eligibleForRule3(utlatande)
        && Strings.nullToEmpty(allmant.getBeskrivningAnnanTypAvDiabetes()).trim().length()
            > MAX_FIFTY_THREE_CHARS) {
      addValidationErrorWithQuestionId(
          validationMessages,
          ALLMANT_CATEGORY_ID,
          annanTypAvDiabetesBeskrivningFieldPath,
  ...
  private void validateMedicineringForDiabetes(
      TsDiabetesUtlatandeV4 utlatande, List<ValidationMessage> validationMessages) {
    final var medicineringForDiabetes =
        ALLMANT_JSON_ID + "." + ALLMANT_MEDICINERING_FOR_DIABETES_JSON_ID;
    final var allmant = Objects.requireNonNull(utlatande.getAllmant());
    if (allmant.getMedicineringForDiabetes() == null) {
      addValidationErrorWithQuestionId(
          validationMessages,
          ALLMANT_CATEGORY_ID,
          medicineringForDiabetes,
          ValidationMessageType.EMPTY,
          ALLMANT_MEDICINERING_FOR_DIABETES_SVAR_ID);
    }
  }

  private void validateMedicineringMedforRiskForHypoglykemi(
      TsDiabetesUtlatandeV4 utlatande, List<ValidationMessage> validationMessages) {
    final var allmant = Objects.requireNonNull(utlatande.getAllmant());
    if (eligibleForRule32(utlatande) && allmant.getMedicineringMedforRiskForHypoglykemi() == null) {
      addValidationErrorWithQuestionId(
          ...
          ValidationMessageType.EMPTY,
          ALLMANT_MEDICINERING_MEDFOR_RISK_FOR_HYPOGYKEMI_SVAR_ID);
    }
  }

  private void validateBehandling(
      TsDiabetesUtlatandeV4 utlatande, List<ValidationMessage> validationMessages) {
    final var allmant = Objects.requireNonNull(utlatande.getAllmant());
    if (eligibleForRule30(utlatande)) {
      ...
      if (behandling == null) {
        addValidationErrorWithQuestionId(
            ...
            ALLMANT_JSON_ID + "." + ALLMANT_BEHANDLING_JSON_ID,
            ValidationMessageType.EMPTY,
            "common.validation.ue-checkgroup.empty",
            ALLMANT_BEHANDLING_SVAR_ID);
        return;
      }
      ...
      if (eligibleForRule18(utlatande)
          && Strings.nullToEmpty(behandling.getAnnanAngeVilken()).trim().isEmpty()) {
        addValidationErrorWithQuestionId(
            validationMessages,
            ALLMANT_CATEGORY_ID,
          ...
          && Strings.nullToEmpty(behandling.getAnnanAngeVilken()).trim().length()
              > MAX_FIFTY_THREE_CHARS) {
        addValidationError(
            validationMessages,
            ALLMANT_CATEGORY_ID,
  ...
  private void validateMedicineringMedforRiskForHypoglykemiTidpunkt(
      TsDiabetesUtlatandeV4 utlatande, List<ValidationMessage> validationMessages) {
    final var allmant = Objects.requireNonNull(utlatande.getAllmant());
    if (eligibleForRule30(utlatande)
        && allmant.getMedicineringMedforRiskForHypoglykemiTidpunkt() == null) {
      addValidationErrorWithQuestionId(
          validationMessages,
          ALLMANT_CATEGORY_ID,
    ...
    if (eligibleForRule19(utlatande)) {
      final var patientBirthDate =
          ValidatorUtil.getBirthDateFromPersonnummer(
              utlatande.getGrundData().getPatient().getPersonId());
      validateDateWithinIntervalWithQuestionId(
  ...
  private void validateKontrollSjukdomstillstand(
      TsDiabetesUtlatandeV4 utlatande, List<ValidationMessage> validationMessages) {
    ...
    if (hypoglykemi.getKontrollSjukdomstillstand() == null) {
      addValidationErrorWithQuestionId(
          validationMessages,
          HYPOGLYKEMI_CATEGORY_ID,
          HYPOGLYKEMI_CATEGORY_ID + "." + HYPOGLYKEMI_KONTROLL_SJUKDOMSTILLSTAND_JSON_ID,
          ValidationMessageType.EMPTY,
          HYPOGLYKEMI_KONTROLL_SJUKDOMSTILLSTAND_SVAR_ID);
      return;
    }

    if (eligibleForRule27(utlatande)
        && Strings.nullToEmpty(hypoglykemi.getKontrollSjukdomstillstandVarfor()).trim().isEmpty()) {
      addValidationErrorWithQuestionId(
          validationMessages,
          HYPOGLYKEMI_CATEGORY_ID,
          HYPOGLYKEMI_CATEGORY_ID + "." + HYPOGLYKEMI_KONTROLL_SJUKDOMSTILLSTAND_VARFOR_JSON_ID,
          ValidationMessageType.EMPTY,
          HYPOGLYKEMI_KONTROLL_SJUKDOMSTILLSTAND_VARFOR_DELSVAR_ID);
    } else if (eligibleForRule27(utlatande)
        && Strings.nullToEmpty(hypoglykemi.getKontrollSjukdomstillstandVarfor()).trim().length()
            > MAX_FIFTY_THREE_CHARS) {
      addValidationErrorWithQuestionId(
          validationMessages,
          HYPOGLYKEMI_CATEGORY_ID,
  ...
  private void validateForstarRiskerMedHypoglykemi(
      TsDiabetesUtlatandeV4 utlatande, List<ValidationMessage> validationMessages) {
    final var hypoglykemi = Objects.requireNonNull(utlatande.getHypoglykemi());
    if (hypoglykemi.getForstarRiskerMedHypoglykemi() == null) {
      addValidationErrorWithQuestionId(
          validationMessages,
          HYPOGLYKEMI_CATEGORY_ID,
          HYPOGLYKEMI_CATEGORY_ID + "." + HYPOGLYKEMI_FORSTAR_RISKER_MED_HYPOGLYKEMI_JSON_ID,
          ValidationMessageType.EMPTY,
          HYPOGLYKEMI_FORSTAR_RISKER_MED_HYPOGLYKEMI_SVAR_ID);
    }
  }

  private void validateFormagaKannaVarningsTecken(
      TsDiabetesUtlatandeV4 utlatande, List<ValidationMessage> validationMessages) {
    final var hypoglykemi = Objects.requireNonNull(utlatande.getHypoglykemi());
    if (hypoglykemi.getFormagaKannaVarningstecken() == null) {
      addValidationErrorWithQuestionId(
          validationMessages,
          HYPOGLYKEMI_CATEGORY_ID,
          HYPOGLYKEMI_CATEGORY_ID + "." + HYPOGLYKEMI_FORMAGA_KANNA_VARNINGSTECKEN_JSON_ID,
          ValidationMessageType.EMPTY,
          HYPOGLYKEMI_FORMAGA_KANNA_VARNINGSTECKEN_SVAR_ID);
    }
  }

  private void validateVidtaAdekvataAtgarder(
      TsDiabetesUtlatandeV4 utlatande, List<ValidationMessage> validationMessages) {
    final var hypoglykemi = Objects.requireNonNull(utlatande.getHypoglykemi());
    if (hypoglykemi.getVidtaAdekvataAtgarder() == null) {
      addValidationErrorWithQuestionId(
          validationMessages,
          HYPOGLYKEMI_CATEGORY_ID,
          HYPOGLYKEMI_CATEGORY_ID + "." + HYPOGLYKEMI_VIDTA_ADEKVATA_ATGARDER_JSON_ID,
          ValidationMessageType.EMPTY,
          HYPOGLYKEMI_VIDTA_ADEKVATA_ATGARDER_SVAR_ID);
    }
  }

  private void validateAterkommandeSenasteAret(
      TsDiabetesUtlatandeV4 utlatande, List<ValidationMessage> validationMessages) {
    final var hypoglykemi = Objects.requireNonNull(utlatande.getHypoglykemi());
    if (hypoglykemi.getAterkommandeSenasteAret() == null) {
      addValidationErrorWithQuestionId(
          validationMessages,
          HYPOGLYKEMI_CATEGORY_ID,
          HYPOGLYKEMI_CATEGORY_ID + "." + HYPOGLYKEMI_ATERKOMMANDE_SENASTE_ARET_JSON_ID,
          ValidationMessageType.EMPTY,
    ...
    if (eligibleForRule8(utlatande) || eligibleForRule20(utlatande)) {
      final var patientBirthDate =
          ValidatorUtil.getBirthDateFromPersonnummer(
              utlatande.getGrundData().getPatient().getPersonId());
      validateDateWithinIntervalWithQuestionId(
          hypoglykemi.getAterkommandeSenasteAretTidpunkt(),
          patientBirthDate,
          LocalDate.now(),
          validationMessages,
          HYPOGLYKEMI_CATEGORY_ID,
    ...
    if (eligibleForRule8(utlatande)
        && hypoglykemi.getAterkommandeSenasteAretKontrolleras() == null) {
      addValidationErrorWithQuestionId(
          validationMessages,
          HYPOGLYKEMI_CATEGORY_ID,
          HYPOGLYKEMI_CATEGORY_ID
              + "."
              + HYPOGLYKEMI_ATERKOMMANDE_SENASTE_ARET_KONTROLLERAS_JSON_ID,
          ValidationMessageType.EMPTY,
    ...
    if (eligibleForRule8(utlatande) && hypoglykemi.getAterkommandeSenasteAretTrafik() == null) {
      addValidationErrorWithQuestionId(
          validationMessages,
          HYPOGLYKEMI_CATEGORY_ID,
          HYPOGLYKEMI_CATEGORY_ID + "." + HYPOGLYKEMI_ATERKOMMANDE_SENASTE_ARET_TRAFIK_JSON_ID,
          ValidationMessageType.EMPTY,
          HYPOGLYKEMI_ATERKOMMANDE_SENASTE_ARET_TRAFIK_DELSVAR_ID);
    }
  }

  private void validateAterkommandeVaketSenasteTolv(
      TsDiabetesUtlatandeV4 utlatande, List<ValidationMessage> validationMessages) {
    final var hypoglykemi = Objects.requireNonNull(utlatande.getHypoglykemi());
    if (hypoglykemi.getAterkommandeVaketSenasteTolv() == null) {
      addValidationErrorWithQuestionId(
          validationMessages,
          HYPOGLYKEMI_CATEGORY_ID,
          HYPOGLYKEMI_CATEGORY_ID + "." + HYPOGLYKEMI_ATERKOMMANDE_VAKET_SENASTE_TOLV_JSON_ID,
          ValidationMessageType.EMPTY,
    ...
    if (eligibleForRule9(utlatande) && hypoglykemi.getAterkommandeVaketSenasteTre() == null) {
      addValidationErrorWithQuestionId(
          validationMessages,
          HYPOGLYKEMI_CATEGORY_ID,
          HYPOGLYKEMI_CATEGORY_ID + "." + HYPOGLYKEMI_ATERKOMMANDE_VAKET_SENASTE_TRE_JSON_ID,
          ValidationMessageType.EMPTY,
    ...
    if (eligibleForRule33(utlatande) || eligibleForRule21(utlatande)) {
      final var patientBirthDate =
          ValidatorUtil.getBirthDateFromPersonnummer(
              utlatande.getGrundData().getPatient().getPersonId());
      validateDateWithinIntervalWithQuestionId(
          hypoglykemi.getAterkommandeVaketSenasteTreTidpunkt(),
          patientBirthDate,
          LocalDate.now(),
          validationMessages,
          HYPOGLYKEMI_CATEGORY_ID,
          HYPOGLYKEMI_CATEGORY_ID
              + "."
              + HYPOGLYKEMI_ATERKOMMANDE_VAKET_SENASTE_TRE_TIDPUNKT_JSON_ID,
          HYPOGLYKEMI_ATERKOMMANDE_VAKET_SENASTE_TRE_TIDPUNKT_DELSVAR_ID);
    }
  }

  private void validateAllvarligSenasteTolvManaderna(
      TsDiabetesUtlatandeV4 utlatande, List<ValidationMessage> validationMessages) {
    final var hypoglykemi = Objects.requireNonNull(utlatande.getHypoglykemi());
    if (hypoglykemi.getAllvarligSenasteTolvManaderna() == null) {
      addValidationErrorWithQuestionId(
          validationMessages,
          HYPOGLYKEMI_CATEGORY_ID,
          HYPOGLYKEMI_CATEGORY_ID + "." + HYPOGLYKEMI_ALLVARLIG_SENASTE_TOLV_MANADERNA_JSON_ID,
          ValidationMessageType.EMPTY,
  ...
  private void validateRegelbundnaBlodsockerkontroller(
      TsDiabetesUtlatandeV4 utlatande, List<ValidationMessage> validationMessages) {
    final var hypoglykemi = Objects.requireNonNull(utlatande.getHypoglykemi());
    if (hypoglykemi.getRegelbundnaBlodsockerkontroller() == null) {
      addValidationErrorWithQuestionId(
          validationMessages,
          HYPOGLYKEMI_CATEGORY_ID,
          HYPOGLYKEMI_CATEGORY_ID + "." + HYPOGLYKEMI_REGELBUNDNA_BLODSOCKERKONTROLLER_JSON_ID,
          ValidationMessageType.EMPTY,
          HYPOGLYKEMI_REGELBUNDNA_BLODSOCKERKONTROLLER_SVAR_ID);
    }
  }

  private void validateKomplikationerAvSjukdomen(
      TsDiabetesUtlatandeV4 utlatande, List<ValidationMessage> validationMessages) {
    ...
    if (ovrigt.getKomplikationerAvSjukdomen() == null) {
      addValidationErrorWithQuestionId(
          validationMessages,
          OVRIGT_CATEGORY_ID,
          OVRIGT_CATEGORY_ID + "." + OVRIGT_KOMPLIKATIONER_AV_SJUKDOMEN_JSON_ID,
          ValidationMessageType.EMPTY,
          OVRIGT_KOMPLIKATIONER_AV_SJUKDOMEN_SVAR_ID);
      return;
    }

    if (eligibleForRule29(utlatande)
        && Strings.nullToEmpty(ovrigt.getKomplikationerAvSjukdomenAnges()).trim().isEmpty()) {
      addValidationErrorWithQuestionId(
          validationMessages,
          OVRIGT_CATEGORY_ID,
          OVRIGT_CATEGORY_ID + "." + OVRIGT_KOMPLIKATIONER_AV_SJUKDOMEN_ANGES_JSON_ID,
          ValidationMessageType.EMPTY,
          OVRIGT_KOMPLIKATIONER_AV_SJUKDOMEN_ANGES_DELSVAR_ID);
    } else if (eligibleForRule29(utlatande)
        && Strings.nullToEmpty(ovrigt.getKomplikationerAvSjukdomenAnges()).trim().length()
            > MAX_HUNDRED_EIGHTY_NINE_CHARS) {
      addValidationErrorWithQuestionId(
          validationMessages,
          OVRIGT_CATEGORY_ID,
          OVRIGT_CATEGORY_ID + "." + OVRIGT_KOMPLIKATIONER_AV_SJUKDOMEN_ANGES_JSON_ID,
  ...
  private void validateBorBedomasAvSpecialist(
      TsDiabetesUtlatandeV4 utlatande, List<ValidationMessage> validationMessages) {
    final var shouldBeExaminedBySpecialist =
        Objects.requireNonNull(utlatande.getOvrigt()).getBorUndersokasAvSpecialist();
    if (shouldBeExaminedBySpecialist != null
        && shouldBeExaminedBySpecialist.length() > MAX_SEVENTY_ONE_CHARS) {
      addValidationErrorWithQuestionId(
          validationMessages,
          OVRIGT_CATEGORY_ID,
          OVRIGT_CATEGORY_ID + "." + OVRIGT_BOR_UNDERSOKAS_AV_SPECIALIST_JSON_ID,
          ValidationMessageType.OTHER,
          OVRIGT_BOR_UNDERSOKAS_AV_SPECIALIST_SVAR_ID);
    }
  }

  private void validateUppfyllerBehorighetskrav(
      TsDiabetesUtlatandeV4 utlatande, List<ValidationMessage> validationMessages) {
    ...
    final var selectedBehorigheter =
        getLicenceTypes(utlatande.getBedomning().getUppfyllerBehorighetskrav());
    if (selectedBehorigheter.contains(KorkortsbehorighetKod.KANINTETASTALLNING)
        && selectedBehorigheter.size() > 1) {
      addValidationErrorWithQuestionId(
          validationMessages,
          BEDOMNING_CATEGORY_ID,
          BEDOMNING_CATEGORY_ID + "." + BEDOMNING_UPPFYLLER_BEHORIGHETSKRAV_JSON_ID,
  ...
  private void validateOvrigaKommentarer(
      TsDiabetesUtlatandeV4 utlatande, List<ValidationMessage> validationMessages) {
    final var commentsAndInfo =
        Objects.requireNonNull(utlatande.getBedomning()).getOvrigaKommentarer();
    if (commentsAndInfo != null && commentsAndInfo.length() > MAX_HUNDRED_EIGHTY_NINE_CHARS) {
      addValidationErrorWithQuestionId(
          validationMessages,
          BEDOMNING_CATEGORY_ID,
          BEDOMNING_CATEGORY_ID + "." + BEDOMNING_OVRIGA_KOMMENTARER_JSON_ID,
          ValidationMessageType.OTHER,
  ...
  private static <T extends Enum<?>> Set<KorkortsbehorighetKod> getLicenceTypes(Set<T> enumCodes) {
    ...
    return enumCodes.stream()
        .map(code -> KorkortsbehorighetKod.fromCode(code.name()))
        .collect(ImmutableSet.toImmutableSet());
  }

  private void validateDateWithinIntervalWithQuestionId(
      InternalDate dateToValidate,
      LocalDate notBeforeDate,
      LocalDate notAfterDate,
    ...
    if (dateToValidate == null) {
      addValidationErrorWithQuestionId(
          validationMessages, category, field, ValidationMessageType.EMPTY, questionId);
      return;
    }
    ...
    if (!dateToValidate.isValidDate()) {
      if (dateToValidate.isCorrectFormat()) {
        ...
        addValidationErrorWithQuestionId(
            validationMessages, category, field, ValidationMessageType.INVALID_FORMAT, questionId);
      }
      return;
    }
    ...
    if (dateToValidateLocalDate.isAfter(notAfterDate)) {
      addValidationErrorWithQuestionId(
          ...
          field,
          ValidationMessageType.OTHER,
          InternalDraftValidatorImpl.D_08,
          questionId);
    }
  }
}
