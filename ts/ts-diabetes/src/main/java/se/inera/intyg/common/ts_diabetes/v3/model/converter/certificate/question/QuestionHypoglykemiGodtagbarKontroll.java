...
public class QuestionHypoglykemiGodtagbarKontroll {

  public static CertificateDataElement toCertificate(
      Hypoglykemier hypoglykemier, int index, CertificateTextProvider textProvider) {
    final var kontroll = hypoglykemier != null ? hypoglykemier.getSjukdomenUnderKontroll() : null;
    return CertificateDataElement.builder()
        .id(HYPOGLYKEMI_GODTAGBAR_KONTROLL_SVAR_ID)
        .parent(HYPOGLYKEMI_CATEGORY_ID)
        .index(index)
        .config(
            CertificateDataConfigViewText.builder()
                .text(textProvider.get(HYPOGLYKEMI_GODTAGBAR_KONTROLL_SVAR_TEXT_ID))
                .build())
        .value(CertificateDataValueViewText.builder().text(booleanValue(kontroll)).build())
        .build();
  }
}
