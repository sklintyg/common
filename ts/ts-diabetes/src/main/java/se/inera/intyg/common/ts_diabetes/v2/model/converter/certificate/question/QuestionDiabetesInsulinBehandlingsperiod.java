...
import static se.inera.intyg.common.ts_diabetes.v2.model.converter.RespConstants.ALLMANT_BEHANDLING_INSULIN_TEXT_ID;
...
public class QuestionDiabetesInsulinBehandlingsperiod {

    public static CertificateDataElement toCertificate(String behandlingsperiod, int index, CertificateTextProvider textProvider) {
        return CertificateDataElement.builder()
            .id(ALLMANT_BEHANDLING_INSULIN_DELSVAR_ID)
            .parent(ALLMANT_BEHANDLING_SVAR_ID)
            .index(index)
            .config(
                CertificateDataConfigViewText.builder()
                    .text(textProvider.get(ALLMANT_BEHANDLING_INSULIN_TEXT_ID))
                    .build()
            )
            .value(
                CertificateDataValueViewText.builder()
                    .text(stringValue(behandlingsperiod))
                    .build()
            )
            .build();
    }
