 ...
 */
angular.module('afmu').controller('afmu.CustomizePdfCtrl',
    [ '$window', '$location', '$log', '$rootScope', '$state', '$stateParams', '$scope','afmu.customizeViewstate', 'common.IntygListService',
        ...
        function($window, $location, $log, $rootScope, $state, $stateParams, $scope, customizeViewstate, listCertService, certificateService, dialogService, messageService) {
            ...
            $scope.downloadAsPdfLink = '/moduleapi/certificate/afmu' + '/' + $stateParams.certificateId + '/pdf/arbetsgivarutskrift';

            certificateService.getCertificate('afmu', $stateParams.certificateId, function(result) {
                ...
                if (result !== null) {
                    ...
                    // show error view
                    $location.path('/afmu/visafel/certnotfound');
                }
            }, function() {
                $log.debug('got error');
                $location.path('/afmu/visafel/certnotfound');
            ...
            $scope.$on('$stateChangeStart', function(event, toState, toParams, fromState, fromParams, options) {
                if (toState.name.indexOf('afmu-customize')!==-1 || leaveConfirmed) {
                    return;
                }
            ...
            $scope.backToViewCertificate = function() {
                ...
                $location.path('/afmu/view/' + $stateParams.certificateId);
            };
