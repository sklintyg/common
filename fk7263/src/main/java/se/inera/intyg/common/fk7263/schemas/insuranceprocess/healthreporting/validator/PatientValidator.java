...
public final class PatientValidator {
  ...
  private static final List<String> PATIENT_ID_OIDS =
      asList(Constants.PERSON_ID_OID, Constants.SAMORDNING_ID_OID);
  ...
  public static List<String> validateAndCorrect(PatientType patient) {
    ...
    if (Pattern.matches(PERSON_NUMBER_WITHOUT_DASH_REGEX, personNumber)) {
      ...
      LOG.warn(
          ...
          "Validation warning: Person-id "
              + personNumber
              + " is lacking a separating dash - corrected.");
    }
    // Check patient o.i.d.
    if (patient.getPersonId().getRoot() == null
        || !PATIENT_ID_OIDS.contains(patient.getPersonId().getRoot())) {
      validationErrors.add(
          String.format(
              "Wrong o.i.d. for Patient Id! Should be %s",
              Joiner.on(" or ").join(PATIENT_ID_OIDS)));
    }
