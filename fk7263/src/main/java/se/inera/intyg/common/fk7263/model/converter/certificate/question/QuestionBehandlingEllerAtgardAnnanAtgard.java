...
public class QuestionBehandlingEllerAtgardAnnanAtgard {

    public static CertificateDataElement toCertificate(String annanAtgard, int index, CertificateMessagesProvider messagesProvider) {
        return CertificateDataElement.builder()
            .id(ANNAN_ATGARD_DELSVAR_ID)
            .parent(BEHANDLING_ELLER_ATGARD_CATEGORY_ID)
            .index(index)
            .config(
                CertificateDataConfigViewText.builder()
                    .text(messagesProvider.get(ANNAN_ATGARD_DELSVAR_TEXT_ID))
                    .build()
            )
            .value(
                CertificateDataValueViewText.builder()
                    .text(stringValue(annanAtgard))
                    .build()
            )
            .build();
    }
