...
public class QuestionBehandlingEllerAtgardSjukvard {

    public static CertificateDataElement toCertificate(String sjukvard, int index, CertificateMessagesProvider messagesProvider) {
        return CertificateDataElement.builder()
            .id(INOM_SJUKVARD_DELSVAR_ID)
            .parent(BEHANDLING_ELLER_ATGARD_CATEGORY_ID)
            .index(index)
            .config(
                CertificateDataConfigViewText.builder()
                    .text(messagesProvider.get(INOM_SJUKVARD_DELSVAR_TEXT_ID))
                    .build()
            )
            .value(
                CertificateDataValueViewText.builder()
                    .text(stringValue(sjukvard))
                    .build()
            )
            .build();
    }
