...
public class InternalDraftValidator {
  ...
  private void validateVardkontakter(
      Fk7263Utlatande utlatande, List<ValidationMessage> validationMessages) {
  ...
  private void validateReferenser(
      Fk7263Utlatande utlatande, List<ValidationMessage> validationMessages) {
    ...
    if (!utlatande.isAvstangningSmittskydd()) {
      ...
      if (utlatande.getUndersokningAvPatienten() == null
          ...
          && utlatande.getAnnanReferens() == null) {
        ValidatorUtil.addValidationError(
            validationMessages, "intygbaseratpa", "intygbaseratpa", ValidationMessageType.EMPTY);
      }
    }

    if (utlatande.getAnnanReferens() != null) {
      ValidatorUtil.validateDate(
          utlatande.getAnnanReferens(), validationMessages, "intygbaseratpa", "referenser", null);
    }
    if (utlatande.getAnnanReferens() != null
        && Strings.nullToEmpty(utlatande.getAnnanReferensBeskrivning()).trim().isEmpty()) {
      ValidatorUtil.addValidationError(
          validationMessages, "intygbaseratpa", "annat", ValidationMessageType.EMPTY);
    }
  ...
  private void validateKommentar(
    ...
    if (utlatande.getPrognosBedomning() == PrognosBedomning.arbetsformagaPrognosGarInteAttBedoma
        && Strings.nullToEmpty(utlatande.getArbetsformagaPrognosGarInteAttBedomaBeskrivning())
            .trim()
            .isEmpty()) {
      ValidatorUtil.addValidationError(
          validationMessages,
  ...
  private void validateArbetsformaga(
    ...
    if (!utlatande.isAvstangningSmittskydd()) {
      if (!utlatande.isNuvarandeArbete()
          && !utlatande.isArbetsloshet()
          && !utlatande.isForaldrarledighet()) {
        ValidatorUtil.addValidationError(
            validationMessages, "sysselsattning", "sysselsattning", ValidationMessageType.EMPTY);
      } else if (utlatande.isNuvarandeArbete()
          && Strings.nullToEmpty(utlatande.getNuvarandeArbetsuppgifter()).trim().isEmpty()) {
        ValidatorUtil.addValidationError(
            validationMessages,
            "sysselsattning",
    ...
    if (isValidDateInIntervals(validationMessages, utlatande)) {
      validateIntervals(
          ...
          "nedsattning",
          "arbetsformaga",
          utlatande.getNedsattMed100(),
          utlatande.getNedsattMed75(),
          utlatande.getNedsattMed50(),
          utlatande.getNedsattMed25());
    }
  }

  private boolean isValidDateInIntervals(
      List<ValidationMessage> validationMessages, Fk7263Utlatande utlatande) {
    ...
    if (ValidatorUtil.allNulls(intervals)) {
      ValidatorUtil.addValidationError(
          validationMessages, "nedsattning", "arbetsformaga", ValidationMessageType.EMPTY);
      return false;
    }
    // if the interval is not null and either from or tom is invalid, raise validation error
    // use independent conditions to check this to be able to give specific validation errors for
    // each case
    if (intervals[nedsattmed100Index] != null) {
      success &=
          ValidatorUtil.validateInternalDateInterval(
              intervals[nedsattmed100Index],
              validationMessages,
              "nedsattning",
              "nedsattMed100",
  ...
  private void validateAktivitetsbegransning(
    ...
    if (!utlatande.isAvstangningSmittskydd()
        && Strings.nullToEmpty(aktivitetsbegransning).trim().isEmpty()) {
      ValidatorUtil.addValidationError(
          validationMessages,
  ...
  private void validateFunktionsnedsattning(
    ...
    if (!utlatande.isAvstangningSmittskydd()
        && Strings.nullToEmpty(funktionsnedsattning).trim().isEmpty()) {
      ValidatorUtil.addValidationError(
          validationMessages,
  ...
  private void validateDiagnose(
    ...
    if (!Strings.nullToEmpty(utlatande.getDiagnosKod()).trim().isEmpty()) {
      ...
      validateDiagnosKod(
          utlatande.getDiagnosKod(),
          kodsystem,
          "diagnos.diagnosKod",
          "fk7263.validation.diagnos.invalid",
          validationMessages);
    } else {
      ValidatorUtil.addValidationError(
          validationMessages,
    ...
    if (!Strings.nullToEmpty(utlatande.getDiagnosKod2()).trim().isEmpty()) {
      ...
      validateDiagnosKod(
          utlatande.getDiagnosKod2(),
          kodsystem,
          "diagnos.diagnosKod2",
          "fk7263.validation.diagnos2.invalid",
          validationMessages);
    }
    ...
    if (!Strings.nullToEmpty(utlatande.getDiagnosKod3()).trim().isEmpty()) {
      ...
      validateDiagnosKod(
          utlatande.getDiagnosKod3(),
          kodsystem,
          "diagnosKod3",
          "fk7263.validation.diagnos3.invalid",
          validationMessages);
    }
  }

  private void validateDiagnosKod(
      String diagnosKod,
      String kodsystem,
      String field,
      String msgKey,
    ...
    if (moduleService == null) {
      LOG.warn(
          "Forced to skip validation of diagnosKod since an implementation of ModuleService is not available");
      return;
    }

    if (!moduleService.validateDiagnosisCode(diagnosKod, kodsystem)) {
      ValidatorUtil.addValidationError(
          validationMessages, "diagnos", field, ValidationMessageType.INVALID_FORMAT, msgKey);
    }
  }
  ...
  private void validateOvrigaRekommendationer(
    ...
    if (utlatande.isRekommendationOvrigtCheck()
        && Strings.nullToEmpty(utlatande.getRekommendationOvrigt()).trim().isEmpty()) {
      ValidatorUtil.addValidationError(
          validationMessages,
          "rekommendationer",
   ...
   * @return booleans
   */
  protected boolean validateIntervals(
      List<ValidationMessage> validationMessages,
      String categoryId,
      String fieldId,
      InternalLocalDateInterval... intervals) {
