...
/**
 * Converts se.inera.certificate.fk7263.insuranceprocess.healthreporting.mu7263.v3.Lakarutlatande
 * Jaxb structure to External model.
 *
 * @author marced
 */
@Component
public final class TransportToInternal {
    ...
    for (FunktionstillstandType funktionstillstand : source.getFunktionstillstand()) {
      if (funktionstillstand.getArbetsformaga() != null) {
        ...
        if (funktionstillstand.getArbetsformaga().getSysselsattning() != null) {
          populateWithSysselsattning(
              utlatande, funktionstillstand.getArbetsformaga().getSysselsattning());
        }

        if (utlatande.isNuvarandeArbete()
            && funktionstillstand.getArbetsformaga().getArbetsuppgift() != null) {
          utlatande.setNuvarandeArbetsuppgifter(
              funktionstillstand.getArbetsformaga().getArbetsuppgift().getTypAvArbetsuppgift());
        }
  ...
  public static LocalDate getValidToDate(List<ArbetsformagaNedsattningType> source) {
    ...
    for (ArbetsformagaNedsattningType nedsattning : source) {
      LocalDateInterval nextObservationsperiod =
          new LocalDateInterval(nedsattning.getVaraktighetFrom(), nedsattning.getVaraktighetTom());
  ...
  private static void populateWithReferens(Fk7263Utlatande utlatande, ReferensType source)
  ...
  private static void populateWithAktivitetType(Fk7263Utlatande utlatande, AktivitetType source)
      throws ConverterException {
  ...
  private static void populateWithSysselsattning(
      Fk7263Utlatande utlatande, List<SysselsattningType> sysselsattnings)
      throws ConverterException {
  ...
  private static void populateWithDiagnose(
      Fk7263Utlatande utlatande, MedicinsktTillstandType source) throws ConverterException {
    if (source == null) {
      return;
    }
    utlatande.setDiagnosBeskrivning(source.getBeskrivning());
    utlatande.setDiagnosKod(source.getTillstandskod().getCode());
    Diagnoskodverk kodverk =
        Diagnoskodverk.getEnumByCodeSystem(source.getTillstandskod().getCodeSystem());
  ...
  /**
   * Create IntyMetadata object with information regarding Patient, SkapadAv and Skickat- and
   * SkapatDatum.
   *
  ...
  private static GrundData populateWithMetaData(LakarutlatandeType source) {
    ...
    metadata.setSkapadAv(
        convertSkapadAv(source.getSkapadAvHosPersonal(), source.getSigneringsdatum()));
    return metadata;
  }
   ...
   * @return HoSPersonal
   */
  private static HoSPersonal convertSkapadAv(
      HosPersonalType source, LocalDateTime signeringsDatum) {
  ...
  private MappedUnit getMappedUnitConfiguration(
      VardgivareType sourceCareProvider,
  ...
  private static void populateWithFunktionstillstand(
      Fk7263Utlatande utlatande, FunktionstillstandType source) throws ConverterException {
  ...
  private static void populateWithPrognos(Fk7263Utlatande utlatande, ArbetsformagaType source)
      throws ConverterException {
  ...
  /**
   * Convert nedsattning to formaga. Note: kind of backwards, but meaning is opposite in transport
   * model vs domain model..
   *
   * @param source source
   */
  private static void populateWithArbetsformaga(
      Fk7263Utlatande utlatande, FunktionstillstandType source) {
    for (ArbetsformagaNedsattningType nedsattning :
        source.getArbetsformaga().getArbetsformagaNedsattning()) {
      if (nedsattning.getNedsattningsgrad() != null
          && nedsattning.getVaraktighetFrom() != null
