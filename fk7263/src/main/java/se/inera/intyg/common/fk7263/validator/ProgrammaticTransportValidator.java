...
/**
 * Validates presence and validity of formal external model properties such as OID's. Preferably
 * this validation should have been implemented using Schema Validation, but since this is not an
 * option we perform this pre-validation in code. more specific business rules should subsequently
 * validated in {@see UtlatandeValidator}
 *
 * @author marced
 */
public class ProgrammaticTransportValidator extends AbstractValidator {
  ...
  private void validateAktuelltSjukdomsforlopp() {
    if (utlatande.getBedomtTillstand() != null
        && utlatande.getBedomtTillstand().getBeskrivning() == null) {
  ...
  private void validateRekommendationer() {
    ...
    AktivitetType atgardSjukvarden =
        findAktivitetWithCode(
            utlatande.getAktivitet(),
    ...
    if (atgardSjukvarden != null && atgardSjukvarden.getBeskrivning() == null) {
      addValidationError(
          "Beskrivning must be set for Aktivitet Rekommendation Planerad eller pågående åtgärd inom sjukvården");
    }

    AktivitetType atgardAnnan =
        findAktivitetWithCode(
            utlatande.getAktivitet(), Aktivitetskod.PLANERAD_ELLER_PAGAENDE_ANNAN_ATGARD);
    if (atgardAnnan != null && atgardAnnan.getBeskrivning() == null) {
      addValidationError(
          "Beskrivning must be set for Aktivitet Rekommendation Planerad eller pågående annan atgärd");
    }
  }
  ...
  private void validatePatient() {
    ...
    //        if (inPatient != null && (inPatient.getFullstandigtNamn() == null ||
    // inPatient.getFullstandigtNamn().length() < 1)) {
  ...
  private void validateHosPersonalEnhet(HosPersonalType inHoSP) {
    ...
    if (inEnhet.getArbetsplatskod() == null) {
      ...
      if (!inEnhet.getArbetsplatskod().getRoot().equalsIgnoreCase(Constants.ARBETSPLATS_KOD_OID)) {
        addValidationError(
            "Wrong o.i.d for arbetsplatskod, should be " + Constants.ARBETSPLATS_KOD_OID);
      }
  ...
  private void validateVardgivare(EnhetType inEnhet) {
    ...
    if (inVardgivare.getVardgivareId() == null
        || isNullOrEmpty(inVardgivare.getVardgivareId().getExtension())) {
  ...
  private void validateSmittskyddRelated() {
    ...
    inSmittskydd =
        findAktivitetWithCode(
                    utlatande.getAktivitet(), Aktivitetskod.AVSTANGNING_ENLIGT_SM_L_PGA_SMITTA)
                != null
            ? true
            : false;
    ...
    // information
    FunktionstillstandType inAktivitetFunktion =
        findFunktionsTillstandType(
    ...
    if (!inSmittskydd) {
      ...
      if (medTillstand.getTillstandskod() == null
          || isNullOrEmpty(medTillstand.getTillstandskod().getCode())) {
      ...
      if (medTillstand.getTillstandskod() == null
          ...
          || !(medTillstand
                  .getTillstandskod()
                  .getCodeSystemName()
                  .equalsIgnoreCase(Diagnoskodverk.ICD_10_SE.getCodeSystemName())
              || medTillstand
                  .getTillstandskod()
                  .getCodeSystemName()
      ...
      FunktionstillstandType inKroppsFunktion =
          findFunktionsTillstandType(
      ...
      VardkontaktType inUndersokning =
          findVardkontaktTyp(
              utlatande.getVardkontakt(), Vardkontakttyp.MIN_UNDERSOKNING_AV_PATIENTEN);

      // Fält 4 - höger näst översta kryssrutan
      VardkontaktType telefonkontakt =
          findVardkontaktTyp(
              utlatande.getVardkontakt(), Vardkontakttyp.MIN_TELEFONKONTAKT_MED_PATIENTEN);
      ...
      if (inUndersokning != null && inUndersokning.getVardkontaktstid() == null) {
        addValidationError(
            "No or wrong date for vardkontakt - min undersokning av patienten found!");
      }
  ...
  private void validateSysselsattning() {
    ...
    // Fält 8a - Check that we got a arbetsformaga element, needs to be present even if smittskydd
    // is set.
    ...
    // Fält 8a
    SysselsattningType inArbete =
        findTypAvSysselsattning(
            inAktivitetFunktion.getArbetsformaga().getSysselsattning(),
            TypAvSysselsattning.NUVARANDE_ARBETE);
    SysselsattningType inArbetslos =
        findTypAvSysselsattning(
            inAktivitetFunktion.getArbetsformaga().getSysselsattning(),
            TypAvSysselsattning.ARBETSLOSHET);
    SysselsattningType inForaldraledig =
        findTypAvSysselsattning(
            inAktivitetFunktion.getArbetsformaga().getSysselsattning(),
    ...
    ArbetsuppgiftType inArbetsBeskrivning =
        inAktivitetFunktion.getArbetsformaga().getArbetsuppgift();
  ...
  private void validateNedsattning() {
    ...
    // Fält 8b - kryssruta 1
    ArbetsformagaNedsattningType nedsatt14del =
        findArbetsformaga(
            inAktivitetFunktion.getArbetsformaga().getArbetsformagaNedsattning(),
            Nedsattningsgrad.NEDSATT_MED_1_4);

    // Fält 8b - kryssruta 2
    ArbetsformagaNedsattningType nedsatthalften =
        findArbetsformaga(
            inAktivitetFunktion.getArbetsformaga().getArbetsformagaNedsattning(),
            Nedsattningsgrad.NEDSATT_MED_1_2);

    // Fält 8b - kryssruta 3
    ArbetsformagaNedsattningType nedsatt34delar =
        findArbetsformaga(
            inAktivitetFunktion.getArbetsformaga().getArbetsformagaNedsattning(),
            Nedsattningsgrad.NEDSATT_MED_3_4);

    // Fält 8b - kryssruta 4
    ArbetsformagaNedsattningType heltNedsatt =
        findArbetsformaga(
            inAktivitetFunktion.getArbetsformaga().getArbetsformagaNedsattning(),
  ...
  private void validatePrognosAngivelse() {
    ...
    if (inAktivitetFunktion != null && inAktivitetFunktion.getArbetsformaga() != null) {
      ...
      if (inAktivitetFunktion.getArbetsformaga().getPrognosangivelse() != null) {
        inArbetsformagaAterstallasHelt =
            inAktivitetFunktion
                    .getArbetsformaga()
                    .getPrognosangivelse()
                    .compareTo(Prognosangivelse.ATERSTALLAS_HELT)
                == 0;
        inArbetsformagaAterstallasDelvis =
            inAktivitetFunktion
                    .getArbetsformaga()
                    .getPrognosangivelse()
                    .compareTo(Prognosangivelse.ATERSTALLAS_DELVIS)
                == 0;
        inArbetsformagaEjAterstallas =
            inAktivitetFunktion
                    .getArbetsformaga()
                    .getPrognosangivelse()
                    .compareTo(Prognosangivelse.INTE_ATERSTALLAS)
                == 0;
        inArbetsformagaGarEjAttBedomma =
            inAktivitetFunktion
                    .getArbetsformaga()
                    .getPrognosangivelse()
                    .compareTo(Prognosangivelse.DET_GAR_INTE_ATT_BEDOMMA)
                == 0;
      }
  ...
  private void validateRessatt() {
    // Fält 11 - optional
    AktivitetType inForandratRessatt =
        findAktivitetWithCode(
            utlatande.getAktivitet(),
            Aktivitetskod.FORANDRAT_RESSATT_TILL_ARBETSPLATSEN_AR_AKTUELLT);
    AktivitetType inEjForandratRessatt =
        findAktivitetWithCode(
            utlatande.getAktivitet(),
  ...
  private void validateKommentar() {
    ...
    if ((inAnnat != null || inArbetsformagaGarEjAttBedomma)
        && isNullOrEmpty(utlatande.getKommentar())) {
      addValidationError("Upplysningar should contain data as field 4 or fields 10 is checked.");
    }
  }

  // Helper methods
  protected static AktivitetType findAktivitetWithCode(
      List<AktivitetType> aktiviteter, Aktivitetskod aktivitetskod) {
  ...
  protected static FunktionstillstandType findFunktionsTillstandType(
      List<FunktionstillstandType> funktionstillstand,
    ...
    if (funktionstillstand != null) {
      for (int i = 0; i < funktionstillstand.size(); i++) {
        ...
        if (listFunktionstillstand.getTypAvFunktionstillstand() != null
            && listFunktionstillstand.getTypAvFunktionstillstand().compareTo(funktionstillstandsTyp)
                == 0) {
  ...
  protected static VardkontaktType findVardkontaktTyp(
      List<VardkontaktType> vardkontakter, Vardkontakttyp vardkontaktTyp) {
  ...
  protected static ReferensType findReferensTyp(
      List<ReferensType> referenser, Referenstyp referensTyp) {
  ...
  protected static SysselsattningType findTypAvSysselsattning(
      List<SysselsattningType> sysselsattning, TypAvSysselsattning sysselsattningsTyp) {
  ...
  protected static ArbetsformagaNedsattningType findArbetsformaga(
      List<ArbetsformagaNedsattningType> arbetsformaga,
    ...
    if (arbetsformaga != null) {
      for (int i = 0; i < arbetsformaga.size(); i++) {
        ...
        if (listArbetsformaga.getNedsattningsgrad() != null
            && listArbetsformaga.getNedsattningsgrad().compareTo(arbetsformagaNedsattningTyp)
                == 0) {
