...
public class InternalToCertificate {
  ...
  public static Certificate convert(
      Fk7263Utlatande internalCertificate, CertificateMessagesProvider messagesProvider) {
    int index = 0;
    return CertificateBuilder.create()
        .metadata(MetaDataGrundData.toCertificate(internalCertificate))
        .addElement(CategoryAvstangningSmittskydd.toCertificate(index++, messagesProvider))
        .addElement(
            QuestionAvstangningSmittskydd.toCertificate(
                internalCertificate.isAvstangningSmittskydd(), index++))
        .addElement(CategoryDiagnos.toCertificate(index++, messagesProvider))
        .addElement(
            QuestionHuvuddiagnoskod.toCertificate(
                internalCertificate.getDiagnosKod(), index++, messagesProvider))
        .addElement(
            QuestionFortydligandeDiagnos.toCertificate(
                internalCertificate.getDiagnosBeskrivning(), index++, messagesProvider))
        .addElement(CategorySjukdomforlopp.toCertificate(index++, messagesProvider))
        .addElement(
            QuestionSjukdomsforlopp.toCertificate(
                internalCertificate.getSjukdomsforlopp(), index++))
        .addElement(CategoryFunktionsnedsattning.toCertificate(index++, messagesProvider))
        .addElement(
            QuestionFunktionsnedsattning.toCertificate(
                internalCertificate.getFunktionsnedsattning(), index++))
        .addElement(CategoryIntygetBaserasPa.toCertificate(index++, messagesProvider))
        .addElement(
            QuestionIntygetBaserasPaUndersokning.toCertificate(
                internalCertificate.getUndersokningAvPatienten(), index++))
        .addElement(
            QuestionIntygetBaserasPaTelefonkontakt.toCertificate(
                internalCertificate.getTelefonkontaktMedPatienten(), index++))
        .addElement(
            QuestionIntygetBaserasPaJournaluppgifter.toCertificate(
                internalCertificate.getJournaluppgifter(), index++))
        .addElement(
            QuestionIntygetBaserasPaAnnat.toCertificate(
                internalCertificate.getAnnanReferens(), index++))
        .conditionalAddElement(
            QuestionAnnanReferensBeskrivning.toCertificate(
                internalCertificate.getAnnanReferensBeskrivning(), index++),
            internalCertificate.getAnnanReferensBeskrivning() != null
                && !internalCertificate.getAnnanReferensBeskrivning().isEmpty())
        .addElement(CategoryAktivitetsbegransningar.toCertificate(index++, messagesProvider))
        .addElement(
            QuestionAktivitetsbegransningar.toCertificate(
                internalCertificate.getAktivitetsbegransning(), index++))
        .addElement(CategoryRekommendationer.toCertificate(index++, messagesProvider))
        .addElement(
            QuestionRekommendationerKontaktMedAf.toCertificate(
                internalCertificate.isRekommendationKontaktArbetsformedlingen(),
                index++,
                messagesProvider))
        .addElement(
            QuestionRekommendationerKontaktMedFhv.toCertificate(
                internalCertificate.isRekommendationKontaktForetagshalsovarden(),
                index++,
                messagesProvider))
        .addElement(
            QuestionRekommendationerOvrigt.toCertificate(
                internalCertificate.getRekommendationOvrigt(), index++, messagesProvider))
        .addElement(CategoryBehandlingEllerAtgard.toCertificate(index++, messagesProvider))
        .addElement(
            QuestionBehandlingEllerAtgardSjukvard.toCertificate(
                internalCertificate.getAtgardInomSjukvarden(), index++, messagesProvider))
        .addElement(
            QuestionBehandlingEllerAtgardAnnanAtgard.toCertificate(
                internalCertificate.getAnnanAtgard(), index++, messagesProvider))
        .addElement(
            CategoryArbetslivsinriktadRehabilitering.toCertificate(index++, messagesProvider))
        .addElement(
            QuestionArbetslivsinriktadRehabilitering.toCertificate(
                internalCertificate.getRehabilitering(), index++))
        .addElement(CategoryPatientensArbetsformagaBedoms.toCertificate(index++, messagesProvider))
        .addElement(
            QuestionPatientensArbetsformogaBedoms.toCertificate(
                internalCertificate.isNuvarandeArbete(),
                internalCertificate.isForaldrarledighet(),
                internalCertificate.isArbetsloshet(),
                internalCertificate.getNuvarandeArbetsuppgifter(),
                index++,
                messagesProvider))
        .addElement(CategoryArbetsformogaBedomning.toCertificate(index++, messagesProvider))
        .addElement(
            QuestionArbetsformogaBedomning.toCertificate(
                internalCertificate.getNedsattMed25(),
                internalCertificate.getNedsattMed50(),
                internalCertificate.getNedsattMed75(),
                internalCertificate.getNedsattMed100(),
                index++,
                messagesProvider))
        .addElement(
            CategoryArbetsformogaForsakringsmedicinskaBeslutstodet.toCertificate(
        ...
        .addElement(CategoryArbetsformogaPrognos.toCertificate(index++, messagesProvider))
        .addElement(
            QuestionArbetsformogaPrognos.toCertificate(
                internalCertificate.getPrognosBedomning(), index++))
        .addElement(
            QuestionArbetsformogaGarInteAttBedomaBeskrivning.toCertificate(
                internalCertificate.getArbetsformagaPrognosGarInteAttBedomaBeskrivning(),
                index++,
                messagesProvider))
        .addElement(CategoryResorTillFranArbete.toCertificate(index++, messagesProvider))
        .addElement(
            QuestionResorTillFranArbete.toCertificate(
                internalCertificate.isRessattTillArbeteAktuellt(), index++))
        .addElement(CategoryKontaktMedFk.toCertificate(index++, messagesProvider))
        .addElement(
            QuestionKontaktMedFk.toCertificate(internalCertificate.isKontaktMedFk(), index++))
        .addElement(CategoryOvrigaUpplysningar.toCertificate(index++, messagesProvider))
        .addElement(
            QuestionOvrigaUpplysningar.toCertificate(internalCertificate.getKommentar(), index++))
        .addElement(CategoryForskrivarkodOchArbetsplatskod.toCertificate(index++, messagesProvider))
        .addElement(
            QuestionForskrivarkodOchArbetsplatskod.toCertificate(
                internalCertificate.getForskrivarkodOchArbetsplatskod(), index))
        .build();
  }
}
