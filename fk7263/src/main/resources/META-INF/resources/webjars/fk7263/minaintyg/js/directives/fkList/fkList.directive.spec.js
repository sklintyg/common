...
xdescribe('fk7263List Directive', function() {
    'use strict';
    ...
    it('should display label when value exists', function() {
        ...
        //expect(element.isolateScope().values.count()).toBe(2);
        expect(element.isolateScope().hasValue()).toBeTruthy();
        expect($(element).find('#property1').text()).toContain('translationKey');

        expect($(element).find('#property2').text()).toContain('translationKey');
    ...
    it('should display "uv-no-value" when no value exists', function() {
        $scope.viewDataMock.property1.name = null;
        $scope.$digest();

        expect(element.isolateScope().values.count()).toBe(0);
