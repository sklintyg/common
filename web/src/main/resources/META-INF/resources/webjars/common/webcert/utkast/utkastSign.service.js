...
angular.module('common').factory('common.UtkastSignService',
    ['$rootScope', '$document', '$log', '$location', '$stateParams', '$timeout', '$window', '$q',
      ...
      function($rootScope, $document, $log, $location, $stateParams, $timeout, $window, $q,
        ...
        function _openNetIdPlugin(hash, intygsId, onSuccess, onError) {
          $timeout(function() {
            ...
            if (resultCode === 0) {
              ...
              var certifikat = getCertificate(intygsId);
        ...
        /**
         * Return the certificate used by Net iD to sign. If the certificate is null/empty, a monitoring log is made to backend including
         * the users Net iD version.
         */
        function getCertificate(intygsId) {
          var certificate = iid_GetProperty('Certificate'); // jshint ignore:line
          if (certificate === undefined || certificate === '') {
            var netIdVersion = iid_GetProperty('Version'); // jshint ignore:line
            monitoringService.signingCertificateMissing(intygsId, netIdVersion);
          }
          return certificate;
        }
