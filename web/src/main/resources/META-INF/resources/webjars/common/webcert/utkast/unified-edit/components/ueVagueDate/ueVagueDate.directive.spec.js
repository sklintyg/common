...
describe('vagueDate', function() {
    ...
    describe('Starting with empty date', function() {
    ...
    describe('Starting with year but empty month', function() {
        beforeEach(inject(function($compile, $rootScope) {
            $scope = $rootScope.$new();
            $scope.model = {
                date: '2017-00-00',
        ...
        it('If initialized with year and "00" month then "Ange år" and "Ange månad" should no longer be available', function() {
            expect($scope.years.length).toBe(3);   // "Ange år", "0000", this year and last year
            expect($scope.months.length).toBe(13); // "00" and 01-12

            expect($scope.model.date).toBe('2017-00-00');
        });
    });
});
