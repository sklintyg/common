...
/**
 * Renders an alert component consisting of a table where cell1 contains an icon and the other cell
 * contains the alert text.
 */
  ...
  public boolean render(Div parent, ScriptObjectMirror currentUvNode) {
    ...
    if (render) {
      ...
      AlertLevel alertLevel =
          AlertLevel.from((String) currentUvNode.get(ALERT_LEVEL_JSON_PROPERTY));
      PdfImageXObject observandumIcon =
          alertLevel.equals(AlertLevel.WARNING)
              ? renderer.getObservandumIcon()
              : renderer.getObservandumInfoIcon();

      AlertColors colors =
          AlertColors.from(renderer.getPrintConfig().getApplicationOrigin(), alertLevel);
          ...
          .orElseThrow(
              () ->
                  new RuntimeException(
                      "Undefined AlertColors for origin "
                          + origin
                          + " / alertLevel "
                          + alertLevel));
    }
