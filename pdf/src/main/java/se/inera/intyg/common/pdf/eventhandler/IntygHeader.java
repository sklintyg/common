...
public class IntygHeader implements IEventHandler {
  ...
  public IntygHeader(
      PrintConfig printConfig, PdfFont kategoriFont, PdfFont fragaDelFragaFont, PdfFont svarFont) {
    ...
    if (widthHeightRatio < WIDTH_SCALE_THRESHOLD) {
      ...
      final var rectangle =
          PdfXObject.calculateProportionallyFitRectangleWithHeight(
              logo, logoX, logoY, LOGOTYPE_MAX_HEIGHT);
      ...
      final var rectangle =
          PdfXObject.calculateProportionallyFitRectangleWithWidth(
              logo, logoX, logoY, LOGOTYPE_MAX_WIDTH);
      pdfCanvas.addXObjectFittedIntoRectangle(logo, rectangle);
    }
  }
  ...
  private void renderUtskriftsDatum(Rectangle pageSize, Canvas canvas) {
    ...
    canvas.showTextAligned(
        ...
        pageSize.getTop() - UTSKRIFTSDATUM_HEADER_Y_TOP_OFFSET,
        TextAlignment.RIGHT);
    canvas.setFont(svarFont).setFontSize(SVAR_FONT_SIZE);
    canvas.showTextAligned(
        LocalDate.now().format(DateTimeFormatter.ISO_DATE),
        pageSize.getWidth() - millimetersToPoints(PAGE_MARGIN_LEFT),
        pageSize.getTop() - UTSKRIFTSDATUM_VALUE_Y_TOP_OFFSET,
        TextAlignment.RIGHT);
  }

  private void renderPersonnummer(Rectangle pageSize, Canvas canvas) {
    float measuredUtskiftsDatum =
        measureTextWidth("Utskriftsdatum", SVAR_FONT_SIZE, fragaDelFragaFont);
    canvas.setFont(fragaDelFragaFont).setFontSize(SVAR_FONT_SIZE);
    canvas.showTextAligned(
        "Person- /samordningsnr",
        pageSize.getWidth()
            - (measuredUtskiftsDatum + DEFAULT_PADDING + millimetersToPoints(PAGE_MARGIN_LEFT)),
        pageSize.getTop() - UTSKRIFTSDATUM_HEADER_Y_TOP_OFFSET,
        TextAlignment.RIGHT);
    canvas.setFont(svarFont).setFontSize(SVAR_FONT_SIZE);
    canvas.showTextAligned(
        printConfig.getPersonnummer(),
        pageSize.getWidth()
            - (measuredUtskiftsDatum + DEFAULT_PADDING + millimetersToPoints(PAGE_MARGIN_LEFT)),
        pageSize.getTop() - UTSKRIFTSDATUM_VALUE_Y_TOP_OFFSET,
        TextAlignment.RIGHT);
  ...
  private void renderIntygNameAndCode(Rectangle pageSize, Canvas canvas) {
    ...
    canvas.showTextAligned(
        ...
        pageSize.getTop() - INTYG_NAME_Y_TOP_OFFSET,
        TextAlignment.LEFT);
    float calculatedWidth =
        measureTextWidth(printConfig.getIntygsNamn(), INTYG_FONT_SIZE, kategoriFont);
  ...
  private String getIntygsKod(PrintConfig printConfig) {
    return printConfig.getIntygsKod()
        + (Strings.isNullOrEmpty(printConfig.getIntygsVersion())
            ? ""
            : " " + printConfig.getIntygsVersion());
  }
  ...
  private void renderRedSquare(PdfPage page, Rectangle pageSize, Canvas canvas) {
    ...
    canvas.showTextAligned(
        paragraph,
        millimetersToPoints(PAGE_MARGIN_LEFT),
        pageSize.getTop() - RED_SQUARE_Y_TOP_OFFSET,
        TextAlignment.LEFT,
