...
public abstract class UVComponent {
      ...
      for (Map.Entry<String, Object> entry : valuesRoot.entrySet()) {
        ...
        if (entry.getValue() instanceof String) {
          ...
          throw new IllegalStateException(
              "Unhandled array type " + entry.getValue().getClass().getName());
        }
      }
    }
    return results;
  ...
  /**
   * Resolves the boolean value of the supplied modelProp and returns Ja/Nej or ERROR if not
   * boolean.
   */
  ...
  boolean handleModelPropOveride(Div parent, String modelProp) {
    if (this.renderer.getPrintConfig().getModelPropReplacements() != null) {
      String replacementValue =
          this.renderer.getPrintConfig().getModelPropReplacements().get(modelProp);
      if (replacementValue != null) {
        parent.add(
            new Paragraph(replacementValue)
                .setItalic()
