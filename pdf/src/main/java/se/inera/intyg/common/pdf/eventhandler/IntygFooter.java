  ...
  public void handleEvent(Event event) {
    ...
    try (Canvas canvas = new Canvas(pdfCanvas, pageSize)) {
      ...
      canvas.showTextAligned(
          "Utskriften skapades med " + appName + " - en tjänst som drivs av Inera AB\nwww.inera.se",
          millimetersToPoints(PAGE_MARGIN_LEFT),
          millimetersToPoints(pageSize.getBottom() + PADDING),
          TextAlignment.LEFT);

      pdfCanvas.moveTo(millimetersToPoints(PAGE_MARGIN_LEFT), LINE_Y_OFFSET);
      pdfCanvas.lineTo(
          page.getPageSize().getWidth() - millimetersToPoints(PAGE_MARGIN_LEFT), LINE_Y_OFFSET);
