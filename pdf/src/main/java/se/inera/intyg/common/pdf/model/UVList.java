 ...
 * string-based lookups in the model or by executing javascript functions defined in the uvConfig.
 */
public class UVList extends UVComponent {
  ...
  public boolean render(Div parent, ScriptObjectMirror currentUvNode) {
    ...
    useLabelKeyForPrint =
        currentUvNode.get("useLabelKeyForPrint") == null
            ? false
            : (Boolean) currentUvNode.get("useLabelKeyForPrint");

    // First, resolve the actual values using either String- or function based resolving and push
    // the textual values
    // into a list.
    ...
    if (listKeyObj instanceof String) {
      ...
      handleListKeyAsScriptObjectMirror(
          results, labelKey, (ScriptObjectMirror) listKeyObj, modelPropObj);
    }
  ...
  /**
   * Handle listKey as ScriptObjectMirror, where the modelProp in its turn may be a String or a
   * ScriptObjectMirror.
   */
  private void handleListKeyAsScriptObjectMirror(
      List<String> results, String labelKey, ScriptObjectMirror listKeyObj, Object modelPropObj) {
  ...
  /**
   * When the listKey is a string, we inspect the modelProp which may be either String or
   * ScriptObjectMirror.
   */
  private void handleListKeyAsString(
      List<String> results, String labelKey, String listKey, Object modelPropObj) {
    ...
    if (modelPropObj instanceof String) {
      ...
      if (eval instanceof ScriptObjectMirror) {
        ...
        if (evalSOM.isArray()) {
          ...
          throw new IllegalStateException(
              "Unhandled result type class in uv-list: "
                  + evalSOM.getClassName()
                  + " expected Array");
        }
      } else {
        throw new IllegalStateException(
            "Unhandled evaluation class in uv-list: "
      ...
      throw new IllegalStateException(
          "Unhandled modelPropObj class in uv-list: "
              + modelPropObj.getClass().getName()
              + " expected String");
    }
  }
  ...
  private void renderListResultWithSeparator(Div parent, List<String> results, String separator) {
    ...
    parent.add(
        new Paragraph(results.stream().collect(Collectors.joining(separator)))
            .setItalic()
  ...
  private void buildListResultFromArray(
      List<String> results,
      String labelKey,
      ScriptObjectMirror listKey,
    ...
    for (Object val : modelProps.values()) {
      ...
      if (listKey.isFunction()) {
        ...
        if (evaluatedValue instanceof Boolean) {
          ...
          if (evalBool) {
            ...
            if (result != null) {
              ...
              if (result instanceof Integer) {
                ...
                throw new IllegalStateException(
                    "UNHANDLED result type from list call: " + result.getClass().getName());
              }
          ...
          throw new IllegalStateException(
              "UNHANDLED evaluated value type in uv-list: " + evaluatedValue.getClass().getName());
        }
        index++;
      } else {
        throw new IllegalStateException(
            "UNHANDLED listKey type in uv-list: " + listKey.getClassName());
      }
    }
  }

  private void buildListResultFromStringModelProp(
      List<String> results, ScriptObjectMirror listKey, String modelPropObj, String labelKey) {
