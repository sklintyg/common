...
/**
 * A way of handling Dates in our internal model that allows faulty user input, this is needed at
 * this stage because of the auto save function among other things. <br>
 * <br>
 * This class contains util methods for various things such as getting a string as a {@link
 * LocalDate} etc.
 *
 * @author erik
 */
public class InternalDate {
  ...
  private static final String DATE_FORMAT =
      "[1-2][0-9]{3,3}(-((0[1-9])|(1[0-2]))(-((0[1-9])|([1-2][0-9])|(3[0-1]))))";
  ...
  public LocalDate asLocalDate() {
    ...
    try {
      ...
      throw new ModelException(
          String.format(
              "Could not parse %s to LocalDate, failed with message: %s", date, ie.getMessage()));
    }
    return localDate;
  }
  ...
  public boolean vagueDateAfterDate(LocalDate dateLimit) {
    ...
    // This needs to be done on the string level since date parts can be 00, not parsable by
    // LocalDate.parse
  ...
  public boolean isReasonable() {
    ...
    return this.asLocalDate().isAfter(MIN_DATE.asLocalDate())
        && this.asLocalDate().isBefore(MAX_DATE.asLocalDate());
  }
  ...
  public boolean isBefore(LocalDate providedDate) {
    return date != null && this.asLocalDate().isBefore(providedDate);
  }
