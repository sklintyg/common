...
public class IntygModuleRegistryImpl implements IntygModuleRegistry, ApplicationContextAware {
  ...
  private static final String VERSIONED_MODULE_API_BEANID_TEMPLATE =
      MODULE_API_BEAN_PREFIX + "%s.v%s";
   ...
   */
  @Autowired private List<ModuleEntryPoint> moduleEntryPoints;
  ...
  @Override
  public ModuleApi getModuleApi(String intygType, String intygTypeVersion)
      throws ModuleNotFoundException {

    if (Strings.isNullOrEmpty(intygType) || Strings.isNullOrEmpty(intygTypeVersion)) {
      throw new ModuleNotFoundException(
          "intygType and intygTypeVersion is required, got '"
              + intygType
              + "' and '"
              + intygTypeVersion
              + "'");
  ...
  @Override
  public String resolveVersionFromUtlatandeJson(String intygType, String internalModel)
      throws ModuleNotFoundException {
    try {
  ...
  private ModuleApi getVersionedModuleApiBean(String intygType, String intygTypeVersion)
      throws ModuleNotFoundException {

    try {
      // Majorversion defines model version, we dont really care about the minor (text) version in
      // this context.
      if (Strings.isNullOrEmpty(intygTypeVersion)) {
        throw new ModuleNotFoundException(
            "Can not resolve ModuleApiBean without intygTypeVersion - got '"
                + intygTypeVersion
                + "'");
      }
      ...
      final String beanName =
          String.format(VERSIONED_MODULE_API_BEANID_TEMPLATE, intygType, majorVersion);

      final Object bean = applicationContext.getBean(beanName);
      if (bean instanceof ModuleApi) {
        LOG.debug(
            String.format(
                "Resolved bean named '%s' as instance of %s", beanName, bean.getClass().getName()));
        return (ModuleApi) bean;
      } else {
        throw new ModuleNotFoundException(
            "Resolved bean (" + beanName + ") was not an instance of ModuleApi bean?");
      }
    } catch (BeansException e) {
      throw new ModuleNotFoundException(
          "Exception while trying to look up ModuleApi bean with for intygType '"
              + intygType
              + "', intygTypeVersion '"
              + intygTypeVersion
              + "'",
