...
public final class ValidatorUtil {
  ...
  /**
   * Validates that the supplied date is parsable and reasonable. Reasonable is defined as somewhere
   * between InternalDate.MIN_DATE and InternalDate.MAX_DATE, typically 1900-01-01 to 2099-12-31.
   *
   * @param date Date to validate.
   * @param validationMessages List of validationMessages. Any validation errors and/or warnings are
   *     added to this list.
  ...
  /**
   * Validates that the supplied interval is parsable and reasonable. Reasonable is defined as
   * somewhere between InternalDate.MIN_DATE and InternalDate.MAX_DATE, typically 1900-01-01 to
   * 2099-12-31.
   *
   ...
   * @return True if valid, false otherwise.
   */
  public static boolean validateInternalDateInterval(
    ...
    return validateDate(interval.getFrom(), validationMessages, category, field, message)
        ...
        && validateInterval(
            interval.getFrom(), interval.getTom(), validationMessages, category, field);
  }

  private static boolean validateInterval(
      InternalDate start,
      InternalDate end,
      List<ValidationMessage> validationMessages,
      String category,
  ...
  /**
   * Performs the normal date validation {@link ValidatorUtil#validateDate(InternalDate, List,
   * String, String, String)} as well as checking if the supplied date is in the future. If future,
   * a {@link ValidationMessage} of type {@link ValidationMessageType#OTHER} is added to the
   * supplied list of validationMessages.
   *
   * @param date Date to validate.
   * @param validationMessages List of validationMessages. Any validation errors and/or warnings are
   *     added to this list.
   ...
   * @return True if date was valid according to {@link ValidatorUtil#validateDate(InternalDate,
   *     List, String, String, String, String)}.
   */
  public static boolean validateDateAndCheckIfFuture(
      InternalDate date,
      List<ValidationMessage> validationMessages,
      String category,
      String field,
      String futureErrorMessage) {
    return validateDateAndCheckIfFuture(
        date, validationMessages, category, field, futureErrorMessage, null);
  }
    ...
    if (date.isValidDate() && date.asLocalDate().isAfter(LocalDate.now())) {
      if (questionId != null) {
        ...
        ValidatorUtil.addValidationError(
            validationMessages, category, field, ValidationMessageType.OTHER, futureErrorMessage);
      }
    }
    return isValid;
  }

  public static void validateVardenhet(
      GrundData grundData, List<ValidationMessage> validationMessages) {
    if (Strings.nullToEmpty(grundData.getSkapadAv().getVardenhet().getPostadress())
        .trim()
        .isEmpty()) {
      addValidationError(
          validationMessages,
    ...
    if (Strings.nullToEmpty(grundData.getSkapadAv().getVardenhet().getPostnummer())
        .trim()
        .isEmpty()) {
      addValidationError(
          validationMessages,
    ...
    } else if (!STRING_VALIDATOR.validateStringAsPostalCode(
        grundData.getSkapadAv().getVardenhet().getPostnummer())) {
      addValidationError(
          validationMessages,
          CATEGORY_VARDENHET,
          "grunddata.skapadAv.vardenhet.postnummer",
    ...
    if (Strings.nullToEmpty(grundData.getSkapadAv().getVardenhet().getPostort()).trim().isEmpty()) {
      addValidationError(
          validationMessages,
          CATEGORY_VARDENHET,
          "grunddata.skapadAv.vardenhet.postort",
          ValidationMessageType.EMPTY,
          "common.validation.patient.postort.missing");
    }

    if (Strings.nullToEmpty(grundData.getSkapadAv().getVardenhet().getTelefonnummer())
        .trim()
        .isEmpty()) {
      addValidationError(
          validationMessages,
          ...
          ValidationMessageType.EMPTY,
          "common.validation.patient.telefon.missing");
    }
  }
  ...
  public static void addValidationErrorWithQuestionId(
      List<ValidationMessage> validationMessages,
      String category,
      String field,
      ValidationMessageType type,
  ...
  public static void addValidationErrorWithQuestionId(
    ...
    validationMessages.add(
        ValidationMessage.create(category, field, type, msg, dynamicLabel, questionId));
    LOG.debug(field + " " + msg);
  }
  ...
  /**
   * Check for null or empty String (after trimming), if so add a validation error for field with
   * errorCode.
   *
   ...
   */
  public static AssertionResult assertDescriptionNotEmpty(
      List<ValidationMessage> validationMessages,
      String beskrivning,
      String category,
      String field,
    ...
    if (beskrivning == null || beskrivning.trim().isEmpty()) {
      addValidationError(
          validationMessages, category, field, ValidationMessageType.EMPTY, errorCode);
  ...
  public static AssertionResult assertDescriptionNotEmptyWithQuestionId(
      List<ValidationMessage> validationMessages,
      String beskrivning,
      String category,
      String field,
  ...
  public static ValidateDraftResponse buildValidateDraftResponse(
      List<ValidationMessage> validationMessages) {
    List<ValidationMessage> errors =
        validationMessages.stream().filter(isValidationError()).collect(Collectors.toList());
    List<ValidationMessage> warnings =
        validationMessages.stream().filter(isValidationWarning()).collect(Collectors.toList());
  ...
  public static boolean isInvalidTjanstgoringstid(String tjanstgoringstid) {
    return tjanstgoringstid != null
        && (Ints.tryParse(tjanstgoringstid) == null && Doubles.tryParse(tjanstgoringstid) == null);
  }
  ...
  /**
   * Since the validator assertions doesn't throw exceptions on assertion failure, they instead
   * return an assertion result. This might be used to implement conditional logic based on if an
   * assertion {@link #failed()} or was {@link #success()}ful.
   */
