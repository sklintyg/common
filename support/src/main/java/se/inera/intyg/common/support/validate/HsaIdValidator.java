...
/**
 * Performs validation of a HSA id on the form 'SE[16]xxxxxxxxxx-nnnn'. The following rules are
 * checked:
 *
 * <ul>
...
public class HsaIdValidator implements RootValidator {
  ...
  private static final Pattern LOCAL_ID_VALID_PATTERN =
  ...
  private void checkHSALength(String extension, List<String> result) {
    if (extension.length() > MAX_EXTENSION_LENGTH) {
      result.add(
          String.format("HSA id '%s' exceeded the maximum length of 31 charcters", extension));
    }
  }
