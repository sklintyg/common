...
/**
 * A way of handling date intervals in our internal model that allows faulty user input, this is
 * needed at this stage because of the auto save function among other things. <br>
 * <br>
 * This class contains util methods for various things such as getting the start or end dates as
 * {@link LocalDate}[s] etc.
 *
 * @author erik
 */
public class InternalLocalDateInterval {
  ...
  public boolean isReasonable() {
    boolean reasonableFrom =
        this.from.asLocalDate().isAfter(MIN_DATE.asLocalDate())
            && this.from.asLocalDate().isBefore(MAX_DATE.asLocalDate());
    boolean reasonableTo =
        this.tom.asLocalDate().isAfter(MIN_DATE.asLocalDate())
