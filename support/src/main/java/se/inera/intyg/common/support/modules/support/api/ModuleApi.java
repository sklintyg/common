...
/**
 * The module API defines methods that interact with one of the tree models that every module
 * handles:
 *
 * <ul>
 ...
 * </ul>
 *
 * <p>There exists methods for converting between models, generate PDFs, interact with the internal
 * model and extract meta data.
 */
public interface ModuleApi extends ModuleFacadeApi {

  /**
   * Validates the internal model. The status (complete, incomplete) and a list of validation errors
   * is returned.
   *
  ...
  default ValidateDraftResponse validateDraft(
      String internalModel, TypeAheadProvider typeAheadProvider) throws ModuleException {
   ...
   * @param applicationOrigin the context from which this method was called (i.e Webcert or Mina
   *     intyg)
   * @param utkastStatus Status indicating whether the certificate we're printing is a draft or not.
   *     Triggers the "UTKAST" or "UTKAST_LOCKED" watermark.
   * @return a {@link PdfResponse} consisting of a binary stream containing a PDF data and a
   *     suitable filename
   ...
   *
   * <p>A list of optional fields is required which specifies which of the optional fields in the
   * PDF should be filled out. The format, meaning and syntax of the optionalFields id's is
   * determined within each implementing module project.
   *
   * @param internalModel the internal model to generate a PDF from
   * @param applicationOrigin the context from which this method was called (i.e Webcert or Mina
   *     intyg)
   ...
   *
   * <p>The model is prepopulated using data contained in the {@link CreateNewDraftHolder}
   * parameter.
   *
   * @param draftCertificateHolder The id of the new internal model, the {@link HoSPersonal} and
   ...
   * @throws ModuleException if the new model could not be created, typically because the conversion
   *     to String failed
   */
   ...
   *
   * <p>The model is prepopulated using data contained in the {@link CreateNewDraftHolder} parameter
   * and template.
   *
   * @param draftCopyHolder the id of the new internal model, the {@link HoSPersonal} and optional
   *     {@link se.inera.intyg.common.support.model.common.internal.Patient} data.
   ...
   *
   * <p>The model is prepopulated using data contained in the {@link CreateNewDraftHolder}
   * parameter, template and the comment if any.
   *
   * @param draftCopyHolder the id of the new internal model, the {@link HoSPersonal} and optional
   *     {@link se.inera.intyg.common.support.model.common.internal.Patient} data.
   * @param template the Utlatande to be used as a template for the new internal model.
   * @param comment optional comment of why no new medicinal information can be entered into new
   *     Utlatande.
   */
  default String createCompletionFromTemplate(
      CreateDraftCopyHolder draftCopyHolder, Utlatande template, String comment)
      throws ModuleException {
   ...
   * @param logicalAddress the logical address of system from the certificate is requested, i.e
   *     Intygstjansten
   * @param recipientId the recipient id for the requester, used to determine which statuses which
   *     should be returned
  ...
  /**
   * Determine whether a notification about changed state in a certificate should be sent, using
   * current and persisted model states.
   *
   ...
   */
  String updateBeforeSave(String internalModel, HoSPersonal hosPerson, LocalDateTime created)
      throws ModuleException;

  /**
   * Returns an updated version of the internal model for saving, with new patient information
   * applied.
   *
   * @param internalModel the internal model to use as a base
   ...
   * @throws ModuleException if the mapping of the internal model to String failed
   */
  String updateBeforeSave(String internalModel, Patient patient, LocalDateTime created)
      throws ModuleException;
  ...
  /**
   * Returns an updated version of the internal model for viewing, with new patient information
   * applied.
   *
   * @param internalModel the internal model to use as a base
   * @param patient the patient complement the model with
   ...
   * @throws ModuleException if the mapping of the internal model to String failed
   */
  String updateBeforeViewing(String internalModel, Patient patient, LocalDateTime created)
      throws ModuleException;
   ...
   */
  String createRevokeRequest(Utlatande utlatande, HoSPersonal skapatAv, String meddelande)
      throws ModuleException;
  ...
  /**
   * Returns an instance of the implementation of Utlatande that this module handles. Uses the
   * provided created timestamp for unit mapping instead of the signing date.
   *
   * @param utlatandeJson the model represented as JSON (internal)
   ...
   * @throws ModuleException if the mapper could not read the Utlatande
   */
  Utlatande getUtlatandeFromJson(String utlatandeJson, LocalDateTime created)
   ...
   *
   * <p>Statistiktjänsten handles RegisterCertificate of version 3. All certificates which are
   * communicated (FK7263) in other ways need to be converted to RegisterCertificate version 3.0
   * before they are sent to Statistiktjänsten.
   *
   * <p>If the XML already is in correct version then nothing should be done and inputXml can be
   * returned.
   *
   ...
   * @return a mapping from frage id to a list of property handles in the JSON-representation of the
   *     certificate
   * @throws ModuleException if the version module api could not be found
   */
  Map<String, List<String>> getModuleSpecificArendeParameters(
      Utlatande utlatande, List<String> frageIds) throws ModuleException;

  /**
   * Creates a new internal representation, which is meant to be used as a renewal of a certificate
   * based on template.
   *
   * <p>This should clear some fields specific to the certificate type which should not be
   * transferred to the renewal copy.
   *
   * <p>NOTE: This can and will change fields in template before writing it to a new String. Do
   * *NOT* use the template after calling this method.
   *
   ...
   * @throws ModuleException if the conversion threw an exception
   */
  String createRenewalFromTemplate(CreateDraftCopyHolder draftCopyHolder, Utlatande template)
      throws ModuleException;
   ...
   * Defaults to FK presets (with no predessorId, <1, ResolveOrder.PU_NAME_ONLY> and <2,
   * ResolveOrder.PARAMS_NAME_ONLY>).
   */
  ...
  /**
   * Returns critera for an intyg's type, major version and age (time since signed) a source
   * candidate intyg must match in order to be a copy-from-candicate when creating a new draft with
   * {@link
   * se.riv.clinicalprocess.healthcond.certificate.createdraftcertificateresponder.v3.CreateDraftCertificateResponderInterface}.
   *
   * <p>Since most types does not supports this, the defaults is none.
   */
  ...
  /**
   * Returns a mapping implementation between this module certificate type and another module
   * certificate type. E.g. mapping between FK7804 and AG7804.
   *
   ...
   */
  default Optional<ValidateDraftCreationResponse> validateDraftCreation(
      Set<String> existingRelatedCertificates) {
    return Optional.empty();
  }

  /**
   * Returns additional metadata that can be stored outside the xml-representation of the
   * certificate. By default no additional metadata is provided.
   *
   * @param certificate the certificate to get additional information about
   * @return Optional additional metadata or empty
   * @throws ModuleException if the certificate could not be converted to internal
   */
  default Optional<AdditionalMetaData> getAdditionalMetaData(Intyg certificate)
      throws ModuleException {
    return Optional.empty();
  }
