...
public class SamordningsnummerValidator extends PersonnummerValidator {
  ...
  /**
   * Samordningsnummer have 60 added to the day. In order to calculate the birth day of the citizen,
  ...
  protected LocalDate getBirthDay(String birthDate) {
    ...
    // In case we got a birthDate YYMMdd or YYYYMMdd we normalize it before checking if it is a
    // samordningsnummer.
    ...
    if (!isSamordningsNummer(Personnummer.createPersonnummer(pnr))) {
      throw new IllegalArgumentException(
          "Personnummer " + pnr + " is not a valid 'samordningsnummer");
    }
