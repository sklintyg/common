 ...
 *
 * <ul>
 *   <li>Strict dash check: If the dash should be validated. If the citizen is 100 years old or
 *       more, the separator should be <code>+</code>, otherwise <code>-</code>.
 *   <li>Reference date: By default all personnummer are validated against the current date. Other
 *       dates can be used by setting {@link #setReferenceDate(java.time.LocalDate)}. Useful for
 *       unit testing.
 * </ul>
 *
...
public class PersonnummerValidator implements RootValidator {
  ...
  private static final Pattern PERSONNUMMER_PATTERN =
      Pattern.compile("(\\d{8})([+-])(\\d{3})(\\d)");
  ...
  public List<String> validateExtension(String pnr) {
    ...
    if (!m.matches()) {
      result.add(
          String.format("Could not parse the SSN '%s' (format should be 'yyyyMMdd-nnnn')", pnr));
      return result;
    }
  ...
  /**
   * Check that the date is in a valid range. It must not be older that {@link
   * #FIRST_PERSONNUMMER_DATE} or later than {@link #referenceDate}.
   *
   * @param pnr The personnummer. Used in validation messages.
  ...
  private void checkSeparator(
      String pnr, LocalDate birthday, String separator, List<String> result) {
    if (strictSeparatorCheck) {
      ...
      if (ageMoreThan100 && dashSeparator) {
        result.add(
            String.format(
                "The SSN '%s' is invalid - citizen is over 100 years old but the separator '-' is used",
                pnr));
      }
      if (!ageMoreThan100 && !dashSeparator) {
        result.add(
            String.format(
                "The SSN '%s' is invalid - citizen is under 100 years old but the separator '+' is used",
                pnr));
      }
    }
  }
  ...
  private void checkChecksum(
      String pnr, String dateString, String nnn, int mod10, List<String> result) {
   ...
   *
   * @return The reference date set with {@link #setReferenceDate(LocalDate)} or todays date
   *     owtherwise.
   */
  ...
  /**
   * This is the only calculation that differs between 'personnummer' and 'samordningsnummer'. Doing
   * the calculation in a separate method makes it overridable.
   *
