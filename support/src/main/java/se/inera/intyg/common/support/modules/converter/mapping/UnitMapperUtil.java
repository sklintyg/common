...
/**
 * Utility class for mapping IDs and names based on a configuration file. The configuration file is
 * loaded at startup and contains mappings for care providers, issuing unit as well as a timestamp
 * for when this mapping should go into effect. This is enabling avbolagisering/bolagisering in the
...
public class UnitMapperUtil {
   ...
   */
  public MappedUnit getMappedUnit(
      final String originalCareProviderId,
    ...
    final var issuedUnitMapping =
        findIssuedUnitMapping(originalIssuedUnitId, certificateIssuedDate);
  ...
  private Optional<IssuedUnitInfo> findIssuedUnitMapping(
      final String issuedUnitId, LocalDateTime certificateIssuedDate) {
    ...
    return unitMappingConfigLoader.getUnitMappings().stream()
        ...
        .filter(
            unitMapping ->
                unitMapping.issuedUnitMapping() != null
                    && unitMapping.issuedUnitMapping().containsKey(unitMappingKey))
        .filter(
            unitMapping ->
                (unitMapping.issuedDateTimeFrom() == null || certificateIssuedDate == null)
                    || (unitMapping.issuedDateTimeFrom().isBefore(certificateIssuedDate)
                            || unitMapping.issuedDateTimeFrom().equals(certificateIssuedDate))
                        && (unitMapping.issuedDateTimeTo() == null
                            || unitMapping.issuedDateTimeTo().isAfter(certificateIssuedDate)))
        .findFirst()
  ...
  private Optional<CareProviderInfo> findCareProviderMapping(final String careProviderId) {
    ...
    return unitMappingConfigLoader.getUnitMappings().stream()
        .filter(
            mappingConfig ->
                LocalDateTime.now().isAfter(mappingConfig.datetime())
