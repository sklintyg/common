...
class UnitMapperUtilTest {
  ...
  private static final List<UnitMapping> CARE_PROVIDER_MAPPINGS =
      List.of(
          new UnitMapping(
              "Region Stockholm",
  ...
  private static final List<UnitMapping> ISSUED_UNIT_MAPPINGS =
      List.of(
          new UnitMapping(
              "Region Gävleborg",
              "Bolagisering av primärvården",
  ...
  private static final List<UnitMapping> ISSUED_UNIT_MAPPINGS_WITHOUT_ISSUED_DATE_CONFIG =
      List.of(
          new UnitMapping(
              "Region Gävleborg",
              "Bolagisering av primärvården",
  ...
  private static final List<UnitMapping> ISSUED_UNIT_MAPPINGS_WITH_FROM_AND_TO_ISSUED_DATE_CONFIG =
      List.of(
          new UnitMapping(
              "Region Gävleborg",
              "Bolagisering av primärvården",
  ...
  @Mock private UnitMappingConfigLoader unitMappingConfigLoader;

  @InjectMocks private UnitMapperUtil unitMapperUtil;
  ...
  class DecorateUnitTests {
    ...
    void shouldNotTryToDecorateIfVardgivaridNull() {
      ...
      when(utlatande.getGrundData().getSkapadAv()).thenReturn(skapadAv);
      when(utlatande.getGrundData().getSkapadAv().getVardenhet()).thenReturn(vardenhet);
      when(utlatande.getGrundData().getSkapadAv().getVardenhet().getVardgivare())
          .thenReturn(vardgivare);
      when(utlatande.getGrundData().getSkapadAv().getVardenhet().getVardgivare().getVardgivarid())
          .thenReturn(null);
      unitMapperUtil.decorateWithMappedCareProvider(utlatande);
      verifyNoInteractions(unitMappingConfigLoader);
    }
  ...
  class GetMappedUnitTests {
    ...
    void shouldMapCareProviderCorrectly(
        String originalId, String originalName, String expectedId, String expectedName) {
      when(unitMappingConfigLoader.getUnitMappings()).thenReturn(CARE_PROVIDER_MAPPINGS);
    ...
    private static Stream<Arguments> provideTestCases() {
      return Stream.of(
          Arguments.of(
              "TSTNMT2321000156-ALFA", "Original Name", "TSTNMT2321000156-BETA", "Beta Regionen"),
          Arguments.of(
              "TSTNMT2321000156-DELTA", "Original Name", "TSTNMT2321000156-DELTA", "Original Name"),
          Arguments.of(
              "tstnmt2321000156-alfa", "Original Name", "TSTNMT2321000156-BETA", "Beta Regionen"),
    ...
    void shouldReturnIssuedUnitMappingIfCertificateIssuedDateIsNull() {
      when(unitMappingConfigLoader.getUnitMappings())
          .thenReturn(ISSUED_UNIT_MAPPINGS_WITHOUT_ISSUED_DATE_CONFIG);
  ...
  class WhenIssuedDateFromAndToIsPresentInConfiguration {
    ...
    void shouldMapUnitIfCertificateIsCreatedWithinFromAndToDate() {
      when(unitMappingConfigLoader.getUnitMappings())
          .thenReturn(ISSUED_UNIT_MAPPINGS_WITH_FROM_AND_TO_ISSUED_DATE_CONFIG);
