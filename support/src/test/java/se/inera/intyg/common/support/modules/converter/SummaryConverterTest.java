...
class SummaryConverterTest {
  ...
  void shallConvertToSummary() throws ModuleException {
    ...
    final var expectedSummary = CertificateSummary.builder().label(LABEL).value(VALUE).build();
  ...
  void shallReturnEmptySummaryIfModuleExceptionIsThrown() {
    ...
    when(summaryConverter.convert(moduleApi, intyg))
        .thenThrow(new ModuleException("Could not convert summary"));
